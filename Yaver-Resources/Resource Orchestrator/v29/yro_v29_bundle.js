(function () {
  ...
  var VERSION = 'v29';
  var NS = 'YRO_V29';
  var PANEL_ID = 'yro_panel_v29';
  var STYLE_ID = 'yro_css_v29';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v29';
  var LS_KEY = 'yro_v29_state';
  ...
  function cleanupOlderInstances() {
    ...
    try { if (window.YRO_V29) delete window.YRO_V29; } catch (e3) {}
...
(function () {
  ...
  Y.log('data module loaded ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦');
})();
...
(function () {
          ...
          while (true) {
            ...
            try {
              ...
              if (msg.indexOf('429') >= 0 || msg.indexOf('HTTP 429') >= 0) {
                ...
                Y.warn('HTTP 429 rate limit for village', vid, 'ÃƒÂ¢Ã¢â€šÂ¬Ã¢â‚¬Â waiting', wait + 'ms');
                await Y.sleep(wait);
                if (tries < 4) continue;
              }
              throw e;
  ...
  Y.log('fetch module loaded ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦');
})();
...
(function () {
  ...
  function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    // v29: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
  ...
  Y.log('compute module loaded ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦');
})();
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v29{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  gap:8px;
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v29" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v29)</span></div>
  <div style="display:flex; align-items:center;">
    ...
    <button class="yro_btn" id="yro_close_v29">X</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v29">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v29"></div></div>
  <span id="yro_prog_txt_v29" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v29" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v29">Copy Test Results</button>
</div>

<div class="yro_msg" id="yro_msg_v29" style="color:#0a6;">Ready</div>

<div id="yro_content_v29">
  <div class="yro_sec_head">
    <div>1) Villages - Production (per hour + 24h) (<span id="yro_t1_title_v29">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v29">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v29"></select>
      <button class="yro_mini" id="yro_t1_pick_v29">Pick...</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v29" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v29"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages - Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v29">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v29">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v29"></select>
      <button class="yro_mini" id="yro_t2_pick_v29">Pick...</button>
      <span class="yro_small" id="yro_scan_ts_v29"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v29" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v29"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v29"></div>

  <div class="yro_msg" id="yro_plan_meta_v29" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Cap Ceiling %:</b> - | <b>Surplus Cap %:</b> - | <b>Merch Used:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -
  </div>

  <div class="yro_msg yro_plan_diag" id="yro_plan_diag_v29">Diagnostics: -</div>

  <div class="yro_sec_head">
    <div id="yro_plan_target_title_v29">TARGET GROUP</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_target_v29">Copy TARGET GROUP</button>
    </div>
  </div>
  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v29"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_surplus_title_v29">SURPLUS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_surplus_v29">Copy SURPLUS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v29"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_ship_title_v29">SHIPMENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_ship_v29">Copy SHIPMENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v29"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v29');
    var tx = Y.qs('#yro_prog_txt_v29');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v29', st.groups.sel1);
    fill('#yro_t2_group_sel_v29', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v29">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v29">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v29">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v29">Pick...</button>
    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v29" type="number" min="1" max="100" step="1" value="${st.modeA.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v29" type="number" min="1" max="100" step="1" value="${st.modeA.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Computed Fill %</label><input id="yro_A_computed_fill_v29" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_A_merch_used_v29" type="number" value="" style="width:70px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_A_plan_v29">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v29">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_B_parents_v29">${optionsHTML(st.groups.B_parents)}</select></div>
    <button class="yro_mini" id="pick_B_parents_v29">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_B_children_v29">${optionsHTML(st.groups.B_children)}</select></div>
    <button class="yro_mini" id="pick_B_children_v29">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v29">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v29">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_B_parent_reserve_v29" type="number" min="0" max="100" step="1" value="${st.modeB.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Children Max Fill %</label><input id="yro_B_child_max_v29" type="number" min="0" max="100" step="1" value="${st.modeB.childrenMaxFillPct}" style="width:78px;"></div>
    <div class="yro_kv"><label>Surplus Cap %</label><input id="yro_B_scap_v29" type="number" min="0" max="100" step="1" value="${st.modeB.surplusCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Child Fill %</label><input id="yro_B_computed_fill_v29" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_B_child_merch_v29" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_B_parent_merch_v29" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_B_plan_v29">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v29">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${(st.orchestrator.mode === 'coin' || st.orchestrator.mode === 'funnel') ? 'active' : ''}" data-mode="coin">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Sources</label><select id="yro_C_sources_v29">${optionsHTML(st.groups.C_sources)}</select></div>
    <button class="yro_mini" id="pick_C_sources_v29">Pick...</button>
    <div class="yro_kv"><label>Coin</label><select id="yro_C_coin_v29">${optionsHTML(st.groups.C_coin)}</select></div>
    <button class="yro_mini" id="pick_C_coin_v29">Pick...</button>
    <div class="yro_kv"><label>Buffer / Surplus</label><select id="yro_C_buffer_v29">${optionsHTML(st.groups.C_buffer)}</select></div>
    <button class="yro_mini" id="pick_C_buffer_v29">Pick...</button>
    <div class="yro_kv"><label>Source Reserve %</label><input id="yro_C_reserve_v29" type="number" min="0" max="100" step="1" value="${st.modeC.sourceReservePct}" style="width:72px;"></div>
    <div class="yro_kv"><label>Buffer Cap %</label><input id="yro_C_bcap_v29" type="number" min="0" max="100" step="1" value="${st.modeC.bufferCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Additional Coins</label><input id="yro_C_additional_coins_v29" type="number" value="" style="width:84px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_C_merch_used_v29" type="number" value="" style="width:72px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_C_plan_v29">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v29">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v29">ÃƒÂ¢Ã…â€œÃ¢â‚¬â€œ</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v29" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v29">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v29">Select none</button>
    <span class="yro_small" id="yro_picker_count_v29"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v29">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v29">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v29">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v29');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v29');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v29').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v29').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v29').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v29').value);
    });

    box.querySelector('#yro_picker_none_v29').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v29').value);
    });

    box.querySelector('#yro_picker_save_v29').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v29').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v29');
    var diagEl = Y.qs('#yro_plan_diag_v29');
    var tTbl = Y.qs('#yro_plan_target_tbl_v29');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v29');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v29');
    var targetTitleEl = Y.qs('#yro_plan_target_title_v29');
    var surplusTitleEl = Y.qs('#yro_plan_surplus_title_v29');
    var shipTitleEl = Y.qs('#yro_plan_ship_title_v29');
    var copyTargetEl = Y.qs('#yro_copy_plan_target_v29');
    var copySurplusEl = Y.qs('#yro_copy_plan_surplus_v29');
    var copyShipEl = Y.qs('#yro_copy_plan_ship_v29');

    function renderEmpty() {
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v29');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v29');
      var modeBFillEl = Y.qs('#yro_B_computed_fill_v29');
      var modeBChildMerchEl = Y.qs('#yro_B_child_merch_v29');
      var modeBParentMerchEl = Y.qs('#yro_B_parent_merch_v29');
      var modeCCoinsEl = Y.qs('#yro_C_additional_coins_v29');
      var modeCMerchEl = Y.qs('#yro_C_merch_used_v29');
    ...
    if (String(plan.mode || '').toLowerCase() === 'balance' && plan.targetSummary) {
      ...
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v29');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v29');
      var computedFillEl0 = Y.qs('#yro_B_computed_fill_v29');
      var childMerchEl0 = Y.qs('#yro_B_child_merch_v29');
      var parentMerchEl0 = Y.qs('#yro_B_parent_merch_v29');
      var coinAddEl0 = Y.qs('#yro_C_additional_coins_v29');
      var coinMerchEl0 = Y.qs('#yro_C_merch_used_v29');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl = Y.qs('#yro_B_computed_fill_v29');
      var childMerchEl = Y.qs('#yro_B_child_merch_v29');
      var parentMerchEl = Y.qs('#yro_B_parent_merch_v29');
      var modeAFillEl2 = Y.qs('#yro_A_computed_fill_v29');
      var modeAMerchEl2 = Y.qs('#yro_A_merch_used_v29');
      var coinAddEl1 = Y.qs('#yro_C_additional_coins_v29');
      var coinMerchEl1 = Y.qs('#yro_C_merch_used_v29');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillElC0 = Y.qs('#yro_B_computed_fill_v29');
      var childMerchElC0 = Y.qs('#yro_B_child_merch_v29');
      var parentMerchElC0 = Y.qs('#yro_B_parent_merch_v29');
      var modeAFillElC0 = Y.qs('#yro_A_computed_fill_v29');
      var modeAMerchElC0 = Y.qs('#yro_A_merch_used_v29');
      var modeCCoinsEl = Y.qs('#yro_C_additional_coins_v29');
      var modeCMerchEl = Y.qs('#yro_C_merch_used_v29');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl2 = Y.qs('#yro_B_computed_fill_v29');
      var childMerchEl2 = Y.qs('#yro_B_child_merch_v29');
      var parentMerchEl2 = Y.qs('#yro_B_parent_merch_v29');
      var modeAFillEl3 = Y.qs('#yro_A_computed_fill_v29');
      var modeAMerchEl3 = Y.qs('#yro_A_merch_used_v29');
      var modeCCoinsEl2 = Y.qs('#yro_C_additional_coins_v29');
      var modeCMerchEl2 = Y.qs('#yro_C_merch_used_v29');
...
<tr>
  ...
  Y.log('ui module loaded ÃƒÂ¢Ã…â€œÃ¢â‚¬Â¦');
})();
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v29');
    ...
    var t2title = Y.qs('#yro_t2_title_v29');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v29', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v29');
      surplusGid = getSelValue('#yro_A_surplus_v29');
      var capA = getNumValue('#yro_A_cap_v29', st.modeA.capPct || 80);
      var scapA = getNumValue('#yro_A_scap_v29', st.modeA.surplusCapPct || 95);
      ...
      var parentGid = getSelValue('#yro_B_parents_v29');
      var childGid = getSelValue('#yro_B_children_v29');
      surplusGid = getSelValue('#yro_B_surplus_v29');
      var parentReservePct = getNumValue('#yro_B_parent_reserve_v29', st.modeB.parentReservePct || 1);
      var childrenMaxFillPct = getNumValue('#yro_B_child_max_v29', st.modeB.childrenMaxFillPct || 80);
      var surplusCapPct = getNumValue('#yro_B_scap_v29', st.modeB.surplusCapPct || 95);
      ...
      var sourceGid = getSelValue('#yro_C_sources_v29');
      var coinGid = getSelValue('#yro_C_coin_v29');
      var bufferGid = getSelValue('#yro_C_buffer_v29');
      var sourceReservePct = getNumValue('#yro_C_reserve_v29', st.modeC.sourceReservePct || 1);
      var bufferCapPct = getNumValue('#yro_C_bcap_v29', st.modeC.bufferCapPct || 95);
  ...
  function copyPlanSurplusBBCode() {
    ...
    if (mode === 'balance') {
      ...
      flashButton('#yro_copy_plan_surplus_v29', 'Copy SURPLUS');
      copyText(text, 'SURPLUS BBCode copied');
      return;
    }
    if (mode === 'coin') {
      text = buildCoinSourcesBufferBBCode(plan);
      flashButton('#yro_copy_plan_surplus_v29', (plan.bufferSummary && plan.bufferSummary.length) ? 'Copy SOURCES / BUFFER' : 'Copy SOURCES');
  ...
  function copyPlanShipmentsBBCode() {
    ...
    flashButton('#yro_copy_plan_ship_v29', 'Copy SHIPMENTS');
  ...
  function getActiveModeKey() {
    var active = Y.qs('#yro_modes_v29 .yro_mode_row.active');
  ...
  function buildCurrentModeInputRows() {
    ...
    if (mode === 'balance') {
      return [
        ...
        ['Target', getSelectText('#yro_A_target_v29', st.groups.A_target)],
        ['Surplus', getSelectText('#yro_A_surplus_v29', st.groups.A_surplus)],
        ['Cap %', getInputText('#yro_A_cap_v29', st.modeA.capPct)],
        ['Surplus Cap %', getInputText('#yro_A_scap_v29', st.modeA.surplusCapPct)]
      ];
    }
    if (mode === 'push') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Parents', getSelectText('#yro_B_parents_v29', st.groups.B_parents)],
        ['Children', getSelectText('#yro_B_children_v29', st.groups.B_children)],
        ['Surplus', getSelectText('#yro_B_surplus_v29', st.groups.B_surplus)],
        ['Parent Reserve %', getInputText('#yro_B_parent_reserve_v29', st.modeB.parentReservePct)],
        ['Children Max Fill %', getInputText('#yro_B_child_max_v29', st.modeB.childrenMaxFillPct)],
        ['Surplus Cap %', getInputText('#yro_B_scap_v29', st.modeB.surplusCapPct)]
      ];
    }
    if (mode === 'coin') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Sources', getSelectText('#yro_C_sources_v29', st.groups.C_sources)],
        ['Coin', getSelectText('#yro_C_coin_v29', st.groups.C_coin)],
        ['Buffer / Surplus', getSelectText('#yro_C_buffer_v29', st.groups.C_buffer)],
        ['Source Reserve %', getInputText('#yro_C_reserve_v29', st.modeC.sourceReservePct)],
        ['Buffer Cap %', getInputText('#yro_C_bcap_v29', st.modeC.bufferCapPct)]
      ];
    }
  ...
  async function copyTestResults() {
    try {
      ...
      flashButton('#yro_copy_bb_v29', 'Copy Test Results');
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v29'), 'click', function () { Y.destroy(); });
    Y.on(Y.qs('#yro_load_v29'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v29');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v29'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v29');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v29');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v29'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v29');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v29');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v29'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v29'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v29'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v29'), 'click', function () { openPicker('t2'); });
    Y.on(Y.qs('#yro_copy_bb_v29'), 'click', function () { copyTestResults(); });
    Y.on(Y.qs('#yro_copy_plan_target_v29'), 'click', function () { copyPlanTargetBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_surplus_v29'), 'click', function () { copyPlanSurplusBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_ship_v29'), 'click', function () { copyPlanShipmentsBBCode(); });

    var modeBox = Y.qs('#yro_modes_v29');
    if (!modeBox) return;
    ...
    Y.on(modeBox, 'click', function (e) {
      ...
      if (t.id === 'yro_A_plan_v29') return planFromMode('balance');
      if (t.id === 'yro_B_plan_v29') return planFromMode('push');
      if (t.id === 'yro_C_plan_v29') return planFromMode('coin');

      if (t.id === 'yro_A_exec_v29' || t.id === 'yro_B_exec_v29' || t.id === 'yro_C_exec_v29') return executePlan();

      if (String(t.id || '').startsWith('pick_') && String(t.id || '').endsWith('_v29')) {
        var key = String(t.id).replace('pick_', '').replace('_v29', '');
      ...
      if (row && row.getAttribute) {
        ...
        if (mode) {
          st.orchestrator.mode = mode;
          Y.saveState();
          Y.qsa('#yro_modes_v29 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
    ...
    Y.on(modeBox, 'change', function (e) {
      ...
      if (t.id === 'yro_A_target_v29') st.groups.A_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_surplus_v29') st.groups.A_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parents_v29') st.groups.B_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_children_v29') st.groups.B_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_surplus_v29') st.groups.B_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_sources_v29') st.groups.C_sources = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_coin_v29') st.groups.C_coin = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_buffer_v29') st.groups.C_buffer = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_cap_v29') st.modeA.capPct = Y.safeInt(t.value, st.modeA.capPct);
      else if (t.id === 'yro_A_scap_v29') st.modeA.surplusCapPct = Y.safeInt(t.value, st.modeA.surplusCapPct);
      else if (t.id === 'yro_B_parent_reserve_v29') st.modeB.parentReservePct = Y.safeInt(t.value, st.modeB.parentReservePct);
      else if (t.id === 'yro_B_child_max_v29') st.modeB.childrenMaxFillPct = Y.safeInt(t.value, st.modeB.childrenMaxFillPct);
      else if (t.id === 'yro_B_scap_v29') st.modeB.surplusCapPct = Y.safeInt(t.value, st.modeB.surplusCapPct);
      else if (t.id === 'yro_C_reserve_v29') st.modeC.sourceReservePct = Y.safeInt(t.value, st.modeC.sourceReservePct);
      else if (t.id === 'yro_C_bcap_v29') st.modeC.bufferCapPct = Y.safeInt(t.value, st.modeC.bufferCapPct);
