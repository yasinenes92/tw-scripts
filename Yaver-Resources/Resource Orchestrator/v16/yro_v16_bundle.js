(function () {
  ...
  var VERSION = 'v16';
  var NS = 'YRO_V16';
  var PANEL_ID = 'yro_panel_v16';
  var STYLE_ID = 'yro_css_v16';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v16';
  var LS_KEY = 'yro_v16_state';
  ...
  try { if (window.YRO_V16) delete window.YRO_V16; } catch (e2) {}
  ...
  function defaultState() {
    return {
      ...
      orchestrator: { mode: 'balance', cap: 80, scap: 95, reserve: 1, ironDelta: 0 },
      cache: {
        groupsFetchedAt: 0,
        groupVillageIds: {},
        villages: {},
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v16{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  function buildPanel() {
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v16" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v16)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Developed by controleng</span>
    <button class="yro_btn" id="yro_close_v16">✖</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v16">Yükle / Yenile (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v16"></div></div>
  <span id="yro_prog_txt_v16" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v16" placeholder="Köy adı…" style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v16">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v16" style="color:#0a6;">Hazır ✅</div>

<div id="yro_content_v16">
  <div class="yro_sec_head">
    <div>1) Villages — Production (per hour + 24h) (<span id="yro_t1_title_v16">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v16">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v16"></select>
      <button class="yro_mini" id="yro_t1_pick_v16">Pick…</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v16" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v16"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages — Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v16">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v16">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v16"></select>
      <button class="yro_mini" id="yro_t2_pick_v16">Pick…</button>
      <span class="yro_small" id="yro_scan_ts_v16"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v16" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v16"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v16"></div>

  <div class="yro_msg" id="yro_plan_meta_v16" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Cap%:</b> - | <b>Reserve(each):</b> - | <b>Surplus Cap%:</b> - | <b>Iron Δ%:</b> - | <b>Shipments:</b> 0
  </div>

  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v16"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v16"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v16"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v16');
    var tx = Y.qs('#yro_prog_txt_v16');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v16', st.groups.sel1);
    fill('#yro_t2_group_sel_v16', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v16">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v16">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v16">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v16">Pick…</button>

    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v16" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v16" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_A_iron_v16" type="number" min="-50" max="50" step="1" value="${st.orchestrator.ironDelta}" style="width:60px;" title="Iron hedefi: Wood/Clay’e göre yüzde fark. Örn -30 => 100/100/70, +30 => 100/100/130"></div>

    <button class="yro_btn btn-confirm" id="yro_A_plan_v16">Plan</button>
    <button class="yro_btn btn-warn" id="yro_A_exec_tabs_v16">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_auto_v16">Execute (Auto)</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Sender</label><select id="yro_B_sender_v16">${optionsHTML(st.groups.B_sender)}</select></div>
    <button class="yro_mini" id="pick_B_sender_v16">Pick…</button>

    <div class="yro_kv"><label>Target</label><select id="yro_B_target_v16">${optionsHTML(st.groups.B_target)}</select></div>
    <button class="yro_mini" id="pick_B_target_v16">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v16">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v16">Pick…</button>

    <div class="yro_kv"><label>Reserve%</label><input id="yro_B_reserve_v16" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_B_cap_v16" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_B_scap_v16" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_B_iron_v16" type="number" min="-50" max="50" step="1" value="${st.orchestrator.ironDelta}" style="width:60px;" title="Iron hedefi: Wood/Clay’e göre yüzde fark."></div>

    <button class="yro_btn btn-confirm" id="yro_B_plan_v16">Plan</button>
    <button class="yro_btn btn-warn" id="yro_B_exec_tabs_v16">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_auto_v16">Execute (Auto)</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_C_target_v16">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v16">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_C_surplus_v16">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v16">Pick…</button>

    <div class="yro_kv"><label>Reserve%</label><input id="yro_C_reserve_v16" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_C_cap_v16" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_C_scap_v16" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_C_iron_v16" type="number" min="-50" max="50" step="1" value="${st.orchestrator.ironDelta}" style="width:60px;" title="Iron hedefi: Wood/Clay’e göre yüzde fark."></div>

    <button class="yro_btn btn-confirm" id="yro_C_plan_v16">Plan</button>
    <button class="yro_btn btn-warn" id="yro_C_exec_tabs_v16">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_auto_v16">Execute (Auto)</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v16">✖</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v16" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v16">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v16">Select none</button>
    <span class="yro_small" id="yro_picker_count_v16"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v16">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v16">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v16">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v16');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v16');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v16').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v16').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v16').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v16').value);
    });

    box.querySelector('#yro_picker_none_v16').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v16').value);
    });

    box.querySelector('#yro_picker_save_v16').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v16').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v16');
    if (metaEl) {
      metaEl.innerHTML =
        '<b>Mode:</b> ' + String(plan.mode || '-').toUpperCase() + ' | ' +
        '<b>Cap%:</b> ' + plan.meta.cap + ' | ' +
        '<b>Reserve(each):</b> ' + plan.meta.reserve + ' | ' +
        '<b>Surplus Cap%:</b> ' + plan.meta.scap + ' | ' + '<b>Iron Δ%:</b> ' + (plan.meta.ironDelta!=null?plan.meta.ironDelta:0) + ' | ' +
        '<b>Shipments:</b> ' + (plan.shipments ? plan.shipments.length : 0);
    }

    var tTbl = Y.qs('#yro_plan_target_tbl_v16');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v16');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v16');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v16');
    ...
    var t2title = Y.qs('#yro_t2_title_v16');
  ...
  function confirmTwoButtons(htmlMsg, onYes) {
    ...
    if (window.UI && UI.ConfirmationBox) UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v16', true);
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v16');
      surplusGid = getSelValue('#yro_A_surplus_v16');
      cap = getNumValue('#yro_A_cap_v16', 80);
      scap = getNumValue('#yro_A_scap_v16', 95);
      var ironDelta = getNumValue('#yro_A_iron_v16', 0);
      reserve = 0;
      ...
      senderGid = getSelValue('#yro_B_sender_v16');
      targetGid = getSelValue('#yro_B_target_v16');
      surplusGid = getSelValue('#yro_B_surplus_v16');
      reserve = getNumValue('#yro_B_reserve_v16', 1);
      cap = getNumValue('#yro_B_cap_v16', 80);
      scap = getNumValue('#yro_B_scap_v16', 95);
      var ironDelta = getNumValue('#yro_B_iron_v16', 0);
      ...
      Y.runtime.plan = {
        ...
        meta: { cap: cap, scap: scap, reserve: reserve, ironDelta: ironDelta, targetGid: targetGid, senderGid: senderGid, surplusGid: surplusGid },
      ...
      targetGid = getSelValue('#yro_C_target_v16');
      surplusGid = getSelValue('#yro_C_surplus_v16');
      reserve = getNumValue('#yro_C_reserve_v16', 1);
      cap = getNumValue('#yro_C_cap_v16', 80);
      scap = getNumValue('#yro_C_scap_v16', 95);
      var ironDelta = getNumValue('#yro_C_iron_v16', 0);
    ...
    st.orchestrator.ironDelta = (typeof ironDelta !== 'undefined') ? ironDelta : (st.orchestrator.ironDelta || 0);
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v16'), 'click', function () { Y.destroy(); });

    Y.on(Y.qs('#yro_load_v16'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v16');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v16'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v16');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v16');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v16'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v16');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v16');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v16'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v16'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v16'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v16'), 'click', function () { openPicker('t2'); });

    Y.on(Y.qs('#yro_copy_bb_v16'), 'click', function () { copyBBCode(); });

    var modeBox = Y.qs('#yro_modes_v16');
    if (modeBox) {
      // click delegation
      Y.on(modeBox, 'click', function (e) {
        ...
        if (t.id === 'yro_A_plan_v16') return planFromMode('balance');
        if (t.id === 'yro_B_plan_v16') return planFromMode('push');
        if (t.id === 'yro_C_plan_v16') return planFromMode('funnel');

        if (t.id === 'yro_A_exec_tabs_v16' || t.id === 'yro_B_exec_tabs_v16' || t.id === 'yro_C_exec_tabs_v16') return executeTabs();
        if (t.id === 'yro_A_exec_auto_v16' || t.id === 'yro_B_exec_auto_v16' || t.id === 'yro_C_exec_auto_v16') return executeAuto();

        if (String(t.id || '').startsWith('pick_') && String(t.id).endsWith('_v16')) {
          var key = String(t.id).replace('pick_', '').replace('_v16', '');
        ...
        if (row && row.getAttribute) {
          ...
          if (mode) {
            st.orchestrator.mode = mode;
            Y.saveState();
            Y.qsa('#yro_modes_v16 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
      ...
      Y.on(modeBox, 'change', function (e) {
        ...
        if (t.id === 'yro_A_target_v16') st.groups.A_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_A_surplus_v16') st.groups.A_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_sender_v16') st.groups.B_sender = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_target_v16') st.groups.B_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_surplus_v16') st.groups.B_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_target_v16') st.groups.C_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_surplus_v16') st.groups.C_surplus = Y.safeInt(t.value, 0);

        else if (t.id === 'yro_A_cap_v16' || t.id === 'yro_B_cap_v16' || t.id === 'yro_C_cap_v16') st.orchestrator.cap = Y.safeInt(t.value, st.orchestrator.cap);
        else if (t.id === 'yro_A_scap_v16' || t.id === 'yro_B_scap_v16' || t.id === 'yro_C_scap_v16') st.orchestrator.scap = Y.safeInt(t.value, st.orchestrator.scap);
        else if (t.id === 'yro_B_reserve_v16' || t.id === 'yro_C_reserve_v16') st.orchestrator.reserve = Y.safeInt(t.value, st.orchestrator.reserve);
