(function () {
  ...
  function loadOne(f) {
    var url = bust(BASE + f);
    console.log("[YRO Loader v6] loading:", url);
    return new Promise(function (res, rej) {
      $.ajax({ url: url, dataType: "script", cache: false, timeout: 30000 })
        .done(function () {
          console.log("[YRO Loader v6] OK:", f);
          res(true);
        })
        .fail(function (jq, st, er) {
          console.log("[YRO Loader v6] FAIL:", f, { st: st, er: er, http: jq && jq.status });
  ...
  (async function () {
    try {
      ...
      if (window[KEY] && typeof window[KEY].init === "function") {
        window[KEY].init();
        if (window.UI && UI.SuccessMessage) UI.SuccessMessage("Resource Orchestrator v6 Hazır ✅", 2000);
      } else {
        alert("YRO init (v6) bulunamadı.");
      }
    } catch (e) {
      console.error("[YRO Loader v6] ERROR:", e);
      if (window.UI && UI.ErrorMessage) UI.ErrorMessage("Resource Orchestrator v6 yüklenemedi ❌ (Console'a bak)", 6000);
