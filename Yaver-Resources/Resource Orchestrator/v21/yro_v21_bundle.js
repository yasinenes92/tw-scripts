(function () {
  ...
  var VERSION = 'v21';
  var NS = 'YRO_V21';
  var PANEL_ID = 'yro_panel_v21';
  var STYLE_ID = 'yro_css_v21';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v21';
  var LS_KEY = 'yro_v21_state';
  ...
  try { if (window.YRO_V21) delete window.YRO_V21; } catch (e2) {}
...
function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    surplusIds = (surplusIds || []).filter(function (id) { return !tSet[String(id)]; });
    // v21: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v21');
    ...
    var t2title = Y.qs('#yro_t2_title_v21');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v21', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v21');
      surplusGid = getSelValue('#yro_A_surplus_v21');
      cap = getNumValue('#yro_A_cap_v21', 80);
      scap = getNumValue('#yro_A_scap_v21', 95);
      var ironDelta = getNumValue('#yro_A_iron_v21', 0);
      reserve = 0;
      ...
      senderGid = getSelValue('#yro_B_sender_v21');
      targetGid = getSelValue('#yro_B_target_v21');
      surplusGid = getSelValue('#yro_B_surplus_v21');
      reserve = getNumValue('#yro_B_reserve_v21', 1);
      cap = 0; // auto
      scap = getNumValue('#yro_B_scap_v21', 95);
      var tolPct = getNumValue('#yro_B_tol_v21', 5);
      var ironDeltaRawEl = Y.qs('#yro_B_iron_v21');
      ...
      var capEl = Y.qs('#yro_B_cap_v21'); if (capEl) capEl.value = (cap >= 2 ? cap : '');
      ...
      targetGid = getSelValue('#yro_C_target_v21');
      surplusGid = getSelValue('#yro_C_surplus_v21');
      reserve = getNumValue('#yro_C_reserve_v21', 1);
      cap = getNumValue('#yro_C_cap_v21', 80);
      scap = getNumValue('#yro_C_scap_v21', 95);
      var ironDelta = getNumValue('#yro_C_iron_v21', 0);
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v21'), 'click', function () { Y.destroy(); });

    Y.on(Y.qs('#yro_load_v21'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v21');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v21'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v21');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v21');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v21'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v21');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v21');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v21'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v21'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v21'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v21'), 'click', function () { openPicker('t2'); });

    Y.on(Y.qs('#yro_copy_bb_v21'), 'click', function () { copyBBCode(); });

    var modeBox = Y.qs('#yro_modes_v21');
    if (modeBox) {
      // click delegation
      Y.on(modeBox, 'click', function (e) {
        ...
        if (t.id === 'yro_A_plan_v21') return planFromMode('balance');
        if (t.id === 'yro_B_plan_v21') return planFromMode('push');
        if (t.id === 'yro_C_plan_v21') return planFromMode('funnel');

        if (t.id === 'yro_A_exec_v21' || t.id === 'yro_B_exec_v21' || t.id === 'yro_C_exec_v21') return executePlan();

        if (String(t.id || '').startsWith('pick_') && String(t.id).endsWith('_v21')) {
          var key = String(t.id).replace('pick_', '').replace('_v21', '');
        ...
        if (row && row.getAttribute) {
          ...
          if (mode) {
            st.orchestrator.mode = mode;
            Y.saveState();
            Y.qsa('#yro_modes_v21 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
      ...
      Y.on(modeBox, 'change', function (e) {
        ...
        if (t.id === 'yro_A_target_v21') st.groups.A_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_A_surplus_v21') st.groups.A_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_sender_v21') st.groups.B_sender = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_target_v21') st.groups.B_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_surplus_v21') st.groups.B_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_target_v21') st.groups.C_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_surplus_v21') st.groups.C_surplus = Y.safeInt(t.value, 0);

        else if (t.id === 'yro_A_cap_v21' || t.id === 'yro_B_cap_v21' || t.id === 'yro_C_cap_v21') st.orchestrator.cap = Y.safeInt(t.value, st.orchestrator.cap);
        else if (t.id === 'yro_A_scap_v21' || t.id === 'yro_B_scap_v21' || t.id === 'yro_C_scap_v21') st.orchestrator.scap = Y.safeInt(t.value, st.orchestrator.scap);
        else if (t.id === 'yro_B_reserve_v21' || t.id === 'yro_C_reserve_v21') st.orchestrator.reserve = Y.safeInt(t.value, st.orchestrator.reserve);
