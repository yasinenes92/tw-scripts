(function () {
  ...
  var VERSION = 'v25';
  var NS = 'YRO_V25';
  var PANEL_ID = 'yro_panel_v25';
  var STYLE_ID = 'yro_css_v25';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v25';
  var LS_KEY = 'yro_v25_state';
  ...
  function cleanupOlderInstances() {
    ...
    try { if (window.YRO_V25) delete window.YRO_V25; } catch (e3) {}
...
function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    // v25: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v25{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  gap:8px;
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v25" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v25)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Full scan backbone preserved</span>
    <button class="yro_btn" id="yro_close_v25">X</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v25">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v25"></div></div>
  <span id="yro_prog_txt_v25" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v25" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v25">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v25" style="color:#0a6;">Ready</div>

<div id="yro_content_v25">
  <div class="yro_sec_head">
    <div>1) Villages - Production (per hour + 24h) (<span id="yro_t1_title_v25">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v25">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v25"></select>
      <button class="yro_mini" id="yro_t1_pick_v25">Pick...</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v25" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v25"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages - Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v25">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v25">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v25"></select>
      <button class="yro_mini" id="yro_t2_pick_v25">Pick...</button>
      <span class="yro_small" id="yro_scan_ts_v25"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v25" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v25"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v25"></div>

  <div class="yro_msg" id="yro_plan_meta_v25" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Cap Ceiling %:</b> - | <b>Surplus Cap %:</b> - | <b>Merch Used:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -
  </div>

  <div class="yro_msg yro_plan_diag" id="yro_plan_diag_v25">Diagnostics: -</div>

  <div class="yro_sec_head">
    <div id="yro_plan_target_title_v25">TARGET GROUP</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_target_v25">Copy TARGET GROUP</button>
    </div>
  </div>
  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v25"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_surplus_title_v25">SURPLUS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_surplus_v25">Copy SURPLUS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v25"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_ship_title_v25">SHIPMENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_ship_v25">Copy SHIPMENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v25"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v25');
    var tx = Y.qs('#yro_prog_txt_v25');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v25', st.groups.sel1);
    fill('#yro_t2_group_sel_v25', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v25">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v25">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v25">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v25">Pick...</button>
    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v25" type="number" min="1" max="100" step="1" value="${st.modeA.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v25" type="number" min="1" max="100" step="1" value="${st.modeA.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Computed Fill %</label><input id="yro_A_computed_fill_v25" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_A_merch_used_v25" type="number" value="" style="width:70px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_A_plan_v25">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v25">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_B_parents_v25">${optionsHTML(st.groups.B_parents)}</select></div>
    <button class="yro_mini" id="pick_B_parents_v25">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_B_children_v25">${optionsHTML(st.groups.B_children)}</select></div>
    <button class="yro_mini" id="pick_B_children_v25">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v25">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v25">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_B_parent_reserve_v25" type="number" min="0" max="100" step="1" value="${st.modeB.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Children Max Fill %</label><input id="yro_B_child_max_v25" type="number" min="0" max="100" step="1" value="${st.modeB.childrenMaxFillPct}" style="width:78px;"></div>
    <div class="yro_kv"><label>Surplus Cap %</label><input id="yro_B_scap_v25" type="number" min="0" max="100" step="1" value="${st.modeB.surplusCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Child Fill %</label><input id="yro_B_computed_fill_v25" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_B_child_merch_v25" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_B_parent_merch_v25" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_B_plan_v25">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v25">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_C_target_v25">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v25">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_C_surplus_v25">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v25">Pick...</button>
    <div class="yro_kv"><label>Reserve%</label><input id="yro_C_reserve_v25" type="number" min="0" max="100" step="1" value="${st.modeC.reservePct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_C_cap_v25" type="number" min="1" max="100" step="1" value="${st.modeC.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_C_scap_v25" type="number" min="1" max="100" step="1" value="${st.modeC.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Tolerance%</label><input id="yro_C_tol_v25" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>
    <div class="yro_kv"><label>Iron Delta%</label><input id="yro_C_iron_v25" type="number" min="-50" max="50" step="1" value="${st.modeC.ironDeltaPct == null ? '' : st.modeC.ironDeltaPct}" style="width:70px;"></div>
    <button class="yro_btn btn-confirm" id="yro_C_plan_v25">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v25">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v25">âœ–</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v25" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v25">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v25">Select none</button>
    <span class="yro_small" id="yro_picker_count_v25"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v25">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v25">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v25">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v25');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v25');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v25').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v25').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v25').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v25').value);
    });

    box.querySelector('#yro_picker_none_v25').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v25').value);
    });

    box.querySelector('#yro_picker_save_v25').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v25').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v25');
    var diagEl = Y.qs('#yro_plan_diag_v25');
    var tTbl = Y.qs('#yro_plan_target_tbl_v25');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v25');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v25');
    var targetTitleEl = Y.qs('#yro_plan_target_title_v25');
    var surplusTitleEl = Y.qs('#yro_plan_surplus_title_v25');
    var shipTitleEl = Y.qs('#yro_plan_ship_title_v25');
    var copyTargetEl = Y.qs('#yro_copy_plan_target_v25');
    var copySurplusEl = Y.qs('#yro_copy_plan_surplus_v25');
    var copyShipEl = Y.qs('#yro_copy_plan_ship_v25');

    function renderEmpty() {
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v25');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v25');
      var modeBFillEl = Y.qs('#yro_B_computed_fill_v25');
      var modeBChildMerchEl = Y.qs('#yro_B_child_merch_v25');
      var modeBParentMerchEl = Y.qs('#yro_B_parent_merch_v25');
    ...
    if (String(plan.mode || '').toLowerCase() === 'balance' && plan.targetSummary) {
      ...
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v25');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v25');
      var computedFillEl0 = Y.qs('#yro_B_computed_fill_v25');
      var childMerchEl0 = Y.qs('#yro_B_child_merch_v25');
      var parentMerchEl0 = Y.qs('#yro_B_parent_merch_v25');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl = Y.qs('#yro_B_computed_fill_v25');
      var childMerchEl = Y.qs('#yro_B_child_merch_v25');
      var parentMerchEl = Y.qs('#yro_B_parent_merch_v25');
      var modeAFillEl2 = Y.qs('#yro_A_computed_fill_v25');
      var modeAMerchEl2 = Y.qs('#yro_A_merch_used_v25');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl2 = Y.qs('#yro_B_computed_fill_v25');
      var childMerchEl2 = Y.qs('#yro_B_child_merch_v25');
      var parentMerchEl2 = Y.qs('#yro_B_parent_merch_v25');
      var modeAFillEl3 = Y.qs('#yro_A_computed_fill_v25');
      var modeAMerchEl3 = Y.qs('#yro_A_merch_used_v25');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v25');
    ...
    var t2title = Y.qs('#yro_t2_title_v25');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v25', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v25');
      surplusGid = getSelValue('#yro_A_surplus_v25');
      var capA = getNumValue('#yro_A_cap_v25', st.modeA.capPct || 80);
      var scapA = getNumValue('#yro_A_scap_v25', st.modeA.surplusCapPct || 95);
      ...
      var parentGid = getSelValue('#yro_B_parents_v25');
      var childGid = getSelValue('#yro_B_children_v25');
      surplusGid = getSelValue('#yro_B_surplus_v25');
      var parentReservePct = getNumValue('#yro_B_parent_reserve_v25', st.modeB.parentReservePct || 1);
      var childrenMaxFillPct = getNumValue('#yro_B_child_max_v25', st.modeB.childrenMaxFillPct || 80);
      var surplusCapPct = getNumValue('#yro_B_scap_v25', st.modeB.surplusCapPct || 95);
      ...
      targetGid = getSelValue('#yro_C_target_v25');
      surplusGid = getSelValue('#yro_C_surplus_v25');
      var reserveC = getNumValue('#yro_C_reserve_v25', st.modeC.reservePct || 1);
      var capC = getNumValue('#yro_C_cap_v25', st.modeC.capPct || 80);
      var scapC = getNumValue('#yro_C_scap_v25', st.modeC.surplusCapPct || 95);
      var ironDeltaC = getNumValue('#yro_C_iron_v25', st.modeC.ironDeltaPct || 0);
  ...
  function copyPlanSurplusBBCode() {
    ...
    if (mode === 'balance') {
      ...
      flashButton('#yro_copy_plan_surplus_v25', 'Copy SURPLUS');
  ...
  function copyPlanShipmentsBBCode() {
    ...
    flashButton('#yro_copy_plan_ship_v25', 'Copy SHIPMENTS');
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v25'), 'click', function () { Y.destroy(); });
    Y.on(Y.qs('#yro_load_v25'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v25');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v25'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v25');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v25');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v25'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v25');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v25');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v25'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v25'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v25'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v25'), 'click', function () { openPicker('t2'); });
    Y.on(Y.qs('#yro_copy_bb_v25'), 'click', function () { copyBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_target_v25'), 'click', function () { copyPlanTargetBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_surplus_v25'), 'click', function () { copyPlanSurplusBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_ship_v25'), 'click', function () { copyPlanShipmentsBBCode(); });

    var modeBox = Y.qs('#yro_modes_v25');
    if (!modeBox) return;
    ...
    Y.on(modeBox, 'click', function (e) {
      ...
      if (t.id === 'yro_A_plan_v25') return planFromMode('balance');
      if (t.id === 'yro_B_plan_v25') return planFromMode('push');
      if (t.id === 'yro_C_plan_v25') return planFromMode('funnel');

      if (t.id === 'yro_A_exec_v25' || t.id === 'yro_B_exec_v25' || t.id === 'yro_C_exec_v25') return executePlan();

      if (String(t.id || '').startsWith('pick_') && String(t.id || '').endsWith('_v25')) {
        var key = String(t.id).replace('pick_', '').replace('_v25', '');
      ...
      if (row && row.getAttribute) {
        ...
        if (mode) {
          st.orchestrator.mode = mode;
          Y.saveState();
          Y.qsa('#yro_modes_v25 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
    ...
    Y.on(modeBox, 'change', function (e) {
      ...
      if (t.id === 'yro_A_target_v25') st.groups.A_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_surplus_v25') st.groups.A_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parents_v25') st.groups.B_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_children_v25') st.groups.B_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_surplus_v25') st.groups.B_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_target_v25') st.groups.C_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_surplus_v25') st.groups.C_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_cap_v25') st.modeA.capPct = Y.safeInt(t.value, st.modeA.capPct);
      else if (t.id === 'yro_A_scap_v25') st.modeA.surplusCapPct = Y.safeInt(t.value, st.modeA.surplusCapPct);
      else if (t.id === 'yro_B_parent_reserve_v25') st.modeB.parentReservePct = Y.safeInt(t.value, st.modeB.parentReservePct);
      else if (t.id === 'yro_B_child_max_v25') st.modeB.childrenMaxFillPct = Y.safeInt(t.value, st.modeB.childrenMaxFillPct);
      else if (t.id === 'yro_B_scap_v25') st.modeB.surplusCapPct = Y.safeInt(t.value, st.modeB.surplusCapPct);
      else if (t.id === 'yro_C_reserve_v25') st.modeC.reservePct = Y.safeInt(t.value, st.modeC.reservePct);
      else if (t.id === 'yro_C_cap_v25') st.modeC.capPct = Y.safeInt(t.value, st.modeC.capPct);
      else if (t.id === 'yro_C_scap_v25') st.modeC.surplusCapPct = Y.safeInt(t.value, st.modeC.surplusCapPct);
      else if (t.id === 'yro_C_iron_v25') st.modeC.ironDeltaPct = String(t.value).trim() === '' ? null : Y.safeInt(t.value, 0);
