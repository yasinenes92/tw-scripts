(function () {
  ...
  var VERSION = 'v32';
  var NS = 'YRO_V32';
  var PANEL_ID = 'yro_panel_v32';
  var STYLE_ID = 'yro_css_v32';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v32';
  var LS_KEY = 'YRO_V32_state';
  ...
  function cleanupOlderInstances() {
    ...
    try { if (window.YRO_V32) delete window.YRO_V32; } catch (e3) {}
  ...
  function sanitizeState(st) {
    ...
    st.custom.D_parents = uniqIds(st.custom.D_parents);
    st.custom.D_children = uniqIds(st.custom.D_children);
    st.custom.D_surplus = uniqIds(st.custom.D_surplus);
    ...
    st.groups.D_parents = safeInt(st.groups.D_parents, 0);
    st.groups.D_children = safeInt(st.groups.D_children, 0);
    st.groups.D_surplus = safeInt(st.groups.D_surplus, 0);
    ...
    st.modeA.balanceBasis = String(st.modeA.balanceBasis || '').toLowerCase() === 'flat' ? 'flat' : 'fill';
    st.modeA.capPct = safeInt(st.modeA.capPct, 80);
    st.modeA.depotCapPct = safeInt(st.modeA.depotCapPct, 80);
    ...
    st.modeD.parentReservePct = safeInt(st.modeD.parentReservePct, 1);
    st.modeD.depotCapPct = safeInt(st.modeD.depotCapPct, 80);
...
(function () {
  ...
  Y.log('data module loaded Ã¢Å“â€¦');
})();
...
(function () {
  ...
  function normalizeModeASets(targetIds, surplusIds) {
    ...
    surplusIds = selectedSurplusIds.filter(function (id) { return !targetSet[id]; });
  ...
  function buildModeADiagnostics(finalRun, targetIds, normalized, snapshotsById, incomingMap, acceptedValue, ceilingValue, options, initialSurplusRoom, remainingSurplusRoom) {
    ...
    var overlapCount = (normalized.excludedOverlapIds || []).length;

    if (!selectedSurplusCount) {
      lines.push('No Surplus group selected for excess parking.');
    } else {
      if (overlapCount) {
        lines.push('Surplus/Target overlap excluded ' + Y.formatTwNumber(overlapCount) + ' village(s): ' + normalized.excludedOverlapIds.join(', ') + '.');
      }
      if (!normalized.surplusIds.length) {
        lines.push('Overlap removed all usable Surplus villages.');
      } else if (!initialSurplusRoom || initialSurplusRoom.total.total <= 0) {
    ...
    if (excess.totals.sinkBlocked.total > 0) {
      ...
      else if (!normalized.surplusIds.length) sinkLabel = 'Surplus/Target overlap removed all usable Surplus villages.';
  ...
  function planBalance(targetIds, surplusIds, snapshotsById, incomingMap, modeAOptions, surplusCapPct) {
    ...
    } else if (normalized.excludedOverlapIds.length && !surplusIds.length) {
      warnings.push('Surplus/Target overlap removed all usable Surplus villages.');
    } else if (normalized.excludedOverlapIds.length) {
      warnings.push('Surplus/Target overlap removed ' + normalized.excludedOverlapIds.length + ' village(s) from the sink set.');
    }
  ...
  function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    // v32: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v32{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  gap:8px;
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v32" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v32)</span></div>
  <div style="display:flex; align-items:center;">
    ...
    <button class="yro_btn" id="yro_close_v32">X</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v32">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v32"></div></div>
  <span id="yro_prog_txt_v32" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v32" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v32">Copy Test Results</button>
</div>

<div class="yro_msg" id="yro_msg_v32" style="color:#0a6;">Ready</div>

<div id="yro_content_v32">
  <div class="yro_sec_head">
    <div>1) Villages - Production (per hour + 24h) (<span id="yro_t1_title_v32">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v32">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v32"></select>
      <button class="yro_mini" id="yro_t1_pick_v32">Pick...</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v32" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v32"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages - Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v32">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v32">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v32"></select>
      <button class="yro_mini" id="yro_t2_pick_v32">Pick...</button>
      <span class="yro_small" id="yro_scan_ts_v32"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v32" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v32"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v32"></div>

  <div class="yro_msg" id="yro_plan_meta_v32" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Cap Ceiling %:</b> - | <b>Surplus Cap %:</b> - | <b>Merch Used:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -
  </div>

  <div class="yro_msg yro_plan_diag" id="yro_plan_diag_v32">Diagnostics: -</div>

  <div class="yro_sec_head">
    <div id="yro_plan_target_title_v32">TARGET GROUP</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_target_v32">Copy TARGET GROUP</button>
    </div>
  </div>
  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v32"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_surplus_title_v32">SURPLUS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_surplus_v32">Copy SURPLUS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v32"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_ship_title_v32">SHIPMENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_ship_v32">Copy SHIPMENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v32"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v32');
    var tx = Y.qs('#yro_prog_txt_v32');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v32', st.groups.sel1);
    fill('#yro_t2_group_sel_v32', st.groups.sel2);
  }

  function buildModesBox(groups) {
    groups = Array.isArray(groups) ? groups : [];
    var st = Y.state;
    var modeABasis = String(st.modeA.balanceBasis || '').toLowerCase() === 'flat' ? 'flat' : 'fill';
    var modeADesc = modeABasis === 'flat'
      ? 'Target group rebalances internally to the highest feasible common flat amount. Surplus last.'
      : 'Target group rebalances internally to the highest feasible common fill. Surplus last.';
    var modeAComputedLabel = modeABasis === 'flat' ? 'Computed Flat Target' : 'Computed Fill %';
    var modeACapDisabled = modeABasis === 'flat' ? ' disabled' : '';
    var modeADepotDisabled = modeABasis === 'fill' ? ' disabled' : '';
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  <div class="yro_mode_left">
    ...
    <div class="yro_mode_desc">${modeADesc}</div>
  </div>
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v32">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v32">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v32">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v32">Pick...</button>
    <div class="yro_kv"><label>Balance Basis</label><select id="yro_A_basis_v32" style="width:180px;"><option value="fill"${modeABasis === 'fill' ? ' selected' : ''}>Equal Fill % by Storage</option><option value="flat"${modeABasis === 'flat' ? ' selected' : ''}>Equal Flat Amount per Village</option></select></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v32" type="number" min="0" max="100" step="1" value="${st.modeA.capPct}" style="width:60px;"${modeACapDisabled}></div>
    <div class="yro_kv"><label>Depot Cap %</label><input id="yro_A_depot_cap_v32" type="number" min="0" max="100" step="1" value="${st.modeA.depotCapPct}" style="width:70px;"${modeADepotDisabled}></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v32" type="number" min="1" max="100" step="1" value="${st.modeA.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label id="yro_A_computed_label_v32">${modeAComputedLabel}</label><input id="yro_A_computed_fill_v32" type="number" value="" style="width:92px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_A_merch_used_v32" type="number" value="" style="width:70px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_A_plan_v32">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v32">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_B_parents_v32">${optionsHTML(st.groups.B_parents)}</select></div>
    <button class="yro_mini" id="pick_B_parents_v32">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_B_children_v32">${optionsHTML(st.groups.B_children)}</select></div>
    <button class="yro_mini" id="pick_B_children_v32">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v32">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v32">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_B_parent_reserve_v32" type="number" min="0" max="100" step="1" value="${st.modeB.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Children Max Fill %</label><input id="yro_B_child_max_v32" type="number" min="0" max="100" step="1" value="${st.modeB.childrenMaxFillPct}" style="width:78px;"></div>
    <div class="yro_kv"><label>Surplus Cap %</label><input id="yro_B_scap_v32" type="number" min="0" max="100" step="1" value="${st.modeB.surplusCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Child Fill %</label><input id="yro_B_computed_fill_v32" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_B_child_merch_v32" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_B_parent_merch_v32" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_B_plan_v32">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v32">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${(st.orchestrator.mode === 'coin' || st.orchestrator.mode === 'funnel') ? 'active' : ''}" data-mode="coin">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Sources</label><select id="yro_C_sources_v32">${optionsHTML(st.groups.C_sources)}</select></div>
    <button class="yro_mini" id="pick_C_sources_v32">Pick...</button>
    <div class="yro_kv"><label>Coin</label><select id="yro_C_coin_v32">${optionsHTML(st.groups.C_coin)}</select></div>
    <button class="yro_mini" id="pick_C_coin_v32">Pick...</button>
    <div class="yro_kv"><label>Buffer / Surplus</label><select id="yro_C_buffer_v32">${optionsHTML(st.groups.C_buffer)}</select></div>
    <button class="yro_mini" id="pick_C_buffer_v32">Pick...</button>
    <div class="yro_kv"><label>Source Reserve %</label><input id="yro_C_reserve_v32" type="number" min="0" max="100" step="1" value="${st.modeC.sourceReservePct}" style="width:72px;"></div>
    <div class="yro_kv"><label>Buffer Cap %</label><input id="yro_C_bcap_v32" type="number" min="0" max="100" step="1" value="${st.modeC.bufferCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Additional Coins</label><input id="yro_C_additional_coins_v32" type="number" value="" style="width:84px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_C_merch_used_v32" type="number" value="" style="width:72px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_C_plan_v32">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v32">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'flat' ? 'active' : ''}" data-mode="flat">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_D_parents_v32">${optionsHTML(st.groups.D_parents)}</select></div>
    <button class="yro_mini" id="pick_D_parents_v32">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_D_children_v32">${optionsHTML(st.groups.D_children)}</select></div>
    <button class="yro_mini" id="pick_D_children_v32">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_D_surplus_v32">${optionsHTML(st.groups.D_surplus)}</select></div>
    <button class="yro_mini" id="pick_D_surplus_v32">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_D_parent_reserve_v32" type="number" min="0" max="100" step="1" value="${st.modeD.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Depot Cap %</label><input id="yro_D_depot_cap_v32" type="number" min="0" max="100" step="1" value="${st.modeD.depotCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Flat Target</label><input id="yro_D_computed_flat_v32" type="number" value="" style="width:98px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_D_child_merch_v32" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_D_parent_merch_v32" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_D_plan_v32">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_D_exec_v32">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v32">Ã¢Å“â€“</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v32" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v32">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v32">Select none</button>
    <span class="yro_small" id="yro_picker_count_v32"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v32">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v32">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v32">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v32');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v32');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v32').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v32').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v32').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v32').value);
    });

    box.querySelector('#yro_picker_none_v32').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v32').value);
    });

    box.querySelector('#yro_picker_save_v32').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v32').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v32');
    var diagEl = Y.qs('#yro_plan_diag_v32');
    var tTbl = Y.qs('#yro_plan_target_tbl_v32');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v32');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v32');
    var targetTitleEl = Y.qs('#yro_plan_target_title_v32');
    var surplusTitleEl = Y.qs('#yro_plan_surplus_title_v32');
    var shipTitleEl = Y.qs('#yro_plan_ship_title_v32');
    var copyTargetEl = Y.qs('#yro_copy_plan_target_v32');
    var copySurplusEl = Y.qs('#yro_copy_plan_surplus_v32');
    var copyShipEl = Y.qs('#yro_copy_plan_ship_v32');

    function renderEmpty() {
      var modeALabelEl = Y.qs('#yro_A_computed_label_v32');
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v32');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v32');
      var modeBFillEl = Y.qs('#yro_B_computed_fill_v32');
      var modeBChildMerchEl = Y.qs('#yro_B_child_merch_v32');
      var modeBParentMerchEl = Y.qs('#yro_B_parent_merch_v32');
      var modeDFlatEl = Y.qs('#yro_D_computed_flat_v32');
      var modeDChildMerchEl = Y.qs('#yro_D_child_merch_v32');
      var modeDParentMerchEl = Y.qs('#yro_D_parent_merch_v32');
      var modeCCoinsEl = Y.qs('#yro_C_additional_coins_v32');
      var modeCMerchEl = Y.qs('#yro_C_merch_used_v32');
      ...
      if (modeALabelEl) modeALabelEl.textContent = String(Y.state.modeA && Y.state.modeA.balanceBasis || '').toLowerCase() === 'flat' ? 'Computed Flat Target' : 'Computed Fill %';
    ...
    if (String(plan.mode || '').toLowerCase() === 'balance' && plan.targetSummary) {
      ...
      var modeABasis = String(plan.meta.balanceBasis || '').toLowerCase() === 'flat' ? 'flat' : 'fill';
      var modeALabel = modeABasis === 'flat' ? 'Computed Flat Target' : 'Computed Fill %';
      if (metaEl) {
        if (modeABasis === 'flat') {
          metaEl.innerHTML =
            '<b>Mode:</b> INTERNAL EQUALIZER | ' +
            '<b>Balance Basis:</b> ' + escHtml(plan.meta.balanceBasisLabel || 'Equal Flat Amount per Village') + ' | ' +
          ...
          metaEl.innerHTML =
            '<b>Mode:</b> INTERNAL EQUALIZER | ' +
            '<b>Balance Basis:</b> ' + escHtml(plan.meta.balanceBasisLabel || 'Equal Fill % by Storage') + ' | ' +
      ...
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v32');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v32');
      var computedFillEl0 = Y.qs('#yro_B_computed_fill_v32');
      var childMerchEl0 = Y.qs('#yro_B_child_merch_v32');
      var parentMerchEl0 = Y.qs('#yro_B_parent_merch_v32');
      var modeDFlatEl0 = Y.qs('#yro_D_computed_flat_v32');
      var modeDChildMerchEl0 = Y.qs('#yro_D_child_merch_v32');
      var modeDParentMerchEl0 = Y.qs('#yro_D_parent_merch_v32');
      var coinAddEl0 = Y.qs('#yro_C_additional_coins_v32');
      var coinMerchEl0 = Y.qs('#yro_C_merch_used_v32');
      if (modeALabelEl) modeALabelEl.textContent = modeALabel;
      if (modeAFillEl) modeAFillEl.value = modeABasis === 'flat'
        ? (plan.meta.computedFlatTarget != null ? plan.meta.computedFlatTarget : '')
        : (plan.meta.computedFillPct != null ? plan.meta.computedFillPct : '');
      ...
      function buildModeATargetTable(rows) {
        var isFlat = modeABasis === 'flat';
        var colSpan = isFlat ? 19 : 18;
        var html = isFlat
          ? `
<tr><th colspan="19">TARGET GROUP</th></tr>
<tr>
  <th>Village</th>
  <th class="yro_right">Storage</th>
  ...
  <th class="yro_right">Merch Used</th>
          ...
          : `
<tr><th colspan="18">TARGET GROUP</th></tr>
<tr>
  <th>Village</th>
  <th class="yro_right">Storage</th>
  <th class="yro_right">Target Each</th>
  ...
  <th class="yro_right">Merch Used</th>
        ...
        if (!rows.length) return html + '<tr><td colspan="' + colSpan + '">-</td></tr>';
        rows.forEach(function (r) {
          html += isFlat ? `
<tr>
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.merchUsed || 0)}</td>
      ...
      function buildModeASurplusTable(rows) {
        var html = `
<tr><th colspan="18">SURPLUS</th></tr>
<tr>
  <th>Village</th>
  <th class="yro_right">Storage</th>
  <th class="yro_right">Cap Each</th>
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.merchUsed || 0)}</td>
      ...
      var computedFillEl = Y.qs('#yro_B_computed_fill_v32');
      var childMerchEl = Y.qs('#yro_B_child_merch_v32');
      var parentMerchEl = Y.qs('#yro_B_parent_merch_v32');
      var modeAFillEl2 = Y.qs('#yro_A_computed_fill_v32');
      var modeAMerchEl2 = Y.qs('#yro_A_merch_used_v32');
      var modeDFlatEl1 = Y.qs('#yro_D_computed_flat_v32');
      var modeDChildMerchEl1 = Y.qs('#yro_D_child_merch_v32');
      var modeDParentMerchEl1 = Y.qs('#yro_D_parent_merch_v32');
      var coinAddEl1 = Y.qs('#yro_C_additional_coins_v32');
      var coinMerchEl1 = Y.qs('#yro_C_merch_used_v32');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.merchUsed || 0)}</td>
      ...
      var computedFillElD0 = Y.qs('#yro_B_computed_fill_v32');
      var childMerchElD0 = Y.qs('#yro_B_child_merch_v32');
      var parentMerchElD0 = Y.qs('#yro_B_parent_merch_v32');
      var modeAFillElD0 = Y.qs('#yro_A_computed_fill_v32');
      var modeAMerchElD0 = Y.qs('#yro_A_merch_used_v32');
      var modeDFlatEl = Y.qs('#yro_D_computed_flat_v32');
      var modeDChildMerchEl = Y.qs('#yro_D_child_merch_v32');
      var modeDParentMerchEl = Y.qs('#yro_D_parent_merch_v32');
      var modeCCoinsElD0 = Y.qs('#yro_C_additional_coins_v32');
      var modeCMerchElD0 = Y.qs('#yro_C_merch_used_v32');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.merchUsed || 0)}</td>
      ...
      var computedFillElC0 = Y.qs('#yro_B_computed_fill_v32');
      var childMerchElC0 = Y.qs('#yro_B_child_merch_v32');
      var parentMerchElC0 = Y.qs('#yro_B_parent_merch_v32');
      var modeAFillElC0 = Y.qs('#yro_A_computed_fill_v32');
      var modeAMerchElC0 = Y.qs('#yro_A_merch_used_v32');
      var modeDFlatElC0 = Y.qs('#yro_D_computed_flat_v32');
      var modeDChildMerchElC0 = Y.qs('#yro_D_child_merch_v32');
      var modeDParentMerchElC0 = Y.qs('#yro_D_parent_merch_v32');
      var modeCCoinsEl = Y.qs('#yro_C_additional_coins_v32');
      var modeCMerchEl = Y.qs('#yro_C_merch_used_v32');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.merchUsed || 0)}</td>
      ...
      var computedFillEl2 = Y.qs('#yro_B_computed_fill_v32');
      var childMerchEl2 = Y.qs('#yro_B_child_merch_v32');
      var parentMerchEl2 = Y.qs('#yro_B_parent_merch_v32');
      var modeAFillEl3 = Y.qs('#yro_A_computed_fill_v32');
      var modeAMerchEl3 = Y.qs('#yro_A_merch_used_v32');
      var modeDFlatEl2 = Y.qs('#yro_D_computed_flat_v32');
      var modeDChildMerchEl2 = Y.qs('#yro_D_child_merch_v32');
      var modeDParentMerchEl2 = Y.qs('#yro_D_parent_merch_v32');
      var modeCCoinsEl2 = Y.qs('#yro_C_additional_coins_v32');
      var modeCMerchEl2 = Y.qs('#yro_C_merch_used_v32');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v32');
    ...
    var t2title = Y.qs('#yro_t2_title_v32');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v32', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v32');
      surplusGid = getSelValue('#yro_A_surplus_v32');
      var basisA = String((Y.qs('#yro_A_basis_v32') && Y.qs('#yro_A_basis_v32').value) || st.modeA.balanceBasis || 'fill').toLowerCase() === 'flat' ? 'flat' : 'fill';
      var capA = getNumValue('#yro_A_cap_v32', st.modeA.capPct || 80);
      var depotCapA = getNumValue('#yro_A_depot_cap_v32', st.modeA.depotCapPct || 80);
      var scapA = getNumValue('#yro_A_scap_v32', st.modeA.surplusCapPct || 95);

      targetIds = await resolveSelection(targetGid, 'A_target', true);
      surplusIds = await resolveSelection(surplusGid, 'A_surplus', true);

      Y.runtime.plan = Y.compute.planBalance(targetIds, surplusIds, Y.runtime.snapshotsById, st.cache.incomingMap, {
        balanceBasis: basisA,
        capPct: capA,
        depotCapPct: depotCapA,
        surplusCapPct: scapA
      });
      ...
      st.modeA.balanceBasis = basisA;
      st.modeA.capPct = capA;
      st.modeA.depotCapPct = depotCapA;
      st.modeA.surplusCapPct = scapA;
      delete st.modeA.ironDeltaPct;
    } else if (mode === 'push') {
      var parentGid = getSelValue('#yro_B_parents_v32');
      var childGid = getSelValue('#yro_B_children_v32');
      surplusGid = getSelValue('#yro_B_surplus_v32');
      var parentReservePct = getNumValue('#yro_B_parent_reserve_v32', st.modeB.parentReservePct || 1);
      var childrenMaxFillPct = getNumValue('#yro_B_child_max_v32', st.modeB.childrenMaxFillPct || 80);
      var surplusCapPct = getNumValue('#yro_B_scap_v32', st.modeB.surplusCapPct || 95);
      ...
      var flatParentGid = getSelValue('#yro_D_parents_v32');
      var flatChildGid = getSelValue('#yro_D_children_v32');
      var flatSurplusGid = getSelValue('#yro_D_surplus_v32');
      var flatParentReservePct = getNumValue('#yro_D_parent_reserve_v32', st.modeD.parentReservePct || 1);
      var depotCapPct = getNumValue('#yro_D_depot_cap_v32', st.modeD.depotCapPct || 80);

      var flatParentIds = await resolveSelection(flatParentGid, 'D_parents', true);
      var flatChildIds = await resolveSelection(flatChildGid, 'D_children', true);
      var flatSurplusIds = await resolveSelection(flatSurplusGid, 'D_surplus', true);

      Y.runtime.plan = Y.compute.planFlat(
        flatParentIds,
        flatChildIds,
        flatSurplusIds,
        Y.runtime.snapshotsById,
        st.cache.incomingMap,
        depotCapPct,
        flatParentReservePct
      );

      if (Y.runtime.plan && Y.runtime.plan.meta) {
        Y.runtime.plan.meta.parentGid = flatParentGid;
        Y.runtime.plan.meta.childGid = flatChildGid;
        Y.runtime.plan.meta.surplusGid = flatSurplusGid;
      }

      st.groups.D_parents = flatParentGid;
      st.groups.D_children = flatChildGid;
      st.groups.D_surplus = flatSurplusGid;
      st.modeD.parentReservePct = flatParentReservePct;
      st.modeD.depotCapPct = depotCapPct;
    } else if (mode === 'coin') {
      var sourceGid = getSelValue('#yro_C_sources_v32');
      var coinGid = getSelValue('#yro_C_coin_v32');
      var bufferGid = getSelValue('#yro_C_buffer_v32');
      var sourceReservePct = getNumValue('#yro_C_reserve_v32', st.modeC.sourceReservePct || 1);
      var bufferCapPct = getNumValue('#yro_C_bcap_v32', st.modeC.bufferCapPct || 95);
  ...
  function tripletCols(res) {
    res = res || {};
    return [
      formatRes(res.wood || 0),
      formatRes(res.clay || 0),
      formatRes(res.iron || 0)
    ];
  }
  function merchPair(row) {
    return formatRes((row && row.merchFree) || 0) + '/' + formatRes((row && row.merchTotal) || 0);
  }
  function coinCostText(cost) {
    return cost ? (formatRes(cost.wood || 0) + '/' + formatRes(cost.clay || 0) + '/' + formatRes(cost.iron || 0)) : '-';
  }
  ...
  function buildFlatMetaLines(plan, title) {
    var lines = ['[quote][b]Yaver Resource Orchestrator[/b] (' + Y.ts() + ')'];
    lines.push('[b]' + bbEscape(title) + '[/b]');
    lines.push('[list]');
    lines.push('[*][b]Mode[/b]: FLAT TARGET EQUALIZER');
    lines.push('[*][b]Computed Flat Target[/b]: ' + (plan.meta.computedFlatTarget != null ? plan.meta.computedFlatTarget : '-'));
    lines.push('[*][b]Depot Cap %[/b]: ' + (plan.meta.depotCapPct != null ? plan.meta.depotCapPct : '-') + '%');
    lines.push('[*][b]Parent Reserve %[/b]: ' + (plan.meta.parentReservePct != null ? plan.meta.parentReservePct : '-') + '%');
    lines.push('[*][b]Child Merch Used[/b]: ' + (plan.meta.childMerchantsUsed != null ? plan.meta.childMerchantsUsed : '-'));
    lines.push('[*][b]Parent Merch Used[/b]: ' + (plan.meta.parentMerchantsUsed != null ? plan.meta.parentMerchantsUsed : '-'));
    if (plan.meta.stopReason) lines.push('[*][b]Stop Reason[/b]: ' + bbEscape(plan.meta.stopReason));
  ...
  function buildChildrenTableBBCode(plan) {
    ...
    var rows = (plan.childSummary || []).map(function (r) {
      return [villageLabel(r),
        formatRes(r.storage),
        merchPair(r),
        formatRes(r.targetEach || 0)
      ].concat(
        tripletCols(r.current),
        [
    ...
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Merch (free/total)', 'Target Each', 'Cur W', 'Cur C', 'Cur I', 'Before', 'Sent', 'Recv', 'After', 'Final W', 'Final C', 'Final I', 'Merch Used'],
      rows
    ));
    lines.push('[/quote]');
    return lines.join('\n');
  ...
  function buildCoinTableBBCode(plan) {
    ...
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Merch (free/total)', 'Coin Cost', 'Base Coins', 'Projected Coins', '+Coins', 'Next Deficit', 'Cur W', 'Cur C', 'Cur I', 'Before', 'Sent', 'Recv', 'After', 'Final W', 'Final C', 'Final I', 'Merch Used'],
      rows
    ));
    lines.push('[/quote]');
    return lines.join('\n');
  ...
  function buildFlatChildrenTableBBCode(plan) {
    var lines = buildFlatMetaLines(plan, 'CHILDREN');
    var rows = (plan.childSummary || []).map(function (r) {
      return [villageLabel(r),
        formatRes(r.storage),
        merchPair(r),
    ...
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Merch (free/total)', 'Cap Each', 'Target Each', 'Capped?', 'Cur W', 'Cur C', 'Cur I', 'Before', 'Sent', 'Recv', 'After', 'Final W', 'Final C', 'Final I', 'Merch Used'],
      rows
    ));
    lines.push('[/quote]');
    return lines.join('\n');
  ...
  function buildParentsTableBBCode(plan) {
    ...
    var parentRows = (plan.parentSummary || []).map(function (r) {
      return [villageLabel(r),
        ...
        [
          ...
          formatRes(r.sent.total),
          formatRes(r.after.total),
          formatRes((r.aboveReserveAfter && r.aboveReserveAfter.total) || 0)
        ],
        tripletCols(r.after),
        villageLabel(r),
        [formatRes(r.spreadAfter || 0), formatRes(r.merchUsed || 0)]
      );
    });
    lines.push('[b]PARENTS[/b]');
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Merch (free/total)', 'Reserve Each', 'Cur W', 'Cur C', 'Cur I', 'Before', 'Sent', 'After', 'Above Reserve After', 'Final W', 'Final C', 'Final I', 'Spread (Above Reserve)', 'Merch Used'],
      parentRows
    ));
    ...
    if (plan.surplusSummary && plan.surplusSummary.length) {
      ...
      lines = lines.concat(bbTable(
        ['Village', 'Storage', 'Merch (free/total)', 'Cap Each', 'Cur W', 'Cur C', 'Cur I', 'Before', 'Sent', 'Recv', 'After', 'Final W', 'Final C', 'Final I', 'Merch Used'],
        surplusRows
      ));
    }
  ...
  function buildFlatParentsTableBBCode(plan) {
    var lines = buildFlatMetaLines(plan, (plan.surplusSummary && plan.surplusSummary.length) ? 'PARENTS / SURPLUS' : 'PARENTS');
    var rows = (plan.parentSummary || []).map(function (r) {
      return [villageLabel(r),
        formatRes(r.storage),
        merchPair(r),
        formatRes(r.reserveEach || 0)
  ...
  function buildCoinSourcesBufferBBCode(plan) {
    ...
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Merch (free/total)', 'Reserve Each', 'Cur W', 'Cur C', 'Cur I', 'Before', 'Sent', 'Recv', 'After', 'Available Left', 'Final W', 'Final C', 'Final I', 'Merch Used'],
      sourceRows
    ));
  ...
  function buildShipmentsTableBBCode(plan) {
    ...
    var isFlat = String(plan.mode || '').toLowerCase() === 'flat';
    var isCoin = String(plan.mode || '').toLowerCase() === 'coin';
    var lines = isPush
      ? buildPushMetaLines(plan, 'SHIPMENTS')
      : (isBalance
        ? buildBalanceMetaLines(plan, 'SHIPMENTS')
        : (isFlat
          ? buildFlatMetaLines(plan, 'SHIPMENTS')
          : (isCoin ? buildCoinMetaLines(plan, 'SHIPMENTS') : buildGenericMetaLines(plan, 'SHIPMENTS'))));

    if ((isPush || isBalance || isFlat || isCoin) && plan.meta && plan.meta.diagnostics && Array.isArray(plan.meta.diagnostics.lines) && plan.meta.diagnostics.lines.length) {
  ...
  function copyPlanTargetBBCode() {
    ...
    if (mode === 'push') {
      ...
      flashButton('#yro_copy_plan_target_v32', 'Copy CHILDREN');
      copyText(text, 'CHILDREN BBCode copied');
      return;
    }
    if (mode === 'balance') {
      text = buildBalanceTargetTableBBCode(plan);
      flashButton('#yro_copy_plan_target_v32', 'Copy TARGET GROUP');
      copyText(text, 'TARGET GROUP BBCode copied');
      return;
    }
    if (mode === 'coin') {
      text = buildCoinTableBBCode(plan);
      flashButton('#yro_copy_plan_target_v32', 'Copy COIN VILLAGES');
      copyText(text, 'COIN VILLAGES BBCode copied');
      return;
    }
    if (mode === 'flat') {
      text = buildFlatChildrenTableBBCode(plan);
      flashButton('#yro_copy_plan_target_v32', 'Copy CHILDREN');
      copyText(text, 'CHILDREN BBCode copied');
      return;
    }
    ...
    flashButton('#yro_copy_plan_target_v32', 'Copy TARGET');
  ...
  function copyPlanSurplusBBCode() {
    ...
    if (mode === 'balance') {
      ...
      flashButton('#yro_copy_plan_surplus_v32', 'Copy SURPLUS');
      copyText(text, 'SURPLUS BBCode copied');
      return;
    }
    if (mode === 'coin') {
      text = buildCoinSourcesBufferBBCode(plan);
      flashButton('#yro_copy_plan_surplus_v32', (plan.bufferSummary && plan.bufferSummary.length) ? 'Copy SOURCES / BUFFER' : 'Copy SOURCES');
      copyText(text, (plan.bufferSummary && plan.bufferSummary.length) ? 'SOURCES / BUFFER BBCode copied' : 'SOURCES BBCode copied');
      return;
    }
    if (mode === 'flat') {
      text = buildFlatParentsTableBBCode(plan);
      flashButton(
        '#yro_copy_plan_surplus_v32',
        (plan.surplusSummary && plan.surplusSummary.length) ? 'Copy PARENTS / SURPLUS' : 'Copy PARENTS'
      );
  ...
  function copyPlanShipmentsBBCode() {
    ...
    flashButton('#yro_copy_plan_ship_v32', 'Copy SHIPMENTS');
  ...
  function formatResTripletShort(res) {
    res = res || {};
    return 'W ' + formatRes(res.wood || 0) + ' / C ' + formatRes(res.clay || 0) + ' / I ' + formatRes(res.iron || 0) + ' / T ' + formatRes(res.total || 0);
  }
  ...
  function getActiveModeKey() {
    var active = Y.qs('#yro_modes_v32 .yro_mode_row.active');
  ...
  function activeModeLabel(mode) {
    ...
    if (mode === 'flat') return 'Mode D: Flat Target Equalizer';
  ...
  function buildCurrentModeInputRows() {
    ...
    if (mode === 'balance') {
      var basisA = String(st.modeA.balanceBasis || '').toLowerCase() === 'flat' ? 'flat' : 'fill';
      return [
        ['Mode', activeModeLabel(mode)],
        ['Target', getSelectText('#yro_A_target_v32', st.groups.A_target)],
        ['Surplus', getSelectText('#yro_A_surplus_v32', st.groups.A_surplus)],
        ['Balance Basis', basisA === 'flat' ? 'Equal Flat Amount per Village' : 'Equal Fill % by Storage'],
        ['Cap %', basisA === 'fill' ? getInputText('#yro_A_cap_v32', st.modeA.capPct) : '-'],
        ['Depot Cap %', basisA === 'flat' ? getInputText('#yro_A_depot_cap_v32', st.modeA.depotCapPct) : '-'],
        ['Surplus Cap %', getInputText('#yro_A_scap_v32', st.modeA.surplusCapPct)]
      ];
    }
    if (mode === 'push') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Parents', getSelectText('#yro_B_parents_v32', st.groups.B_parents)],
        ['Children', getSelectText('#yro_B_children_v32', st.groups.B_children)],
        ['Surplus', getSelectText('#yro_B_surplus_v32', st.groups.B_surplus)],
        ['Parent Reserve %', getInputText('#yro_B_parent_reserve_v32', st.modeB.parentReservePct)],
        ['Children Max Fill %', getInputText('#yro_B_child_max_v32', st.modeB.childrenMaxFillPct)],
        ['Surplus Cap %', getInputText('#yro_B_scap_v32', st.modeB.surplusCapPct)]
      ];
    }
    if (mode === 'flat') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Parents', getSelectText('#yro_D_parents_v32', st.groups.D_parents)],
        ['Children', getSelectText('#yro_D_children_v32', st.groups.D_children)],
        ['Surplus', getSelectText('#yro_D_surplus_v32', st.groups.D_surplus)],
        ['Parent Reserve %', getInputText('#yro_D_parent_reserve_v32', st.modeD.parentReservePct)],
        ['Depot Cap %', getInputText('#yro_D_depot_cap_v32', st.modeD.depotCapPct)]
      ];
    }
    if (mode === 'coin') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Sources', getSelectText('#yro_C_sources_v32', st.groups.C_sources)],
        ['Coin', getSelectText('#yro_C_coin_v32', st.groups.C_coin)],
        ['Buffer / Surplus', getSelectText('#yro_C_buffer_v32', st.groups.C_buffer)],
        ['Source Reserve %', getInputText('#yro_C_reserve_v32', st.modeC.sourceReservePct)],
        ['Buffer Cap %', getInputText('#yro_C_bcap_v32', st.modeC.bufferCapPct)]
      ];
    }
  ...
  function buildSelectionTablesMarkdown(title, ids, noteLines, incomingMap, outByVid, scanAvailable) {
    var lines = ['### ' + title, ''];
    (noteLines || []).filter(Boolean).forEach(function (line) {
      lines.push('- ' + line);
    });
    if (noteLines && noteLines.length) lines.push('');

    lines.push('#### Table 1');
    if (!scanAvailable) {
      lines.push('Scan not available.');
    } else if (!ids || !ids.length) {
      lines.push('No villages in this set.');
    } else {
      var table1 = buildTable1Markdown(ids);
      lines.push(table1 || 'Group exists but has no scanned rows.');
    }
  ...
  function buildActiveSelectionSnapshotsMarkdown(plan) {
    var lines = ['## Active Selection Snapshots', ''];
    if (!plan || !plan.meta) {
    ...
    if (mode === 'balance') {
      var normalization = plan.meta.normalization || {};
      var beforeParking = plan.meta.surplusSupport && plan.meta.surplusSupport.beforeParking ? plan.meta.surplusSupport.beforeParking : { total: { wood: 0, clay: 0, iron: 0, total: 0 } };
      var afterParking = plan.meta.surplusSupport && plan.meta.surplusSupport.afterParking ? plan.meta.surplusSupport.afterParking : { total: { wood: 0, clay: 0, iron: 0, total: 0 } };
      lines.push(buildSelectionTablesMarkdown(
        'Selected Target',
        plan.targetIds || [],
        [
          'Balance Basis: ' + (plan.meta.balanceBasisLabel || '-'),
          String(plan.meta.balanceBasis || '').toLowerCase() === 'flat'
            ? ('Computed Flat Target: ' + formatRes(plan.meta.computedFlatTarget || 0))
            : ('Computed Fill %: ' + formatRes(plan.meta.computedFillPct || 0) + '%')
        ],
        incomingMap,
        outByVid,
        scanAvailable
      ...
      lines.push(buildSelectionTablesMarkdown(
        'Selected Surplus (raw selection)',
        plan.selectedSurplusIds || [],
        [
          'Selected Surplus villages: ' + formatRes((plan.selectedSurplusIds || []).length),
          normalization.excludedOverlapIds && normalization.excludedOverlapIds.length
            ? ('Overlap excluded: ' + normalization.excludedOverlapIds.join(', '))
            : 'Overlap excluded: -',
          'Usable room before parking: ' + formatResTripletShort(beforeParking.total)
        ],
        incomingMap,
        outByVid,
        scanAvailable
      ...
      lines.push(buildSelectionTablesMarkdown(
        'Normalized Usable Surplus',
        plan.surplusIds || [],
        [
          'Usable Surplus villages: ' + formatRes((plan.surplusIds || []).length),
          'Usable room before parking: ' + formatResTripletShort(beforeParking.total),
          'Remaining room after parking: ' + formatResTripletShort(afterParking.total)
        ],
        incomingMap,
        outByVid,
        scanAvailable
    ...
    if (mode === 'push') {
      lines.push(buildSelectionTablesMarkdown('Parents', plan.parentIds || [], [], incomingMap, outByVid, scanAvailable));
      lines.push(buildSelectionTablesMarkdown('Children', plan.childIds || [], [], incomingMap, outByVid, scanAvailable));
      lines.push(buildSelectionTablesMarkdown('Usable Surplus', plan.surplusIds || [], [], incomingMap, outByVid, scanAvailable));
      return lines.join('\n');
    }
    ...
    if (mode === 'coin') {
      lines.push(buildSelectionTablesMarkdown('Sources', plan.sourceIds || [], [], incomingMap, outByVid, scanAvailable));
      lines.push(buildSelectionTablesMarkdown('Coin', plan.coinIds || [], [], incomingMap, outByVid, scanAvailable));
      lines.push(buildSelectionTablesMarkdown('Buffer / Surplus', plan.bufferIds || [], [], incomingMap, outByVid, scanAvailable));
      return lines.join('\n');
    }

    lines.push('No active selection snapshot builder for this mode.');
    return lines.join('\n');
  }
  ...
  function buildActivePlanMetadataMarkdown(plan) {
    ...
    if (mode === 'balance') {
      var modeABasis = String(plan.meta.balanceBasis || '').toLowerCase() === 'flat' ? 'flat' : 'fill';
      rows.push(['Balance Basis', String(plan.meta.balanceBasisLabel || (modeABasis === 'flat' ? 'Equal Flat Amount per Village' : 'Equal Fill % by Storage'))]);
      if (modeABasis === 'flat') {
        rows.push(['Computed Flat Target', String(plan.meta.computedFlatTarget != null ? plan.meta.computedFlatTarget : '-')]);
        rows.push(['Depot Cap %', String(plan.meta.depotCapPct != null ? plan.meta.depotCapPct : '-')]);
      } else {
      ...
      if (plan.meta.normalization) {
        rows.push(['Selected Target villages', String(plan.meta.normalization.selectedTargetCount != null ? plan.meta.normalization.selectedTargetCount : '-')]);
        rows.push(['Selected Surplus villages', String(plan.meta.normalization.selectedSurplusCount != null ? plan.meta.normalization.selectedSurplusCount : '-')]);
        rows.push(['Usable Surplus villages', String(plan.meta.normalization.usableSurplusCount != null ? plan.meta.normalization.usableSurplusCount : '-')]);
      }
      if (plan.meta.surplusSupport && plan.meta.surplusSupport.beforeParking) {
        rows.push(['Usable Surplus room before parking', formatResTripletShort(plan.meta.surplusSupport.beforeParking.total)]);
      }
      if (plan.meta.surplusSupport && plan.meta.surplusSupport.afterParking) {
        rows.push(['Remaining Surplus room after parking', formatResTripletShort(plan.meta.surplusSupport.afterParking.total)]);
      }
    } else if (mode === 'push') {
      ...
      rows.push(['Depot Cap %', String(plan.meta.depotCapPct != null ? plan.meta.depotCapPct : '-')]);
      rows.push(['Parent Reserve %', String(plan.meta.parentReservePct != null ? plan.meta.parentReservePct : '-')]);
      rows.push(['Surplus', groupNameById(plan.meta.surplusGid)]);
      rows.push(['Child Merch Used', String(plan.meta.childMerchantsUsed != null ? plan.meta.childMerchantsUsed : '-')]);
      rows.push(['Parent Merch Used', String(plan.meta.parentMerchantsUsed != null ? plan.meta.parentMerchantsUsed : '-')]);
    } else if (mode === 'coin') {
  ...
  function buildActivePlanMainTablesMarkdown(plan) {
    ...
    if (mode === 'balance') {
      ...
      lines.push(buildMarkdownSection(
        'TARGET GROUP',
        modeABasis === 'flat'
          ? ['Village', 'Storage', 'Cap Each', 'Target Each', 'Capped?', 'Cur W', 'Cur C', 'Cur I', 'Inc W', 'Inc C', 'Inc I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used']
          : ['Village', 'Storage', 'Target Each', 'Cur W', 'Cur C', 'Cur I', 'Inc W', 'Inc C', 'Inc I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
        (plan.targetSummary || []).map(function (r) {
          return modeABasis === 'flat'
            ? [
              ...
              tripletCols(r.after),
              [merchPair(r), formatRes(r.merchUsed || 0)]
            )
            : [
              String(r.name || ('Village ' + r.id)),
              formatRes(r.storage),
      ...
      lines.push(buildMarkdownSection(
        'SURPLUS',
        ['Village', 'Storage', 'Cap Each', 'Cur W', 'Cur C', 'Cur I', 'Inc W', 'Inc C', 'Inc I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
      ...
      lines.push(buildMarkdownSection(
        'CHILDREN',
        ['Village', 'Storage', 'Target Each', 'Cur W', 'Cur C', 'Cur I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
      ...
      lines.push(buildMarkdownSection(
        'PARENTS',
        ['Village', 'Storage', 'Reserve Each', 'Cur W', 'Cur C', 'Cur I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Above Reserve After', 'Final W', 'Final C', 'Final I', 'Spread (Above Reserve)', 'Merch (free/total)', 'Merch Used'],
        (plan.parentSummary || []).map(function (r) {
          return [
            ...
            tripletCols(r.after),
            [formatRes(r.spreadAfter || 0), merchPair(r), formatRes(r.merchUsed || 0)]
          );
        }),
        'No parent rows.'
      ));
      if (plan.surplusSummary && plan.surplusSummary.length) {
        ...
        lines.push(buildMarkdownSection(
          'SURPLUS',
          ['Village', 'Storage', 'Cap Each', 'Cur W', 'Cur C', 'Cur I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
          plan.surplusSummary.map(function (r) {
            return [
              String(r.name || ('Village ' + r.id)),
              formatRes(r.storage),
      ...
      lines.push(buildMarkdownSection(
        'CHILDREN',
        ['Village', 'Storage', 'Cap Each', 'Target Each', 'Capped?', 'Cur W', 'Cur C', 'Cur I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
        (plan.childSummary || []).map(function (r) {
          return [
            String(r.name || ('Village ' + r.id)),
            formatRes(r.storage),
      ...
      lines.push(buildMarkdownSection(
        'COIN VILLAGES',
        ['Village', 'Storage', 'Coin Cost', 'Base Coins', 'Projected Coins', '+Coins', 'Next Deficit', 'Cur W', 'Cur C', 'Cur I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
      ...
      lines.push(buildMarkdownSection(
        'SOURCES',
        ['Village', 'Storage', 'Reserve Each', 'Cur W', 'Cur C', 'Cur I', 'Before Total', 'Sent', 'Recv', 'After Total', 'Available Left', 'Final W', 'Final C', 'Final I', 'Merch (free/total)', 'Merch Used'],
  ...
  async function buildTestResultsMarkdown() {
    ...
    var parts = [
      ...
      '',
      buildActiveSelectionSnapshotsMarkdown(plan),
      '',
  ...
  async function copyTestResults() {
    try {
      ...
      flashButton('#yro_copy_bb_v32', 'Copy Test Results');
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v32'), 'click', function () { Y.destroy(); });
    Y.on(Y.qs('#yro_load_v32'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v32');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v32'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v32');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v32');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v32'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v32');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v32');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v32'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v32'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v32'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v32'), 'click', function () { openPicker('t2'); });
    Y.on(Y.qs('#yro_copy_bb_v32'), 'click', function () { copyTestResults(); });
    Y.on(Y.qs('#yro_copy_plan_target_v32'), 'click', function () { copyPlanTargetBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_surplus_v32'), 'click', function () { copyPlanSurplusBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_ship_v32'), 'click', function () { copyPlanShipmentsBBCode(); });

    var modeBox = Y.qs('#yro_modes_v32');
    if (!modeBox) return;
    ...
    Y.on(modeBox, 'click', function (e) {
      ...
      if (t.id === 'yro_A_plan_v32') return planFromMode('balance');
      if (t.id === 'yro_B_plan_v32') return planFromMode('push');
      if (t.id === 'yro_D_plan_v32') return planFromMode('flat');
      if (t.id === 'yro_C_plan_v32') return planFromMode('coin');

      if (t.id === 'yro_A_exec_v32' || t.id === 'yro_B_exec_v32' || t.id === 'yro_D_exec_v32' || t.id === 'yro_C_exec_v32') return executePlan();

      if (String(t.id || '').startsWith('pick_') && String(t.id || '').endsWith('_v32')) {
        var key = String(t.id).replace('pick_', '').replace('_v32', '');
      ...
      if (row && row.getAttribute) {
        ...
        if (mode) {
          st.orchestrator.mode = mode;
          Y.saveState();
          Y.qsa('#yro_modes_v32 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
    ...
    Y.on(modeBox, 'change', function (e) {
      var t = e.target;
      if (!t || !t.id) return;
      var rerenderModes = false;

      if (t.id === 'yro_A_target_v32') st.groups.A_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_surplus_v32') st.groups.A_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parents_v32') st.groups.B_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_children_v32') st.groups.B_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_surplus_v32') st.groups.B_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_D_parents_v32') st.groups.D_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_D_children_v32') st.groups.D_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_D_surplus_v32') st.groups.D_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_sources_v32') st.groups.C_sources = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_coin_v32') st.groups.C_coin = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_buffer_v32') st.groups.C_buffer = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_basis_v32') { st.modeA.balanceBasis = String(t.value || '').toLowerCase() === 'flat' ? 'flat' : 'fill'; rerenderModes = true; }
      else if (t.id === 'yro_A_cap_v32') st.modeA.capPct = Y.safeInt(t.value, st.modeA.capPct);
      else if (t.id === 'yro_A_depot_cap_v32') st.modeA.depotCapPct = Y.safeInt(t.value, st.modeA.depotCapPct);
      else if (t.id === 'yro_A_scap_v32') st.modeA.surplusCapPct = Y.safeInt(t.value, st.modeA.surplusCapPct);
      else if (t.id === 'yro_B_parent_reserve_v32') st.modeB.parentReservePct = Y.safeInt(t.value, st.modeB.parentReservePct);
      else if (t.id === 'yro_B_child_max_v32') st.modeB.childrenMaxFillPct = Y.safeInt(t.value, st.modeB.childrenMaxFillPct);
      else if (t.id === 'yro_B_scap_v32') st.modeB.surplusCapPct = Y.safeInt(t.value, st.modeB.surplusCapPct);
      else if (t.id === 'yro_D_parent_reserve_v32') st.modeD.parentReservePct = Y.safeInt(t.value, st.modeD.parentReservePct);
      else if (t.id === 'yro_D_depot_cap_v32') st.modeD.depotCapPct = Y.safeInt(t.value, st.modeD.depotCapPct);
      else if (t.id === 'yro_C_reserve_v32') st.modeC.sourceReservePct = Y.safeInt(t.value, st.modeC.sourceReservePct);
      else if (t.id === 'yro_C_bcap_v32') st.modeC.bufferCapPct = Y.safeInt(t.value, st.modeC.bufferCapPct);
