(function () {
  ...
  var VERSION = 'v14';
  var NS = 'YRO_V14';
  var PANEL_ID = 'yro_panel_v14';
  var STYLE_ID = 'yro_css_v14';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v14';
  var LS_KEY = 'yro_v14_state';
  ...
  removeIfExists('yro_panel_v13'); removeIfExists('yro_css_v13');
  ...
  try { if (window.YRO_V12) delete window.YRO_V12; } catch (e2) {}
  try { if (window.YRO_V13) delete window.YRO_V13; } catch (e3) {}
  try { if (window.YRO_V14) delete window.YRO_V14; } catch (e2) {}
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v14{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  function buildPanel() {
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v14" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v14)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Developed by controleng</span>
    <button class="yro_btn" id="yro_close_v14">✖</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v14">Yükle / Yenile (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v14"></div></div>
  <span id="yro_prog_txt_v14" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v14" placeholder="Köy adı…" style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v14">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v14" style="color:#0a6;">Hazır ✅</div>

<div id="yro_content_v14">
  <div class="yro_sec_head">
    <div>1) Villages — Production (per hour + 24h) (<span id="yro_t1_title_v14">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v14">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v14"></select>
      <button class="yro_mini" id="yro_t1_pick_v14">Pick…</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v14" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v14"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages — Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v14">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v14">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v14"></select>
      <button class="yro_mini" id="yro_t2_pick_v14">Pick…</button>
      <span class="yro_small" id="yro_scan_ts_v14"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v14" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v14"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v14"></div>

  <div class="yro_msg" id="yro_plan_meta_v14" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Cap%:</b> - | <b>Reserve(each):</b> - | <b>Surplus Cap%:</b> - | <b>Shipments:</b> 0
  </div>

  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v14"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v14"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v14"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v14');
    var tx = Y.qs('#yro_prog_txt_v14');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v14', st.groups.sel1);
    fill('#yro_t2_group_sel_v14', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v14">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v14">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v14">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v14">Pick…</button>

    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v14" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v14" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <button class="yro_btn btn-confirm" id="yro_A_plan_v14">Plan</button>
    <button class="yro_btn btn-warn" id="yro_A_exec_tabs_v14">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_auto_v14">Execute (Auto)</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Sender</label><select id="yro_B_sender_v14">${optionsHTML(st.groups.B_sender)}</select></div>
    <button class="yro_mini" id="pick_B_sender_v14">Pick…</button>

    <div class="yro_kv"><label>Target</label><select id="yro_B_target_v14">${optionsHTML(st.groups.B_target)}</select></div>
    <button class="yro_mini" id="pick_B_target_v14">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v14">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v14">Pick…</button>

    <div class="yro_kv"><label>Reserve%</label><input id="yro_B_reserve_v14" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_B_cap_v14" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_B_scap_v14" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <button class="yro_btn btn-confirm" id="yro_B_plan_v14">Plan</button>
    <button class="yro_btn btn-warn" id="yro_B_exec_tabs_v14">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_auto_v14">Execute (Auto)</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_C_target_v14">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v14">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_C_surplus_v14">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v14">Pick…</button>

    <div class="yro_kv"><label>Reserve%</label><input id="yro_C_reserve_v14" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_C_cap_v14" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_C_scap_v14" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <button class="yro_btn btn-confirm" id="yro_C_plan_v14">Plan</button>
    <button class="yro_btn btn-warn" id="yro_C_exec_tabs_v14">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_auto_v14">Execute (Auto)</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v14">✖</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v14" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v14">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v14">Select none</button>
    <span class="yro_small" id="yro_picker_count_v14"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v14">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v14">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v14">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v14');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v14');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v14').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v14').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v14').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v14').value);
    });

    box.querySelector('#yro_picker_none_v14').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v14').value);
    });

    box.querySelector('#yro_picker_save_v14').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v14').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v14');
    if (metaEl) {
      metaEl.innerHTML =
    ...
    var tTbl = Y.qs('#yro_plan_target_tbl_v14');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v14');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v14');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v14');
    ...
    var t2title = Y.qs('#yro_t2_title_v14');
  ...
  function confirmTwoButtons(htmlMsg, onYes) {
    ...
    if (window.UI && UI.ConfirmationBox) UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v14', true);
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v14');
      surplusGid = getSelValue('#yro_A_surplus_v14');
      cap = getNumValue('#yro_A_cap_v14', 80);
      scap = getNumValue('#yro_A_scap_v14', 95);
      reserve = 0;
      ...
      senderGid = getSelValue('#yro_B_sender_v14');
      targetGid = getSelValue('#yro_B_target_v14');
      surplusGid = getSelValue('#yro_B_surplus_v14');
      reserve = getNumValue('#yro_B_reserve_v14', 1);
      cap = getNumValue('#yro_B_cap_v14', 80);
      scap = getNumValue('#yro_B_scap_v14', 95);
      ...
      targetGid = getSelValue('#yro_C_target_v14');
      surplusGid = getSelValue('#yro_C_surplus_v14');
      reserve = getNumValue('#yro_C_reserve_v14', 1);
      cap = getNumValue('#yro_C_cap_v14', 80);
      scap = getNumValue('#yro_C_scap_v14', 95);
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v14'), 'click', function () { Y.destroy(); });

    Y.on(Y.qs('#yro_load_v14'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v14');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v14'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v14');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v14');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v14'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v14');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v14');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v14'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v14'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v14'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v14'), 'click', function () { openPicker('t2'); });

    Y.on(Y.qs('#yro_copy_bb_v14'), 'click', function () { copyBBCode(); });

    var modeBox = Y.qs('#yro_modes_v14');
    if (modeBox) {
      // click delegation
      Y.on(modeBox, 'click', function (e) {
        ...
        if (t.id === 'yro_A_plan_v14') return planFromMode('balance');
        if (t.id === 'yro_B_plan_v14') return planFromMode('push');
        if (t.id === 'yro_C_plan_v14') return planFromMode('funnel');

        if (t.id === 'yro_A_exec_tabs_v14' || t.id === 'yro_B_exec_tabs_v14' || t.id === 'yro_C_exec_tabs_v14') return executeTabs();
        if (t.id === 'yro_A_exec_auto_v14' || t.id === 'yro_B_exec_auto_v14' || t.id === 'yro_C_exec_auto_v14') return executeAuto();

        if (String(t.id || '').startsWith('pick_') && String(t.id).endsWith('_v14')) {
          var key = String(t.id).replace('pick_', '').replace('_v14', '');
        ...
        if (row && row.getAttribute) {
          ...
          if (mode) {
            st.orchestrator.mode = mode;
            Y.saveState();
            Y.qsa('#yro_modes_v14 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
      ...
      Y.on(modeBox, 'change', function (e) {
        ...
        if (t.id === 'yro_A_target_v14') st.groups.A_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_A_surplus_v14') st.groups.A_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_sender_v14') st.groups.B_sender = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_target_v14') st.groups.B_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_surplus_v14') st.groups.B_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_target_v14') st.groups.C_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_surplus_v14') st.groups.C_surplus = Y.safeInt(t.value, 0);

        else if (t.id === 'yro_A_cap_v14' || t.id === 'yro_B_cap_v14' || t.id === 'yro_C_cap_v14') st.orchestrator.cap = Y.safeInt(t.value, st.orchestrator.cap);
        else if (t.id === 'yro_A_scap_v14' || t.id === 'yro_B_scap_v14' || t.id === 'yro_C_scap_v14') st.orchestrator.scap = Y.safeInt(t.value, st.orchestrator.scap);
        else if (t.id === 'yro_B_reserve_v14' || t.id === 'yro_C_reserve_v14') st.orchestrator.reserve = Y.safeInt(t.value, st.orchestrator.reserve);
  ...
  async function init() {
    ...
    if (!Object.keys(Y.state.cache.villages || {}).length) {
      ...
      try { Y.ui.setMsg('Hata ❌ — Console\u2019a bak', '#c00'); } catch (e2) {}
      console.error(e);
      throw e;
    }
...
try {
  if (window.YRO_V14 && typeof window.YRO_V14.init === 'function') {
    Promise.resolve(window.YRO_V14.init()).catch(function(e){ console.error(e); });
  }
