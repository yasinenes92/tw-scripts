(function () {
  ...
  var VERSION = 'v11';
  var NS = 'YRO_V11';
  var PANEL_ID = 'yro_panel_v11';
  var STYLE_ID = 'yro_css_v11';
  var LS_KEY = 'yro_v11_state';

  // Cleanup prior versions (only our ids)
  ...
  removeIfExists('yro_css_v10');
  removeIfExists(STYLE_ID);

  try { if (window.YRO_V10) delete window.YRO_V10; } catch (e) {}
  try { if (window.__YAVER_RESOURCE_ORCHESTRATOR_V7__) delete window.__YAVER_RESOURCE_ORCHESTRATOR_V7__; } catch (e2) {}
  ...
  // TW numbers like "1.262.616" => 1262616
  ...
  function getCurrentVillageId() {
    try {
      if (window.game_data && game_data.village && game_data.village.id) return safeInt(game_data.village.id, 0);
    } catch (e) {}
    try {
  ...
  // Extract JSON argument of TribalWars.updateGameData(...)
  ...
  function defaultState() {
    return {
      ...
      cache: {
        groupsFetchedAt: 0,
        groupVillageIds: {},     // gid -> {at, ids:[vid]}
        villages: {},            // vid -> {at, snapshot:{...}, transports:{...}}
        incomingMap: {},         // toVid -> {wood,clay,iron,total, byFrom:{fromVid:{...}}}
        outgoingMap: {},         // fromVid -> {wood,clay,iron,total, byTo:{toVid:{...}}}
        lastFullScanAt: 0,
      },
    };
  }
  ...
  Y.runtime = {
    plan: null,
    // filled after full scan
    snapshotsById: {}, // vid -> snapshot
    coordToId: {},     // "x|y" -> vid
    nameToIds: {},     // lowerName -> [vid]
  };
