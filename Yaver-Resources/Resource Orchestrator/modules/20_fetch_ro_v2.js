(function () {
  ...
  function parseGroupsFromDoc(doc) {
    ...
    var items = doc.querySelectorAll(
      (Y.data && Y.data.SELECTORS && Y.data.SELECTORS.groupMenuItems) ||
        ".group-menu-item[data-group-id], strong.group-menu-item[data-group-id]"
    );
    var seen = new Set([0]);
  ...
  function parseProductionVillages(doc) {
    var table =
      doc.querySelector((Y.data && Y.data.SELECTORS && Y.data.SELECTORS.productionTable) || "#production_table") || doc;
  ...
  function parseIncomingTransports(doc) {
    var rows = doc.querySelectorAll(
      (Y.data && Y.data.SELECTORS && Y.data.SELECTORS.tradesRows) || "#trades_table tr.row_a, #trades_table tr.row_b"
    );
    var inc = new Map();
  ...
  async function ensureGroups(onStep) {
    ...
    // If default Parents/Children exist, keep Table2 defaults aligned
