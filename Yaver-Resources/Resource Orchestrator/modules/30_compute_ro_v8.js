(function () {
  ...
  function filterByGroup(list, gid, targetSetIfMinusOne) {
    ...
    if (gid === -1) {
      // All except Target (targetSetIfMinusOne provided)
      var out = [];
    ...
    if (!set) return []; // must be ensured by main before compute
    var res = [];
  ...
  Y.compute.runTable1 = function () {
    ...
    var base = effListAll(); // reuse for village order + name/points
