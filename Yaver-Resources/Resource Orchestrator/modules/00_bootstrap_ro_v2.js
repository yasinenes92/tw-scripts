(function () {
  ...
  Y.cfg = {
    ...
    // Receiver fill cap for target group
    TARGET_CAP_PCT: 0.8,

    // Safe cap for surplus destination (avoid overflow)
    SURPLUS_DEST_CAP_PCT: 0.95
  };

  Y.state = {
    // discovered groups: [{id,name}]
    groups: [],

    // cache per groupId: { villages: [...], incomingRes: Map(villageId -> {wood,stone,iron}) }
    cache: new Map(),
    ...
    table2: {
      mode: "push", // balance | push | funnel
      senderGroupId: 35681, // default Parents (if exists)
      targetGroupId: 35682, // default Children (if exists)
      surplusGroupId: 35681, // default Parents
      reservePct: 1 // percent (each resource)
    },
