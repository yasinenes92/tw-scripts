(function () {
  ...
  async function ensureGroupsAndMembersForTables() {
    await Y.fetch.ensureGroups(function (d,t,m){ if (Y.ui.setProgress) Y.ui.setProgress(d,t,m); });

    // ensure member sets for current selections (if gid>0)
    var gids = [
    ...
    for (var i = 0; i < gids.length; i++) {
      ...
      if (gid > 0 && !Y.state.groupMembers.has(gid)) {
        try { await Y.fetch.ensureGroupMembers(gid); } catch(e) {}
      }
    }
  }
  ...
  Y.main.planMode = async function (mode) {
    try {
      ...
      // Ensure members for selected t3 groups
      ...
      for (var i = 0; i < need.length; i++) if (!Y.state.groupMembers.has(need[i])) await Y.fetch.ensureGroupMembers(need[i]);
