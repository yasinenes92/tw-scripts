...
`;
  ...
  function ensurePanel() {
    ...
    el.innerHTML = `
<div class="yro_hdr" id="yro_drag_v2" style="cursor:move;">
  <div class="yro_ttl">
    ${Y.util.iconSpan("wood")} Resource Orchestrator <span class="yro_small">(v2)</span>
  </div>
  <div>
    <span class="yro_dev">Developed by controleng</span>
    <button class="yro_btn" id="yro_close_v2">✖</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v2">Yükle / Yenile</button>

  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v2"></div></div>
  <span id="yro_prog_txt_v2" class="yro_small">Bekleniyor…</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v2" placeholder="Köy adı…" />

  <button class="yro_btn" id="yro_toggle_v2">Minimize villages</button>
</div>

<div class="yro_msg" id="yro_msg_v2">
  Kaynak: <b>overview_villages → prod</b> + <b>overview_villages → trader</b> (incoming resources).
</div>

<div id="yro_content_v2"></div>
`;

    document.body.appendChild(el);

    document.getElementById("yro_close_v2").onclick = function () { Y.destroy(); };

    // drag
    var isDragging = false, startX = 0, startY = 0, initL = 0, initT = 0;
    var handle = document.getElementById("yro_drag_v2");
    ...
    document.getElementById("yro_load_v2").onclick = function () {
      if (typeof Y.main.loadAndRender === "function") Y.main.loadAndRender(true);
    };

    document.getElementById("yro_toggle_v2").onclick = function () {
      Y.state.table1.villagesCollapsed = !Y.state.table1.villagesCollapsed;
      Y.ui.render();
    };

    var inp = document.getElementById("yro_search_v2");
  ...
  function setMsg(html, kind) {
    var el = document.getElementById("yro_msg_v2");
    if (!el) return;
    el.innerHTML = html;
  ...
  function setProgress(done, total, msg) {
    var bar = document.getElementById("yro_prog_bar_v2");
    var txt = document.getElementById("yro_prog_txt_v2");
...
<div class="yro_sec_head">
  ...
  <div class="yro_sec_controls">
    ...
    <select id="yro_t1_group_sel_v2">${opts}</select>
  </div>
</div>`;
...
<tr>
  ...
  <td class="yro_right">${cellEff(incTotal, r.eff.total, r.base.wood + r.base.stone + r.base.iron)}</td>
    ...
    if (Y.state.table1.villagesCollapsed) {
      body = `
<tr class="yro_linkrow" id="yro_t1_expand_row_v2">
...
<div class="yro_scroll">
  ...
  function buildTable2() {
    ...
    var includeAllExcept = (g.mode === "funnel");
...
<div class="yro_sec_head">
  ...
  <div class="yro_sec_controls">
    ...
    <select id="yro_t2_mode_v2">${modeOpts}</select>

    <span class="yro_small"><b>Sender:</b></span>
    <select id="yro_t2_sender_v2">${groupOptions(g.senderGroupId, includeAllExcept)}</select>

    <span class="yro_small"><b>Target:</b></span>
    <select id="yro_t2_target_v2">${groupOptions(g.targetGroupId, false)}</select>

    <span class="yro_small"><b>Surplus →</b></span>
    <select id="yro_t2_surplus_v2">${groupOptions(g.surplusGroupId, false)}</select>

    ${Y.util.iconImg("/graphic/buildings/storage.png","Reserve (each resource)")}
    <input id="yro_t2_reserve_v2" type="number" min="0" max="100" step="1" value="${String(g.reservePct)}" style="width:60px;" />
    <span class="yro_small">%</span>

    <button class="yro_btn btn-confirm" id="yro_t2_plan_v2">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_t2_exec_v2">Execute</button>
  </div>
</div>
${!c ? `<div class="yro_msg">Tablo 2 için plan yok. “Plan” bas.</div>` : ""}
`;
  }

  function renderTable2Details() {
    var c = Y.state.computed.t2;
    if (!c) return "";
...
<tr>
  ...
  <td class="yro_center"><span class="yro_small">${s.tag}</span></td>
    ...
    return (
      warnHtml +
      `<div class="yro_msg">
        <b>Tri-balance total (Bsum):</b> ${Y.util.n(c.Bsum)} per resource under cap(${Math.round(c.capPct * 100)}%).
  ...
  function bindHandlers() {
    var sel1 = document.getElementById("yro_t1_group_sel_v2");
    if (sel1) {
    ...
    var exp = document.getElementById("yro_t1_expand_row_v2");
    if (exp) {
    ...
    var modeSel = document.getElementById("yro_t2_mode_v2");
    var sndSel = document.getElementById("yro_t2_sender_v2");
    var tgtSel = document.getElementById("yro_t2_target_v2");
    var surSel = document.getElementById("yro_t2_surplus_v2");
    var resInp = document.getElementById("yro_t2_reserve_v2");
    var btnPlan = document.getElementById("yro_t2_plan_v2");
    var btnExec = document.getElementById("yro_t2_exec_v2");

    if (modeSel) {
      modeSel.onchange = function () {
        Y.state.table2.mode = modeSel.value;

        // Mode C: sender must display All except Target (-1)
        if (Y.state.table2.mode === "funnel") Y.state.table2.senderGroupId = -1;

        if (typeof Y.main.loadAndRender === "function") Y.main.loadAndRender(false);
      };
    }
    if (sndSel) sndSel.onchange = function () {
    ...
    if (surSel) surSel.onchange = function () {
    ...
    if (resInp) resInp.onchange = function () {
      var v = parseInt(resInp.value, 10);
      if (!isFinite(v)) v = 0;
      Y.state.table2.reservePct = v;
    };

    if (btnPlan) btnPlan.onclick = function () { if (Y.main.planTable2) Y.main.planTable2(); };
    if (btnExec) btnExec.onclick = function () { if (Y.main.executeTable2) Y.main.executeTable2(); };

    var btnToggle = document.getElementById("yro_toggle_v2");
  ...
  Y.ui.render = function () {
    var root = document.getElementById("yro_content_v2");
    if (!root) return;

    var html = "";
    html += buildTable1();
    html += buildTable2();
    html += renderTable2Details();
