(function () {
  ...
  Y.compute.planTable2 = function (mode) {
    ...
    // target, sender, surplus lists
    var listTarget = effListForData(getDataForGroup(cfg.targetGroupId || 0));
    var listSurplus = effListForData(getDataForGroup(cfg.surplusGroupId || 0));

    var listSender = [];
    if (mode === "push") {
      listSender = effListForData(getDataForGroup(cfg.senderGroupId || 0));
    } else if (mode === "funnel") {
      ...
      // balance: no external sender
      listSender = [];
    }

    // caps and tri-balance sum
    var targetCaps = listTarget.map(function (e) { return Math.floor((e.storage || 0) * capPct); });
    var capSum = 0; for (var i = 0; i < targetCaps.length; i++) capSum += targetCaps[i];
    ...
    var mSurplus = cloneMutable(listSurplus, surplusCapPct);

    var shipments = [];
    var warnings = [];

    // bottleneck-first order (important for merchant usage)
    ...
    // abundant-first for surplus routing
    var surplusOrder = order.slice().reverse();

    function destSpace(v, rk) {
      var cap = Math.floor((v.storage || 0) * surplusCapPct);
      return Math.max(0, cap - (v.cur[rk] || 0));
    }

    function allocateSurplusOneResource(rk) {
      var donors = [];
      for (var i = 0; i < mTarget.length; i++) {
        var v = mTarget[i];
        var b = bMap.get(v.id) || 0;
    ...
    surplusOrder.forEach(allocateSurplusOneResource);
