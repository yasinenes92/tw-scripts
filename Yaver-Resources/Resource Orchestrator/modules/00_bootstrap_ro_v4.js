(function () {
  ...
  Y.cfg = {
    PANEL_ID: "yro_panel_v4",
    STYLE_ID: "yro_style_v4",
  ...
  Y.state = {
    ...
    table2: {
      // active mode among "balance" | "push" | "funnel"
      mode: "balance",

      // per-mode config (kept separately for clarity)
      A: {
        targetGroupId: 0,
        surplusGroupId: 0,
        reservePct: 0,     // not used in A (kept for consistency)
        capPct: 80,        // target cap (per resource)
        surplusCapPct: 95  // surplus destination cap
      },
      B: {
        senderGroupId: 0,
        targetGroupId: 0,
      ...
      C: {
        // Funnel: default sender is All except target (pseudo = -1)
        senderGroupId: -1,
        targetGroupId: 0,
        surplusGroupId: 0,
        reservePct: 1,
    ...
    computed: {
      t1: null,
      t2: null // { mode, cfg, shipments, snapshots, warnings, meta }
    }
  };
