(function () {
  ...
  var VERSION = 'v30';
  var NS = 'YRO_V30';
  var PANEL_ID = 'yro_panel_v30';
  var STYLE_ID = 'yro_css_v30';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v30';
  var LS_KEY = 'yro_v30_state';
  ...
  function cleanupOlderInstances() {
    ...
    try { if (window.YRO_V30) delete window.YRO_V30; } catch (e3) {}
...
(function () {
  ...
  function normalizeModeDSets(parentIds, childIds) {
    parentIds = uniq(parentIds);
    childIds = uniq(childIds);
    var warnings = [];
    var excluded = [];
    ...
    return { parentIds: parentIds, childIds: childIds, warnings: warnings, excludedOverlapIds: excluded };
  }
  ...
  function buildModeDDiagnostics(finalRun, parentIds, childIds, snapshotsById, incomingMap, bestFlatTarget, depotCapPct, parentReservePct) {
    ...
    var excess = buildNonMovableExcessDiagnostics(finalRun.states, childIds, finalRun.targetsById, false);
    var acceptedParentAvail = sumParentAvailability(finalRun.states, parentIds);
    var acceptedParentSupport = buildParentNextRatioDiagnostics(finalRun.states, parentIds);
    ...
    if (excess.totals.sinkBlocked.total > 0) {
      lines.push('Sink-blocked excess: ' + formatResShort(excess.totals.sinkBlocked) + '. Leftover child excess stays home in Mode D.');
      var topSink = topDiagRows(excess.rows, 'sink_blocked', 2);
      if (topSink.length) lines.push('Top sink-blocked villages: ' + topSink.join(' | '));
    }
    if (acceptedParentAvail.total > 0) {
      lines.push(
    ...
    if (acceptedParentSupport.noMerchant.total > 0) {
      lines.push('Still trapped on no-merchant Parents after the accepted flat target: ' + formatResShort(acceptedParentSupport.noMerchant) + '.');
    }
  ...
  function simulateModeDCandidate(parentIds, childIds, snapshotsById, incomingMap, flatTargetEach, reservePct, depotCapPct) {
    var allIds = uniq(parentIds.concat(childIds));
    var st = buildPlanState(allIds, snapshotsById, incomingMap, 0, reservePct, null);
    var allocations = [];
    var built = buildModeDTargets(st, childIds, flatTargetEach, depotCapPct);
  ...
  function planFlat(parentIds, childIds, snapshotsById, incomingMap, depotCapPct, parentReservePct) {
    var normalized = normalizeModeDSets(parentIds, childIds);
    parentIds = normalized.parentIds;
    childIds = normalized.childIds;
    var warnings = normalized.warnings.slice();
    ...
    if (!childIds.length) {
      ...
      return {
        mode: 'flat',
        states: buildPlanState(uniq(parentIds), snapshotsById, incomingMap, 0, parentReservePct, null),
        parentIds: parentIds,
        childIds: [],
        allocations: [],
        shipments: [],
    ...
    while (lo <= hi) {
      var mid = Math.floor((lo + hi) / 2);
      var test = simulateModeDCandidate(parentIds, childIds, snapshotsById, incomingMap, mid, parentReservePct, depotCapPct);
      if (test.feasible) {
        bestFlatTarget = mid;
        lo = mid + 1;
      } else {
    ...
    var finalRun = simulateModeDCandidate(parentIds, childIds, snapshotsById, incomingMap, bestFlatTarget, parentReservePct, depotCapPct);
    var shipments = compileShipments(finalRun.states);
    var diagnostics = buildModeDDiagnostics(finalRun, parentIds, childIds, snapshotsById, incomingMap, bestFlatTarget, depotCapPct, parentReservePct);
    var plan = {
      mode: 'flat',
      states: finalRun.states,
      parentIds: parentIds,
  ...
  function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    // v30: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
#${Y.cfg.PANEL_ID} .yro_dev a{ color:#c9a45d; text-decoration:none; font-weight:bold; }
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v30{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  gap:8px;
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v30" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v30)</span></div>
  <div style="display:flex; align-items:center;">
    ...
    <button class="yro_btn" id="yro_close_v30">X</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v30">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v30"></div></div>
  <span id="yro_prog_txt_v30" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v30" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v30">Copy Test Results</button>
</div>

<div class="yro_msg" id="yro_msg_v30" style="color:#0a6;">Ready</div>

<div id="yro_content_v30">
  <div class="yro_sec_head">
    <div>1) Villages - Production (per hour + 24h) (<span id="yro_t1_title_v30">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v30">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v30"></select>
      <button class="yro_mini" id="yro_t1_pick_v30">Pick...</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v30" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v30"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages - Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v30">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v30">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v30"></select>
      <button class="yro_mini" id="yro_t2_pick_v30">Pick...</button>
      <span class="yro_small" id="yro_scan_ts_v30"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v30" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v30"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v30"></div>

  <div class="yro_msg" id="yro_plan_meta_v30" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Cap Ceiling %:</b> - | <b>Surplus Cap %:</b> - | <b>Merch Used:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -
  </div>

  <div class="yro_msg yro_plan_diag" id="yro_plan_diag_v30">Diagnostics: -</div>

  <div class="yro_sec_head">
    <div id="yro_plan_target_title_v30">TARGET GROUP</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_target_v30">Copy TARGET GROUP</button>
    </div>
  </div>
  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v30"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_surplus_title_v30">SURPLUS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_surplus_v30">Copy SURPLUS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v30"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_ship_title_v30">SHIPMENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_ship_v30">Copy SHIPMENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v30"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v30');
    var tx = Y.qs('#yro_prog_txt_v30');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v30', st.groups.sel1);
    fill('#yro_t2_group_sel_v30', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v30">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v30">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v30">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v30">Pick...</button>
    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v30" type="number" min="1" max="100" step="1" value="${st.modeA.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v30" type="number" min="1" max="100" step="1" value="${st.modeA.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Computed Fill %</label><input id="yro_A_computed_fill_v30" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_A_merch_used_v30" type="number" value="" style="width:70px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_A_plan_v30">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v30">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_B_parents_v30">${optionsHTML(st.groups.B_parents)}</select></div>
    <button class="yro_mini" id="pick_B_parents_v30">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_B_children_v30">${optionsHTML(st.groups.B_children)}</select></div>
    <button class="yro_mini" id="pick_B_children_v30">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v30">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v30">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_B_parent_reserve_v30" type="number" min="0" max="100" step="1" value="${st.modeB.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Children Max Fill %</label><input id="yro_B_child_max_v30" type="number" min="0" max="100" step="1" value="${st.modeB.childrenMaxFillPct}" style="width:78px;"></div>
    <div class="yro_kv"><label>Surplus Cap %</label><input id="yro_B_scap_v30" type="number" min="0" max="100" step="1" value="${st.modeB.surplusCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Child Fill %</label><input id="yro_B_computed_fill_v30" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_B_child_merch_v30" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_B_parent_merch_v30" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_B_plan_v30">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v30">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'flat' ? 'active' : ''}" data-mode="flat">
  <div class="yro_mode_left">
    ...
    <div class="yro_mode_desc">Parents feed Children to the highest feasible common flat target. Small-storage children cap at Depot Cap %.</div>
  </div>
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_D_parents_v30">${optionsHTML(st.groups.D_parents)}</select></div>
    <button class="yro_mini" id="pick_D_parents_v30">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_D_children_v30">${optionsHTML(st.groups.D_children)}</select></div>
    <button class="yro_mini" id="pick_D_children_v30">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_D_parent_reserve_v30" type="number" min="0" max="100" step="1" value="${st.modeD.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Depot Cap %</label><input id="yro_D_depot_cap_v30" type="number" min="0" max="100" step="1" value="${st.modeD.depotCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Flat Target</label><input id="yro_D_computed_flat_v30" type="number" value="" style="width:98px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_D_child_merch_v30" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_D_parent_merch_v30" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_D_plan_v30">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_D_exec_v30">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${(st.orchestrator.mode === 'coin' || st.orchestrator.mode === 'funnel') ? 'active' : ''}" data-mode="coin">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Sources</label><select id="yro_C_sources_v30">${optionsHTML(st.groups.C_sources)}</select></div>
    <button class="yro_mini" id="pick_C_sources_v30">Pick...</button>
    <div class="yro_kv"><label>Coin</label><select id="yro_C_coin_v30">${optionsHTML(st.groups.C_coin)}</select></div>
    <button class="yro_mini" id="pick_C_coin_v30">Pick...</button>
    <div class="yro_kv"><label>Buffer / Surplus</label><select id="yro_C_buffer_v30">${optionsHTML(st.groups.C_buffer)}</select></div>
    <button class="yro_mini" id="pick_C_buffer_v30">Pick...</button>
    <div class="yro_kv"><label>Source Reserve %</label><input id="yro_C_reserve_v30" type="number" min="0" max="100" step="1" value="${st.modeC.sourceReservePct}" style="width:72px;"></div>
    <div class="yro_kv"><label>Buffer Cap %</label><input id="yro_C_bcap_v30" type="number" min="0" max="100" step="1" value="${st.modeC.bufferCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Additional Coins</label><input id="yro_C_additional_coins_v30" type="number" value="" style="width:84px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_C_merch_used_v30" type="number" value="" style="width:72px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_C_plan_v30">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v30">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v30">Ã¢Å“â€“</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v30" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v30">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v30">Select none</button>
    <span class="yro_small" id="yro_picker_count_v30"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v30">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v30">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v30">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v30');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v30');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v30').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v30').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v30').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v30').value);
    });

    box.querySelector('#yro_picker_none_v30').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v30').value);
    });

    box.querySelector('#yro_picker_save_v30').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v30').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v30');
    var diagEl = Y.qs('#yro_plan_diag_v30');
    var tTbl = Y.qs('#yro_plan_target_tbl_v30');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v30');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v30');
    var targetTitleEl = Y.qs('#yro_plan_target_title_v30');
    var surplusTitleEl = Y.qs('#yro_plan_surplus_title_v30');
    var shipTitleEl = Y.qs('#yro_plan_ship_title_v30');
    var copyTargetEl = Y.qs('#yro_copy_plan_target_v30');
    var copySurplusEl = Y.qs('#yro_copy_plan_surplus_v30');
    var copyShipEl = Y.qs('#yro_copy_plan_ship_v30');

    function renderEmpty() {
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v30');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v30');
      var modeBFillEl = Y.qs('#yro_B_computed_fill_v30');
      var modeBChildMerchEl = Y.qs('#yro_B_child_merch_v30');
      var modeBParentMerchEl = Y.qs('#yro_B_parent_merch_v30');
      var modeDFlatEl = Y.qs('#yro_D_computed_flat_v30');
      var modeDChildMerchEl = Y.qs('#yro_D_child_merch_v30');
      var modeDParentMerchEl = Y.qs('#yro_D_parent_merch_v30');
      var modeCCoinsEl = Y.qs('#yro_C_additional_coins_v30');
      var modeCMerchEl = Y.qs('#yro_C_merch_used_v30');
    ...
    if (String(plan.mode || '').toLowerCase() === 'balance' && plan.targetSummary) {
      ...
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v30');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v30');
      var computedFillEl0 = Y.qs('#yro_B_computed_fill_v30');
      var childMerchEl0 = Y.qs('#yro_B_child_merch_v30');
      var parentMerchEl0 = Y.qs('#yro_B_parent_merch_v30');
      var modeDFlatEl0 = Y.qs('#yro_D_computed_flat_v30');
      var modeDChildMerchEl0 = Y.qs('#yro_D_child_merch_v30');
      var modeDParentMerchEl0 = Y.qs('#yro_D_parent_merch_v30');
      var coinAddEl0 = Y.qs('#yro_C_additional_coins_v30');
      var coinMerchEl0 = Y.qs('#yro_C_merch_used_v30');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl = Y.qs('#yro_B_computed_fill_v30');
      var childMerchEl = Y.qs('#yro_B_child_merch_v30');
      var parentMerchEl = Y.qs('#yro_B_parent_merch_v30');
      var modeAFillEl2 = Y.qs('#yro_A_computed_fill_v30');
      var modeAMerchEl2 = Y.qs('#yro_A_merch_used_v30');
      var modeDFlatEl1 = Y.qs('#yro_D_computed_flat_v30');
      var modeDChildMerchEl1 = Y.qs('#yro_D_child_merch_v30');
      var modeDParentMerchEl1 = Y.qs('#yro_D_parent_merch_v30');
      var coinAddEl1 = Y.qs('#yro_C_additional_coins_v30');
      var coinMerchEl1 = Y.qs('#yro_C_merch_used_v30');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      function buildParentsAndSurplusTable(parentRows, surplusRows) {
        var html = `
<tr><th colspan="12">PARENTS</th></tr>
<tr>
  <th>Village</th>
  <th class="yro_right">Storage</th>
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillElD0 = Y.qs('#yro_B_computed_fill_v30');
      var childMerchElD0 = Y.qs('#yro_B_child_merch_v30');
      var parentMerchElD0 = Y.qs('#yro_B_parent_merch_v30');
      var modeAFillElD0 = Y.qs('#yro_A_computed_fill_v30');
      var modeAMerchElD0 = Y.qs('#yro_A_merch_used_v30');
      var modeDFlatEl = Y.qs('#yro_D_computed_flat_v30');
      var modeDChildMerchEl = Y.qs('#yro_D_child_merch_v30');
      var modeDParentMerchEl = Y.qs('#yro_D_parent_merch_v30');
      var modeCCoinsElD0 = Y.qs('#yro_C_additional_coins_v30');
      var modeCMerchElD0 = Y.qs('#yro_C_merch_used_v30');
      ...
      if (targetTitleEl) targetTitleEl.textContent = 'CHILDREN';
      if (surplusTitleEl) surplusTitleEl.textContent = 'PARENTS';
      if (shipTitleEl) shipTitleEl.textContent = 'SHIPMENTS';
      if (copyTargetEl) copyTargetEl.textContent = 'Copy CHILDREN';
      if (copySurplusEl) copySurplusEl.textContent = 'Copy PARENTS';
      if (copyShipEl) copyShipEl.textContent = 'Copy SHIPMENTS';

      function buildFlatChildrenTable(rows) {
        var html = `
<tr><th colspan="12">CHILDREN</th></tr>
<tr>
  <th>Village</th>
  <th class="yro_right">Storage</th>
  <th class="yro_right">Cap Each</th>
  ...
  <th class="yro_right"><span class="icon header iron"></span></th>
        ...
        if (!rows.length) return html + '<tr><td colspan="12">-</td></tr>';
        rows.forEach(function (r) {
          html += `
<tr>
  <td><b>${escHtml(String(r.name))}</b></td>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      function buildFlatParentsTable(rows) {
        var html = `
<tr><th colspan="12">PARENTS</th></tr>
<tr>
  <th>Village</th>
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.merchUsed || 0)}</td>
      ...
      if (sTbl) sTbl.innerHTML = buildFlatParentsTable(plan.parentSummary || []);
      ...
      var computedFillElC0 = Y.qs('#yro_B_computed_fill_v30');
      var childMerchElC0 = Y.qs('#yro_B_child_merch_v30');
      var parentMerchElC0 = Y.qs('#yro_B_parent_merch_v30');
      var modeAFillElC0 = Y.qs('#yro_A_computed_fill_v30');
      var modeAMerchElC0 = Y.qs('#yro_A_merch_used_v30');
      var modeDFlatElC0 = Y.qs('#yro_D_computed_flat_v30');
      var modeDChildMerchElC0 = Y.qs('#yro_D_child_merch_v30');
      var modeDParentMerchElC0 = Y.qs('#yro_D_parent_merch_v30');
      var modeCCoinsEl = Y.qs('#yro_C_additional_coins_v30');
      var modeCMerchEl = Y.qs('#yro_C_merch_used_v30');
      ...
      function buildCoinTable(rows) {
        var html = `
<tr><th colspan="11">COIN VILLAGES</th></tr>
<tr>
  <th>Village</th>
  <th class="yro_right">Storage</th>
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl2 = Y.qs('#yro_B_computed_fill_v30');
      var childMerchEl2 = Y.qs('#yro_B_child_merch_v30');
      var parentMerchEl2 = Y.qs('#yro_B_parent_merch_v30');
      var modeAFillEl3 = Y.qs('#yro_A_computed_fill_v30');
      var modeAMerchEl3 = Y.qs('#yro_A_merch_used_v30');
      var modeDFlatEl2 = Y.qs('#yro_D_computed_flat_v30');
      var modeDChildMerchEl2 = Y.qs('#yro_D_child_merch_v30');
      var modeDParentMerchEl2 = Y.qs('#yro_D_parent_merch_v30');
      var modeCCoinsEl2 = Y.qs('#yro_C_additional_coins_v30');
      var modeCMerchEl2 = Y.qs('#yro_C_merch_used_v30');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v30');
    ...
    var t2title = Y.qs('#yro_t2_title_v30');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v30', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v30');
      surplusGid = getSelValue('#yro_A_surplus_v30');
      var capA = getNumValue('#yro_A_cap_v30', st.modeA.capPct || 80);
      var scapA = getNumValue('#yro_A_scap_v30', st.modeA.surplusCapPct || 95);
      ...
      var parentGid = getSelValue('#yro_B_parents_v30');
      var childGid = getSelValue('#yro_B_children_v30');
      surplusGid = getSelValue('#yro_B_surplus_v30');
      var parentReservePct = getNumValue('#yro_B_parent_reserve_v30', st.modeB.parentReservePct || 1);
      var childrenMaxFillPct = getNumValue('#yro_B_child_max_v30', st.modeB.childrenMaxFillPct || 80);
      var surplusCapPct = getNumValue('#yro_B_scap_v30', st.modeB.surplusCapPct || 95);
      ...
      var flatParentGid = getSelValue('#yro_D_parents_v30');
      var flatChildGid = getSelValue('#yro_D_children_v30');
      var flatParentReservePct = getNumValue('#yro_D_parent_reserve_v30', st.modeD.parentReservePct || 1);
      var depotCapPct = getNumValue('#yro_D_depot_cap_v30', st.modeD.depotCapPct || 80);
      ...
      var sourceGid = getSelValue('#yro_C_sources_v30');
      var coinGid = getSelValue('#yro_C_coin_v30');
      var bufferGid = getSelValue('#yro_C_buffer_v30');
      var sourceReservePct = getNumValue('#yro_C_reserve_v30', st.modeC.sourceReservePct || 1);
      var bufferCapPct = getNumValue('#yro_C_bcap_v30', st.modeC.bufferCapPct || 95);
  ...
  function buildFlatChildrenTableBBCode(plan) {
    ...
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Cap Each', 'Target Each', 'Capped?', 'Before', 'Sent', 'Recv', 'After', 'Wood', 'Clay', 'Iron'],
      rows
    ));
    lines.push('[/quote]');
    return lines.join('\n');
  ...
  function buildParentsTableBBCode(plan) {
    ...
    lines = lines.concat(bbTable(
      ['Village', 'Storage', 'Reserve Each', 'Before', 'Sent', 'After', 'Above Reserve After', 'Wood', 'Clay', 'Iron', 'Spread (Above Reserve)', 'Merch Used'],
      parentRows
    ));
  ...
  function buildFlatParentsTableBBCode(plan) {
    var lines = buildFlatMetaLines(plan, 'PARENTS');
  ...
  function copyPlanTargetBBCode() {
    ...
    if (mode === 'push') {
      ...
      flashButton('#yro_copy_plan_target_v30', 'Copy CHILDREN');
      copyText(text, 'CHILDREN BBCode copied');
      return;
    }
    if (mode === 'balance') {
      text = buildBalanceTargetTableBBCode(plan);
      flashButton('#yro_copy_plan_target_v30', 'Copy TARGET GROUP');
      copyText(text, 'TARGET GROUP BBCode copied');
      return;
    }
    if (mode === 'coin') {
      text = buildCoinTableBBCode(plan);
      flashButton('#yro_copy_plan_target_v30', 'Copy COIN VILLAGES');
    ...
    flashButton('#yro_copy_plan_target_v30', 'Copy TARGET');
  ...
  function copyPlanSurplusBBCode() {
    ...
    if (mode === 'balance') {
      ...
      flashButton('#yro_copy_plan_surplus_v30', 'Copy SURPLUS');
      copyText(text, 'SURPLUS BBCode copied');
      return;
    }
    if (mode === 'coin') {
      text = buildCoinSourcesBufferBBCode(plan);
      flashButton('#yro_copy_plan_surplus_v30', (plan.bufferSummary && plan.bufferSummary.length) ? 'Copy SOURCES / BUFFER' : 'Copy SOURCES');
    ...
    if (mode === 'flat') {
      ...
      flashButton('#yro_copy_plan_surplus_v30', 'Copy PARENTS');
      copyText(text, 'PARENTS BBCode copied');
      return;
    }
  ...
  function copyPlanShipmentsBBCode() {
    ...
    flashButton('#yro_copy_plan_ship_v30', 'Copy SHIPMENTS');
  ...
  function getActiveModeKey() {
    var active = Y.qs('#yro_modes_v30 .yro_mode_row.active');
  ...
  function buildCurrentModeInputRows() {
    ...
    if (mode === 'balance') {
      return [
        ...
        ['Target', getSelectText('#yro_A_target_v30', st.groups.A_target)],
        ['Surplus', getSelectText('#yro_A_surplus_v30', st.groups.A_surplus)],
        ['Cap %', getInputText('#yro_A_cap_v30', st.modeA.capPct)],
        ['Surplus Cap %', getInputText('#yro_A_scap_v30', st.modeA.surplusCapPct)]
      ];
    }
    if (mode === 'push') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Parents', getSelectText('#yro_B_parents_v30', st.groups.B_parents)],
        ['Children', getSelectText('#yro_B_children_v30', st.groups.B_children)],
        ['Surplus', getSelectText('#yro_B_surplus_v30', st.groups.B_surplus)],
        ['Parent Reserve %', getInputText('#yro_B_parent_reserve_v30', st.modeB.parentReservePct)],
        ['Children Max Fill %', getInputText('#yro_B_child_max_v30', st.modeB.childrenMaxFillPct)],
        ['Surplus Cap %', getInputText('#yro_B_scap_v30', st.modeB.surplusCapPct)]
      ];
    }
    if (mode === 'flat') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Parents', getSelectText('#yro_D_parents_v30', st.groups.D_parents)],
        ['Children', getSelectText('#yro_D_children_v30', st.groups.D_children)],
        ['Parent Reserve %', getInputText('#yro_D_parent_reserve_v30', st.modeD.parentReservePct)],
        ['Depot Cap %', getInputText('#yro_D_depot_cap_v30', st.modeD.depotCapPct)]
      ];
    }
    if (mode === 'coin') {
      return [
        ['Mode', activeModeLabel(mode)],
        ['Sources', getSelectText('#yro_C_sources_v30', st.groups.C_sources)],
        ['Coin', getSelectText('#yro_C_coin_v30', st.groups.C_coin)],
        ['Buffer / Surplus', getSelectText('#yro_C_buffer_v30', st.groups.C_buffer)],
        ['Source Reserve %', getInputText('#yro_C_reserve_v30', st.modeC.sourceReservePct)],
        ['Buffer Cap %', getInputText('#yro_C_bcap_v30', st.modeC.bufferCapPct)]
      ];
    }
  ...
  async function copyTestResults() {
    try {
      ...
      flashButton('#yro_copy_bb_v30', 'Copy Test Results');
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v30'), 'click', function () { Y.destroy(); });
    Y.on(Y.qs('#yro_load_v30'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v30');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v30'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v30');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v30');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v30'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v30');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v30');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v30'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v30'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v30'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v30'), 'click', function () { openPicker('t2'); });
    Y.on(Y.qs('#yro_copy_bb_v30'), 'click', function () { copyTestResults(); });
    Y.on(Y.qs('#yro_copy_plan_target_v30'), 'click', function () { copyPlanTargetBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_surplus_v30'), 'click', function () { copyPlanSurplusBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_ship_v30'), 'click', function () { copyPlanShipmentsBBCode(); });

    var modeBox = Y.qs('#yro_modes_v30');
    if (!modeBox) return;
    ...
    Y.on(modeBox, 'click', function (e) {
      ...
      if (t.id === 'yro_A_plan_v30') return planFromMode('balance');
      if (t.id === 'yro_B_plan_v30') return planFromMode('push');
      if (t.id === 'yro_D_plan_v30') return planFromMode('flat');
      if (t.id === 'yro_C_plan_v30') return planFromMode('coin');

      if (t.id === 'yro_A_exec_v30' || t.id === 'yro_B_exec_v30' || t.id === 'yro_D_exec_v30' || t.id === 'yro_C_exec_v30') return executePlan();

      if (String(t.id || '').startsWith('pick_') && String(t.id || '').endsWith('_v30')) {
        var key = String(t.id).replace('pick_', '').replace('_v30', '');
      ...
      if (row && row.getAttribute) {
        ...
        if (mode) {
          st.orchestrator.mode = mode;
          Y.saveState();
          Y.qsa('#yro_modes_v30 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
    ...
    Y.on(modeBox, 'change', function (e) {
      ...
      if (t.id === 'yro_A_target_v30') st.groups.A_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_surplus_v30') st.groups.A_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parents_v30') st.groups.B_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_children_v30') st.groups.B_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_surplus_v30') st.groups.B_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_D_parents_v30') st.groups.D_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_D_children_v30') st.groups.D_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_sources_v30') st.groups.C_sources = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_coin_v30') st.groups.C_coin = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_buffer_v30') st.groups.C_buffer = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_cap_v30') st.modeA.capPct = Y.safeInt(t.value, st.modeA.capPct);
      else if (t.id === 'yro_A_scap_v30') st.modeA.surplusCapPct = Y.safeInt(t.value, st.modeA.surplusCapPct);
      else if (t.id === 'yro_B_parent_reserve_v30') st.modeB.parentReservePct = Y.safeInt(t.value, st.modeB.parentReservePct);
      else if (t.id === 'yro_B_child_max_v30') st.modeB.childrenMaxFillPct = Y.safeInt(t.value, st.modeB.childrenMaxFillPct);
      else if (t.id === 'yro_B_scap_v30') st.modeB.surplusCapPct = Y.safeInt(t.value, st.modeB.surplusCapPct);
      else if (t.id === 'yro_D_parent_reserve_v30') st.modeD.parentReservePct = Y.safeInt(t.value, st.modeD.parentReservePct);
      else if (t.id === 'yro_D_depot_cap_v30') st.modeD.depotCapPct = Y.safeInt(t.value, st.modeD.depotCapPct);
      else if (t.id === 'yro_C_reserve_v30') st.modeC.sourceReservePct = Y.safeInt(t.value, st.modeC.sourceReservePct);
      else if (t.id === 'yro_C_bcap_v30') st.modeC.bufferCapPct = Y.safeInt(t.value, st.modeC.bufferCapPct);
