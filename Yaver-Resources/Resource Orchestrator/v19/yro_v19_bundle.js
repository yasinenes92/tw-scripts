(function () {
  ...
  var VERSION = 'v18';
  var NS = 'YRO_V18';
  var PANEL_ID = 'yro_panel_v18';
  var STYLE_ID = 'yro_css_v18';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v18';
  var LS_KEY = 'yro_v18_state';
  ...
  try { if (window.YRO_V18) delete window.YRO_V18; } catch (e2) {}
  ...
  async function fullScanAllVillages(force, onProgress) {
    ...
    async function worker() {
      while (idx < ids.length) {
        ...
        await Y.sleep(had429 ? 1200 : delayMs + Math.floor(Math.random()*120));
      }
    }
...
(function () {
  ...
  function computeDonorSupply(ids, statesMap, reservePct) {
    ...
    (ids || []).forEach(function (vid) {
      ...
      keepById[vid] = keep;
      var inc = (s.inc0 || { wood:0, clay:0, iron:0 });
      var w = Math.max(0, Math.floor(((s.base0.wood || 0) + (inc.wood || 0)) - keep));
      var c = Math.max(0, Math.floor(((s.base0.clay || 0) + (inc.clay || 0)) - keep));
      var i = Math.max(0, Math.floor(((s.base0.iron || 0) + (inc.iron || 0)) - keep));
...
function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    surplusIds = (surplusIds || []).filter(function (id) { return !tSet[String(id)]; });
    // v18: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v18{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  function buildPanel() {
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v18" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v18)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Developed by controleng</span>
    <button class="yro_btn" id="yro_close_v18">✖</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v18">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v18"></div></div>
  <span id="yro_prog_txt_v18" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v18" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v18">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v18" style="color:#0a6;">Ready ✅</div>

<div id="yro_content_v18">
  <div class="yro_sec_head">
    <div>1) Villages — Production (per hour + 24h) (<span id="yro_t1_title_v18">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v18">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v18"></select>
      <button class="yro_mini" id="yro_t1_pick_v18">Pick…</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v18" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v18"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages — Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v18">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v18">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v18"></select>
      <button class="yro_mini" id="yro_t2_pick_v18">Pick…</button>
      <span class="yro_small" id="yro_scan_ts_v18"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v18" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v18"></table>
  </div>

  <div class="yro_sec_head">
    <div>3) Orchestrator — Tri-balance + Surplus Fill (biggest storage first)</div>
    <div class="yro_sec_controls"><span class="yro_small">Tip: click on the empty area of a row.</span></div>
  </div>

  <div class="yro_mode_box" id="yro_modes_v18"></div>

  <div class="yro_msg" id="yro_plan_meta_v18" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Cap%:</b> - | <b>Reserve(each):</b> - | <b>Surplus Cap%:</b> - | <b>Tol%:</b> - | <b>Iron Δ%:</b> - | <b>Shipments:</b> 0
  </div>

  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v18"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v18"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v18"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v18');
    var tx = Y.qs('#yro_prog_txt_v18');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v18', st.groups.sel1);
    fill('#yro_t2_group_sel_v18', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  <div class="yro_mode_left">
    ...
    <div class="yro_mode_desc">Balances the Target group; sends leftovers to Surplus (biggest storage first).</div>
  </div>
  <div class="yro_mode_controls">
    <div class="yro_kv"><label title="Target group: villages that should be filled and balanced.">Target</label><select id="yro_A_target_v18">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v18">Pick…</button>

    <div class="yro_kv"><label title="Optional. Extra resources (leftovers) can be parked into this group after the Target is filled.">Surplus</label><select id="yro_A_surplus_v18">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v18">Pick…</button>

    <div class="yro_kv"><label title="Cap% is the target fill level per resource (Wood/Clay/Iron) as a percent of Storage. In Push mode it is auto-calculated.">Cap%</label><input id="yro_A_cap_v18" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Maximum fill for Surplus villages (as % of Storage per resource).">Surplus Cap%</label><input id="yro_A_scap_v18" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label title="Tolerance controls how far Wood and Clay may deviate from the target level (Cap%). Example: Tolerance 5 means around 1000 you allow 950–1050.">Tolerance%</label><input id="yro_A_tol_v18" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>

    <div class="yro_kv"><label title="Optional. If left empty, Iron uses the same tolerance as Wood/Clay. If set, Iron can deviate from the target level (Cap%) by this percent. Example: Iron Δ 20 means around 1000 you allow 800–1200 (for iron only).">Iron Δ%</label></div>


    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_A_iron_v18" type="number" min="0" max="50" step="1" value="" style="width:60px;" title="Optional. If empty, Iron uses the same tolerance as Wood/Clay. If set, it is Iron-only tolerance (percent). Example: 20 means around 1000 you allow 800–1200 for Iron."></div>

    <button class="yro_btn btn-confirm" id="yro_A_plan_v18">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v18">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  <div class="yro_mode_left">
    ...
    <div class="yro_mode_desc">Sends resources from Sender → Target while keeping Target balanced; parks leftovers into Surplus.</div>
  </div>
  <div class="yro_mode_controls">
    <div class="yro_kv"><label title="Sender group: villages that will send resources to the Target group.">Sender</label><select id="yro_B_sender_v18">${optionsHTML(st.groups.B_sender)}</select></div>
    <button class="yro_mini" id="pick_B_sender_v18">Pick…</button>

    <div class="yro_kv"><label title="Target group: villages that should be filled and balanced.">Target</label><select id="yro_B_target_v18">${optionsHTML(st.groups.B_target)}</select></div>
    <button class="yro_mini" id="pick_B_target_v18">Pick…</button>

    <div class="yro_kv"><label title="Optional. Extra resources (leftovers) can be parked into this group after the Target is filled.">Surplus</label><select id="yro_B_surplus_v18">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v18">Pick…</button>

    <div class="yro_kv"><label title="Reserve% keeps at least this percent of Storage in EACH resource at sender villages (safety floor).">Reserve%</label><input id="yro_B_reserve_v18" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Cap% is the target fill level per resource (Wood/Clay/Iron) as a percent of Storage. In Push mode it is auto-calculated.">Cap%</label><input id="yro_B_cap_v18" type="number" placeholder="auto" min="1" max="100" step="1" value="" style="width:60px;" disabled title="Auto calculated. The script chooses the highest feasible Cap% so that all TARGET villages reach a similar fill level within tolerance."></div>
    <div class="yro_kv"><label title="Maximum fill for Surplus villages (as % of Storage per resource).">Surplus Cap%</label><input id="yro_B_scap_v18" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label title="Tolerance controls how far Wood and Clay may deviate from the target level (Cap%). Example: Tolerance 5 means around 1000 you allow 950–1050.">Tolerance%</label><input id="yro_B_tol_v18" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>

    <div class="yro_kv"><label title="Optional. If left empty, Iron uses the same tolerance as Wood/Clay. If set, Iron can deviate from the target level (Cap%) by this percent. Example: Iron Δ 20 means around 1000 you allow 800–1200 (for iron only).">Iron Δ%</label><input id="yro_B_iron_v18" type="number" min="0" max="50" step="1" value="" placeholder="(same as Tol)" style="width:80px;"></div>


    <button class="yro_btn btn-confirm" id="yro_B_plan_v18">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v18">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  <div class="yro_mode_left">
    ...
    <div class="yro_mode_desc">Everyone outside Target → fills Target up to Cap; leftovers go to Surplus.</div>
  </div>
  <div class="yro_mode_controls">
    <div class="yro_kv"><label title="Target group: villages that should be filled and balanced.">Target</label><select id="yro_C_target_v18">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v18">Pick…</button>

    <div class="yro_kv"><label title="Optional. Extra resources (leftovers) can be parked into this group after the Target is filled.">Surplus</label><select id="yro_C_surplus_v18">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v18">Pick…</button>

    <div class="yro_kv"><label title="Reserve% keeps at least this percent of Storage in EACH resource at sender villages (safety floor).">Reserve%</label><input id="yro_C_reserve_v18" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Cap% is the target fill level per resource (Wood/Clay/Iron) as a percent of Storage. In Push mode it is auto-calculated.">Cap%</label><input id="yro_C_cap_v18" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Maximum fill for Surplus villages (as % of Storage per resource).">Surplus Cap%</label><input id="yro_C_scap_v18" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label title="Tolerance controls how far Wood and Clay may deviate from the target level (Cap%). Example: Tolerance 5 means around 1000 you allow 950–1050.">Tolerance%</label><input id="yro_C_tol_v18" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>

    <div class="yro_kv"><label title="Optional. If left empty, Iron uses the same tolerance as Wood/Clay. If set, Iron can deviate from the target level (Cap%) by this percent. Example: Iron Δ 20 means around 1000 you allow 800–1200 (for iron only).">Iron Δ%</label><input id="yro_C_iron_v18" type="number" min="0" max="50" step="1" value="" placeholder="(same as Tol)" style="width:80px;"></div>


    <button class="yro_btn btn-confirm" id="yro_C_plan_v18">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v18">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v18">✖</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v18" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v18">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v18">Select none</button>
    <span class="yro_small" id="yro_picker_count_v18"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v18">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v18">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v18">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v18');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v18');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v18').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v18').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v18').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v18').value);
    });

    box.querySelector('#yro_picker_none_v18').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v18').value);
    });

    box.querySelector('#yro_picker_save_v18').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v18').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v18');
    if (metaEl) {
      metaEl.innerHTML =
    ...
    var tTbl = Y.qs('#yro_plan_target_tbl_v18');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v18');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v18');
