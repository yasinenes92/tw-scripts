(function () {
  ...
  var VERSION = 'v20';
  var NS = 'YRO_V20';
  var PANEL_ID = 'yro_panel_v20';
  var STYLE_ID = 'yro_css_v20';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v20';
  var LS_KEY = 'yro_v20_state';
  ...
  try { if (window.YRO_V20) delete window.YRO_V20; } catch (e2) {}
...
function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    surplusIds = (surplusIds || []).filter(function (id) { return !tSet[String(id)]; });
    // v20: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v20{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  function buildPanel() {
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v20" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v20)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Developed by controleng</span>
    <button class="yro_btn" id="yro_close_v20">✖</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v20">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v20"></div></div>
  <span id="yro_prog_txt_v20" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v20" placeholder="Village adı…" style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v20">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v20" style="color:#0a6;">Hazır ✅</div>

<div id="yro_content_v20">
  <div class="yro_sec_head">
    <div>1) Villages — Production (per hour + 24h) (<span id="yro_t1_title_v20">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v20">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v20"></select>
      <button class="yro_mini" id="yro_t1_pick_v20">Pick…</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v20" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v20"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages — Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v20">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v20">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v20"></select>
      <button class="yro_mini" id="yro_t2_pick_v20">Pick…</button>
      <span class="yro_small" id="yro_scan_ts_v20"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v20" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v20"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v20"></div>

  <div class="yro_msg" id="yro_plan_meta_v20" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Cap%:</b> - | <b>Reserve(each):</b> - | <b>Surplus Cap%:</b> - | <b>Tol%:</b> - | <b>Iron Δ%:</b> - | <b>Shipments:</b> 0
  </div>

  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v20"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v20"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v20"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v20');
    var tx = Y.qs('#yro_prog_txt_v20');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v20', st.groups.sel1);
    fill('#yro_t2_group_sel_v20', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label title="Target group: villages that should be filled and balanced.">Target</label><select id="yro_A_target_v20">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v20">Pick…</button>

    <div class="yro_kv"><label title="Optional. Extra resources (leftovers) can be parked into this group after the Target is filled.">Surplus</label><select id="yro_A_surplus_v20">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v20">Pick…</button>

    <div class="yro_kv"><label title="Cap% is the target fill level per resource (Wood/Clay/Iron) as a percent of Storage. In Push mode it is auto-calculated.">Cap%</label><input id="yro_A_cap_v20" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Maximum fill for Surplus villages (as % of Storage per resource).">Surplus Cap%</label><input id="yro_A_scap_v20" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label title="Tolerance controls how far Wood and Clay may deviate from the target level (Cap%). Example: Tolerance 5 means around 1000 you allow 950–1050.">Tolerance%</label><input id="yro_A_tol_v20" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>

    <div class="yro_kv"><label title="Optional. If left empty, Iron uses the same tolerance as Wood/Clay. If set, Iron can deviate from the target level (Cap%) by this percent. Example: Iron Δ 20 means around 1000 you allow 800–1200 (for iron only).">Iron Δ%</label><input id="yro_A_iron_v20" type="number" min="0" max="50" step="1" value="" placeholder="(same as Tol)" style="width:80px;"></div>


    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_A_iron_v20" type="number" min="-50" max="50" step="1" value="" style="width:60px;" title="Iron hedefi: Wood/Clay’e göre yüzde fark. Örn -30 => 100/100/70, +30 => 100/100/130"></div>

    <button class="yro_btn btn-confirm" id="yro_A_plan_v20">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v20">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label title="Sender group: villages that will send resources to the Target group.">Sender</label><select id="yro_B_sender_v20">${optionsHTML(st.groups.B_sender)}</select></div>
    <button class="yro_mini" id="pick_B_sender_v20">Pick…</button>

    <div class="yro_kv"><label title="Target group: villages that should be filled and balanced.">Target</label><select id="yro_B_target_v20">${optionsHTML(st.groups.B_target)}</select></div>
    <button class="yro_mini" id="pick_B_target_v20">Pick…</button>

    <div class="yro_kv"><label title="Optional. Extra resources (leftovers) can be parked into this group after the Target is filled.">Surplus</label><select id="yro_B_surplus_v20">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v20">Pick…</button>

    <div class="yro_kv"><label title="Reserve% keeps at least this percent of Storage in EACH resource at sender villages (safety floor).">Reserve%</label><input id="yro_B_reserve_v20" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Cap% is the target fill level per resource (Wood/Clay/Iron) as a percent of Storage. In Push mode it is auto-calculated.">Cap%</label><input id="yro_B_cap_v20" type="number" placeholder="auto" min="1" max="100" step="1" value="" style="width:60px;" disabled title="Auto calculated. The script chooses the highest feasible Cap% so that all TARGET villages reach a similar fill level within tolerance."></div>
    <div class="yro_kv"><label title="Maximum fill for Surplus villages (as % of Storage per resource).">Surplus Cap%</label><input id="yro_B_scap_v20" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label title="Tolerance controls how far Wood and Clay may deviate from the target level (Cap%). Example: Tolerance 5 means around 1000 you allow 950–1050.">Tolerance%</label><input id="yro_B_tol_v20" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>

    <div class="yro_kv"><label title="Optional. If left empty, Iron uses the same tolerance as Wood/Clay. If set, Iron can deviate from the target level (Cap%) by this percent. Example: Iron Δ 20 means around 1000 you allow 800–1200 (for iron only).">Iron Δ%</label><input id="yro_B_iron_v20" type="number" min="0" max="50" step="1" value="" placeholder="(same as Tol)" style="width:80px;"></div>


    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_B_iron_v20" type="number" min="-50" max="50" step="1" value="" style="width:60px;" title="Iron hedefi: Wood/Clay’e göre yüzde fark."></div>

    <button class="yro_btn btn-confirm" id="yro_B_plan_v20">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v20">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label title="Target group: villages that should be filled and balanced.">Target</label><select id="yro_C_target_v20">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v20">Pick…</button>

    <div class="yro_kv"><label title="Optional. Extra resources (leftovers) can be parked into this group after the Target is filled.">Surplus</label><select id="yro_C_surplus_v20">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v20">Pick…</button>

    <div class="yro_kv"><label title="Reserve% keeps at least this percent of Storage in EACH resource at sender villages (safety floor).">Reserve%</label><input id="yro_C_reserve_v20" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Cap% is the target fill level per resource (Wood/Clay/Iron) as a percent of Storage. In Push mode it is auto-calculated.">Cap%</label><input id="yro_C_cap_v20" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label title="Maximum fill for Surplus villages (as % of Storage per resource).">Surplus Cap%</label><input id="yro_C_scap_v20" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <div class="yro_kv"><label title="Tolerance controls how far Wood and Clay may deviate from the target level (Cap%). Example: Tolerance 5 means around 1000 you allow 950–1050.">Tolerance%</label><input id="yro_C_tol_v20" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>

    <div class="yro_kv"><label title="Optional. If left empty, Iron uses the same tolerance as Wood/Clay. If set, Iron can deviate from the target level (Cap%) by this percent. Example: Iron Δ 20 means around 1000 you allow 800–1200 (for iron only).">Iron Δ%</label><input id="yro_C_iron_v20" type="number" min="0" max="50" step="1" value="" placeholder="(same as Tol)" style="width:80px;"></div>


    <div class="yro_kv"><label>Iron Δ%</label><input id="yro_C_iron_v20" type="number" min="-50" max="50" step="1" value="" style="width:60px;" title="Iron hedefi: Wood/Clay’e göre yüzde fark."></div>

    <button class="yro_btn btn-confirm" id="yro_C_plan_v20">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v20">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v20">✖</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v20" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v20">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v20">Select none</button>
    <span class="yro_small" id="yro_picker_count_v20"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v20">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v20">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v20">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v20');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v20');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v20').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v20').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v20').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v20').value);
    });

    box.querySelector('#yro_picker_none_v20').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v20').value);
    });

    box.querySelector('#yro_picker_save_v20').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v20').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v20');
    if (metaEl) {
      metaEl.innerHTML =
    ...
    var tTbl = Y.qs('#yro_plan_target_tbl_v20');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v20');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v20');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v20');
    ...
    var t2title = Y.qs('#yro_t2_title_v20');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v20', true);
    else {
...
').replace(/<[^>]*>/g, ''));
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v20');
      surplusGid = getSelValue('#yro_A_surplus_v20');
      cap = getNumValue('#yro_A_cap_v20', 80);
      scap = getNumValue('#yro_A_scap_v20', 95);
      var ironDelta = getNumValue('#yro_A_iron_v20', 0);
      reserve = 0;
      ...
      senderGid = getSelValue('#yro_B_sender_v20');
      targetGid = getSelValue('#yro_B_target_v20');
      surplusGid = getSelValue('#yro_B_surplus_v20');
      reserve = getNumValue('#yro_B_reserve_v20', 1);
      cap = 0; // auto
      scap = getNumValue('#yro_B_scap_v20', 95);
      var tolPct = getNumValue('#yro_B_tol_v20', 5);
      var ironDeltaRawEl = Y.qs('#yro_B_iron_v20');
      ...
      var capEl = Y.qs('#yro_B_cap_v20'); if (capEl) capEl.value = (cap >= 2 ? cap : '');
      ...
      targetGid = getSelValue('#yro_C_target_v20');
      surplusGid = getSelValue('#yro_C_surplus_v20');
      reserve = getNumValue('#yro_C_reserve_v20', 1);
      cap = getNumValue('#yro_C_cap_v20', 80);
      scap = getNumValue('#yro_C_scap_v20', 95);
      var ironDelta = getNumValue('#yro_C_iron_v20', 0);
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v20'), 'click', function () { Y.destroy(); });

    Y.on(Y.qs('#yro_load_v20'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v20');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v20'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v20');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v20');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v20'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v20');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v20');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v20'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v20'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v20'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v20'), 'click', function () { openPicker('t2'); });

    Y.on(Y.qs('#yro_copy_bb_v20'), 'click', function () { copyBBCode(); });

    var modeBox = Y.qs('#yro_modes_v20');
    if (modeBox) {
      // click delegation
      Y.on(modeBox, 'click', function (e) {
        ...
        if (t.id === 'yro_A_plan_v20') return planFromMode('balance');
        if (t.id === 'yro_B_plan_v20') return planFromMode('push');
        if (t.id === 'yro_C_plan_v20') return planFromMode('funnel');

        if (t.id === 'yro_A_exec_v20' || t.id === 'yro_B_exec_v20' || t.id === 'yro_C_exec_v20') return executePlan();

        if (String(t.id || '').startsWith('pick_') && String(t.id).endsWith('_v20')) {
          var key = String(t.id).replace('pick_', '').replace('_v20', '');
        ...
        if (row && row.getAttribute) {
          ...
          if (mode) {
            st.orchestrator.mode = mode;
            Y.saveState();
            Y.qsa('#yro_modes_v20 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
      ...
      Y.on(modeBox, 'change', function (e) {
        ...
        if (t.id === 'yro_A_target_v20') st.groups.A_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_A_surplus_v20') st.groups.A_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_sender_v20') st.groups.B_sender = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_target_v20') st.groups.B_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_surplus_v20') st.groups.B_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_target_v20') st.groups.C_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_surplus_v20') st.groups.C_surplus = Y.safeInt(t.value, 0);

        else if (t.id === 'yro_A_cap_v20' || t.id === 'yro_B_cap_v20' || t.id === 'yro_C_cap_v20') st.orchestrator.cap = Y.safeInt(t.value, st.orchestrator.cap);
        else if (t.id === 'yro_A_scap_v20' || t.id === 'yro_B_scap_v20' || t.id === 'yro_C_scap_v20') st.orchestrator.scap = Y.safeInt(t.value, st.orchestrator.scap);
        else if (t.id === 'yro_B_reserve_v20' || t.id === 'yro_C_reserve_v20') st.orchestrator.reserve = Y.safeInt(t.value, st.orchestrator.reserve);
