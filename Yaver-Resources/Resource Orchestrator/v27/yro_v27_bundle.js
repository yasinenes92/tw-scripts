(function () {
  ...
  var VERSION = 'v27';
  var NS = 'YRO_V27';
  var PANEL_ID = 'yro_panel_v27';
  var STYLE_ID = 'yro_css_v27';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v27';
  var LS_KEY = 'yro_v27_state';
  ...
  function cleanupOlderInstances() {
    ...
    try { if (window.YRO_V27) delete window.YRO_V27; } catch (e3) {}
...
function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    // v27: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v27{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  gap:8px;
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v27" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v27)</span></div>
  <div style="display:flex; align-items:center;">
    ...
    <button class="yro_btn" id="yro_close_v27">X</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v27">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v27"></div></div>
  <span id="yro_prog_txt_v27" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v27" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v27">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v27" style="color:#0a6;">Ready</div>

<div id="yro_content_v27">
  <div class="yro_sec_head">
    <div>1) Villages - Production (per hour + 24h) (<span id="yro_t1_title_v27">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v27">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v27"></select>
      <button class="yro_mini" id="yro_t1_pick_v27">Pick...</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v27" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v27"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages - Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v27">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v27">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v27"></select>
      <button class="yro_mini" id="yro_t2_pick_v27">Pick...</button>
      <span class="yro_small" id="yro_scan_ts_v27"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v27" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v27"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v27"></div>

  <div class="yro_msg" id="yro_plan_meta_v27" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Cap Ceiling %:</b> - | <b>Surplus Cap %:</b> - | <b>Merch Used:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -
  </div>

  <div class="yro_msg yro_plan_diag" id="yro_plan_diag_v27">Diagnostics: -</div>

  <div class="yro_sec_head">
    <div id="yro_plan_target_title_v27">TARGET GROUP</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_target_v27">Copy TARGET GROUP</button>
    </div>
  </div>
  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v27"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_surplus_title_v27">SURPLUS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_surplus_v27">Copy SURPLUS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v27"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_ship_title_v27">SHIPMENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_ship_v27">Copy SHIPMENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v27"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v27');
    var tx = Y.qs('#yro_prog_txt_v27');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v27', st.groups.sel1);
    fill('#yro_t2_group_sel_v27', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v27">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v27">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v27">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v27">Pick...</button>
    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v27" type="number" min="1" max="100" step="1" value="${st.modeA.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v27" type="number" min="1" max="100" step="1" value="${st.modeA.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Computed Fill %</label><input id="yro_A_computed_fill_v27" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Merch Used</label><input id="yro_A_merch_used_v27" type="number" value="" style="width:70px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_A_plan_v27">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v27">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_B_parents_v27">${optionsHTML(st.groups.B_parents)}</select></div>
    <button class="yro_mini" id="pick_B_parents_v27">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_B_children_v27">${optionsHTML(st.groups.B_children)}</select></div>
    <button class="yro_mini" id="pick_B_children_v27">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v27">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v27">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_B_parent_reserve_v27" type="number" min="0" max="100" step="1" value="${st.modeB.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Children Max Fill %</label><input id="yro_B_child_max_v27" type="number" min="0" max="100" step="1" value="${st.modeB.childrenMaxFillPct}" style="width:78px;"></div>
    <div class="yro_kv"><label>Surplus Cap %</label><input id="yro_B_scap_v27" type="number" min="0" max="100" step="1" value="${st.modeB.surplusCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Child Fill %</label><input id="yro_B_computed_fill_v27" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_B_child_merch_v27" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_B_parent_merch_v27" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_B_plan_v27">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v27">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_C_target_v27">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v27">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_C_surplus_v27">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v27">Pick...</button>
    <div class="yro_kv"><label>Reserve%</label><input id="yro_C_reserve_v27" type="number" min="0" max="100" step="1" value="${st.modeC.reservePct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_C_cap_v27" type="number" min="1" max="100" step="1" value="${st.modeC.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_C_scap_v27" type="number" min="1" max="100" step="1" value="${st.modeC.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Tolerance%</label><input id="yro_C_tol_v27" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>
    <div class="yro_kv"><label>Iron Delta%</label><input id="yro_C_iron_v27" type="number" min="-50" max="50" step="1" value="${st.modeC.ironDeltaPct == null ? '' : st.modeC.ironDeltaPct}" style="width:70px;"></div>
    <button class="yro_btn btn-confirm" id="yro_C_plan_v27">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v27">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v27">Ã¢Å“â€“</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v27" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v27">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v27">Select none</button>
    <span class="yro_small" id="yro_picker_count_v27"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v27">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v27">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v27">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v27');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v27');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v27').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v27').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v27').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v27').value);
    });

    box.querySelector('#yro_picker_none_v27').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v27').value);
    });

    box.querySelector('#yro_picker_save_v27').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v27').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v27');
    var diagEl = Y.qs('#yro_plan_diag_v27');
    var tTbl = Y.qs('#yro_plan_target_tbl_v27');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v27');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v27');
    var targetTitleEl = Y.qs('#yro_plan_target_title_v27');
    var surplusTitleEl = Y.qs('#yro_plan_surplus_title_v27');
    var shipTitleEl = Y.qs('#yro_plan_ship_title_v27');
    var copyTargetEl = Y.qs('#yro_copy_plan_target_v27');
    var copySurplusEl = Y.qs('#yro_copy_plan_surplus_v27');
    var copyShipEl = Y.qs('#yro_copy_plan_ship_v27');

    function renderEmpty() {
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v27');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v27');
      var modeBFillEl = Y.qs('#yro_B_computed_fill_v27');
      var modeBChildMerchEl = Y.qs('#yro_B_child_merch_v27');
      var modeBParentMerchEl = Y.qs('#yro_B_parent_merch_v27');
    ...
    if (String(plan.mode || '').toLowerCase() === 'balance' && plan.targetSummary) {
      ...
      var modeAFillEl = Y.qs('#yro_A_computed_fill_v27');
      var modeAMerchEl = Y.qs('#yro_A_merch_used_v27');
      var computedFillEl0 = Y.qs('#yro_B_computed_fill_v27');
      var childMerchEl0 = Y.qs('#yro_B_child_merch_v27');
      var parentMerchEl0 = Y.qs('#yro_B_parent_merch_v27');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl = Y.qs('#yro_B_computed_fill_v27');
      var childMerchEl = Y.qs('#yro_B_child_merch_v27');
      var parentMerchEl = Y.qs('#yro_B_parent_merch_v27');
      var modeAFillEl2 = Y.qs('#yro_A_computed_fill_v27');
      var modeAMerchEl2 = Y.qs('#yro_A_merch_used_v27');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl2 = Y.qs('#yro_B_computed_fill_v27');
      var childMerchEl2 = Y.qs('#yro_B_child_merch_v27');
      var parentMerchEl2 = Y.qs('#yro_B_parent_merch_v27');
      var modeAFillEl3 = Y.qs('#yro_A_computed_fill_v27');
      var modeAMerchEl3 = Y.qs('#yro_A_merch_used_v27');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v27');
    ...
    var t2title = Y.qs('#yro_t2_title_v27');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v27', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v27');
      surplusGid = getSelValue('#yro_A_surplus_v27');
      var capA = getNumValue('#yro_A_cap_v27', st.modeA.capPct || 80);
      var scapA = getNumValue('#yro_A_scap_v27', st.modeA.surplusCapPct || 95);
      ...
      var parentGid = getSelValue('#yro_B_parents_v27');
      var childGid = getSelValue('#yro_B_children_v27');
      surplusGid = getSelValue('#yro_B_surplus_v27');
      var parentReservePct = getNumValue('#yro_B_parent_reserve_v27', st.modeB.parentReservePct || 1);
      var childrenMaxFillPct = getNumValue('#yro_B_child_max_v27', st.modeB.childrenMaxFillPct || 80);
      var surplusCapPct = getNumValue('#yro_B_scap_v27', st.modeB.surplusCapPct || 95);
      ...
      targetGid = getSelValue('#yro_C_target_v27');
      surplusGid = getSelValue('#yro_C_surplus_v27');
      var reserveC = getNumValue('#yro_C_reserve_v27', st.modeC.reservePct || 1);
      var capC = getNumValue('#yro_C_cap_v27', st.modeC.capPct || 80);
      var scapC = getNumValue('#yro_C_scap_v27', st.modeC.surplusCapPct || 95);
      var ironDeltaC = getNumValue('#yro_C_iron_v27', st.modeC.ironDeltaPct || 0);
  ...
  function copyPlanSurplusBBCode() {
    ...
    if (mode === 'balance') {
      ...
      flashButton('#yro_copy_plan_surplus_v27', 'Copy SURPLUS');
  ...
  function copyPlanShipmentsBBCode() {
    ...
    flashButton('#yro_copy_plan_ship_v27', 'Copy SHIPMENTS');
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v27'), 'click', function () { Y.destroy(); });
    Y.on(Y.qs('#yro_load_v27'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v27');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v27'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v27');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v27');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v27'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v27');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v27');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v27'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v27'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v27'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v27'), 'click', function () { openPicker('t2'); });
    Y.on(Y.qs('#yro_copy_bb_v27'), 'click', function () { copyBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_target_v27'), 'click', function () { copyPlanTargetBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_surplus_v27'), 'click', function () { copyPlanSurplusBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_ship_v27'), 'click', function () { copyPlanShipmentsBBCode(); });

    var modeBox = Y.qs('#yro_modes_v27');
    if (!modeBox) return;
    ...
    Y.on(modeBox, 'click', function (e) {
      ...
      if (t.id === 'yro_A_plan_v27') return planFromMode('balance');
      if (t.id === 'yro_B_plan_v27') return planFromMode('push');
      if (t.id === 'yro_C_plan_v27') return planFromMode('funnel');

      if (t.id === 'yro_A_exec_v27' || t.id === 'yro_B_exec_v27' || t.id === 'yro_C_exec_v27') return executePlan();

      if (String(t.id || '').startsWith('pick_') && String(t.id || '').endsWith('_v27')) {
        var key = String(t.id).replace('pick_', '').replace('_v27', '');
      ...
      if (row && row.getAttribute) {
        ...
        if (mode) {
          st.orchestrator.mode = mode;
          Y.saveState();
          Y.qsa('#yro_modes_v27 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
    ...
    Y.on(modeBox, 'change', function (e) {
      ...
      if (t.id === 'yro_A_target_v27') st.groups.A_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_surplus_v27') st.groups.A_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parents_v27') st.groups.B_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_children_v27') st.groups.B_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_surplus_v27') st.groups.B_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_target_v27') st.groups.C_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_surplus_v27') st.groups.C_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_cap_v27') st.modeA.capPct = Y.safeInt(t.value, st.modeA.capPct);
      else if (t.id === 'yro_A_scap_v27') st.modeA.surplusCapPct = Y.safeInt(t.value, st.modeA.surplusCapPct);
      else if (t.id === 'yro_B_parent_reserve_v27') st.modeB.parentReservePct = Y.safeInt(t.value, st.modeB.parentReservePct);
      else if (t.id === 'yro_B_child_max_v27') st.modeB.childrenMaxFillPct = Y.safeInt(t.value, st.modeB.childrenMaxFillPct);
      else if (t.id === 'yro_B_scap_v27') st.modeB.surplusCapPct = Y.safeInt(t.value, st.modeB.surplusCapPct);
      else if (t.id === 'yro_C_reserve_v27') st.modeC.reservePct = Y.safeInt(t.value, st.modeC.reservePct);
      else if (t.id === 'yro_C_cap_v27') st.modeC.capPct = Y.safeInt(t.value, st.modeC.capPct);
      else if (t.id === 'yro_C_scap_v27') st.modeC.surplusCapPct = Y.safeInt(t.value, st.modeC.surplusCapPct);
      else if (t.id === 'yro_C_iron_v27') st.modeC.ironDeltaPct = String(t.value).trim() === '' ? null : Y.safeInt(t.value, 0);
