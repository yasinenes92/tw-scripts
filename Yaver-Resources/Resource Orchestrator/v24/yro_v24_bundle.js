(function () {
  ...
  var VERSION = 'v24';
  var NS = 'YRO_V24';
  var PANEL_ID = 'yro_panel_v24';
  var STYLE_ID = 'yro_css_v24';
  var PICKER_OVERLAY_ID = 'yro_picker_overlay_v24';
  var LS_KEY = 'yro_v24_state';
  ...
  function cleanupOlderInstances() {
    ...
    try { if (window.YRO_V24) delete window.YRO_V24; } catch (e3) {}
  ...
  function sanitizeState(st) {
    ...
    st.modeA.ironDeltaPct = st.modeA.ironDeltaPct == null || st.modeA.ironDeltaPct === '' ? null : safeInt(st.modeA.ironDeltaPct, 0);
...
function planFunnel(allIds, targetIds, surplusIds, snapshotsById, incomingMap, capPct, surplusCapPct, reservePct, ironDeltaPct) {
    ...
    surplusIds = (surplusIds || []).filter(function (id) { return !tSet[String(id)]; });
    // v24: Sender list should NOT be nulled by Surplus selection; only exclude Targets.
...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v24{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  gap:8px;
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v24" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v24)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Full scan backbone preserved</span>
    <button class="yro_btn" id="yro_close_v24">X</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v24">Load / Refresh (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v24"></div></div>
  <span id="yro_prog_txt_v24" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v24" placeholder="Village name..." style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v24">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v24" style="color:#0a6;">Ready</div>

<div id="yro_content_v24">
  <div class="yro_sec_head">
    <div>1) Villages - Production (per hour + 24h) (<span id="yro_t1_title_v24">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v24">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v24"></select>
      <button class="yro_mini" id="yro_t1_pick_v24">Pick...</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v24" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v24"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages - Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v24">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v24">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v24"></select>
      <button class="yro_mini" id="yro_t2_pick_v24">Pick...</button>
      <span class="yro_small" id="yro_scan_ts_v24"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v24" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v24"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v24"></div>

  <div class="yro_msg" id="yro_plan_meta_v24" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Children Max Fill %:</b> - | <b>Parent Reserve %:</b> - | <b>Surplus Cap %:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -
  </div>

  <div class="yro_msg yro_plan_diag" id="yro_plan_diag_v24">Diagnostics: -</div>

  <div class="yro_sec_head">
    <div id="yro_plan_target_title_v24">CHILDREN</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_target_v24">Copy CHILDREN</button>
    </div>
  </div>
  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v24"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_surplus_title_v24">PARENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_surplus_v24">Copy PARENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v24"></table>
  </div>

  <div class="yro_sec_head">
    <div id="yro_plan_ship_title_v24">SHIPMENTS</div>
    <div class="yro_sec_controls yro_table_tools">
      <button class="yro_mini" id="yro_copy_plan_ship_v24">Copy SHIPMENTS</button>
    </div>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v24"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v24');
    var tx = Y.qs('#yro_prog_txt_v24');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v24', st.groups.sel1);
    fill('#yro_t2_group_sel_v24', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  <div class="yro_mode_left">
    ...
    <div class="yro_mode_desc">Target tri-balance + leftovers into Surplus.</div>
  </div>
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v24">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v24">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v24">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v24">Pick...</button>
    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v24" type="number" min="1" max="100" step="1" value="${st.modeA.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v24" type="number" min="1" max="100" step="1" value="${st.modeA.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Tolerance%</label><input id="yro_A_tol_v24" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>
    <div class="yro_kv"><label>Iron Delta%</label><input id="yro_A_iron_v24" type="number" min="-50" max="50" step="1" value="${st.modeA.ironDeltaPct == null ? '' : st.modeA.ironDeltaPct}" style="width:70px;"></div>
    <button class="yro_btn btn-confirm" id="yro_A_plan_v24">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_v24">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Parents</label><select id="yro_B_parents_v24">${optionsHTML(st.groups.B_parents)}</select></div>
    <button class="yro_mini" id="pick_B_parents_v24">Pick...</button>
    <div class="yro_kv"><label>Children</label><select id="yro_B_children_v24">${optionsHTML(st.groups.B_children)}</select></div>
    <button class="yro_mini" id="pick_B_children_v24">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v24">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v24">Pick...</button>
    <div class="yro_kv"><label>Parent Reserve %</label><input id="yro_B_parent_reserve_v24" type="number" min="0" max="100" step="1" value="${st.modeB.parentReservePct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Children Max Fill %</label><input id="yro_B_child_max_v24" type="number" min="0" max="100" step="1" value="${st.modeB.childrenMaxFillPct}" style="width:78px;"></div>
    <div class="yro_kv"><label>Surplus Cap %</label><input id="yro_B_scap_v24" type="number" min="0" max="100" step="1" value="${st.modeB.surplusCapPct}" style="width:70px;"></div>
    <div class="yro_kv"><label>Computed Child Fill %</label><input id="yro_B_computed_fill_v24" type="number" value="" style="width:78px;" disabled></div>
    <div class="yro_kv"><label>Child Merch Used</label><input id="yro_B_child_merch_v24" type="number" value="" style="width:70px;" disabled></div>
    <div class="yro_kv"><label>Parent Merch Used</label><input id="yro_B_parent_merch_v24" type="number" value="" style="width:78px;" disabled></div>
    <button class="yro_btn btn-confirm" id="yro_B_plan_v24">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_v24">Execute</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_C_target_v24">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v24">Pick...</button>
    <div class="yro_kv"><label>Surplus</label><select id="yro_C_surplus_v24">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v24">Pick...</button>
    <div class="yro_kv"><label>Reserve%</label><input id="yro_C_reserve_v24" type="number" min="0" max="100" step="1" value="${st.modeC.reservePct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_C_cap_v24" type="number" min="1" max="100" step="1" value="${st.modeC.capPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_C_scap_v24" type="number" min="1" max="100" step="1" value="${st.modeC.surplusCapPct}" style="width:60px;"></div>
    <div class="yro_kv"><label>Tolerance%</label><input id="yro_C_tol_v24" type="number" min="0" max="25" step="1" value="5" style="width:60px;"></div>
    <div class="yro_kv"><label>Iron Delta%</label><input id="yro_C_iron_v24" type="number" min="-50" max="50" step="1" value="${st.modeC.ironDeltaPct == null ? '' : st.modeC.ironDeltaPct}" style="width:70px;"></div>
    <button class="yro_btn btn-confirm" id="yro_C_plan_v24">Plan</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_v24">Execute</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v24">Ã¢Å“â€“</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v24" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v24">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v24">Select none</button>
    <span class="yro_small" id="yro_picker_count_v24"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v24">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v24">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v24">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v24');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v24');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v24').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v24').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v24').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v24').value);
    });

    box.querySelector('#yro_picker_none_v24').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v24').value);
    });

    box.querySelector('#yro_picker_save_v24').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v24').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v24');
    var diagEl = Y.qs('#yro_plan_diag_v24');
    var tTbl = Y.qs('#yro_plan_target_tbl_v24');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v24');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v24');
    var targetTitleEl = Y.qs('#yro_plan_target_title_v24');
    var surplusTitleEl = Y.qs('#yro_plan_surplus_title_v24');
    var shipTitleEl = Y.qs('#yro_plan_ship_title_v24');
    var copyTargetEl = Y.qs('#yro_copy_plan_target_v24');
    var copySurplusEl = Y.qs('#yro_copy_plan_surplus_v24');
    var copyShipEl = Y.qs('#yro_copy_plan_ship_v24');

    function renderEmpty() {
      if (metaEl) metaEl.innerHTML = '<b>Mode:</b> - | <b>Computed Fill %:</b> - | <b>Children Max Fill %:</b> - | <b>Parent Reserve %:</b> - | <b>Surplus Cap %:</b> - | <b>Shipments:</b> 0 | <b>Warnings:</b> -';
    ...
    if (String(plan.mode || '').toLowerCase() === 'push' && plan.childSummary && plan.parentSummary) {
      ...
      var computedFillEl = Y.qs('#yro_B_computed_fill_v24');
      var childMerchEl = Y.qs('#yro_B_child_merch_v24');
      var parentMerchEl = Y.qs('#yro_B_parent_merch_v24');
...
<tr>
  ...
  <td class="yro_right">${Y.formatTwNumber(r.after.iron)}</td>
      ...
      var computedFillEl2 = Y.qs('#yro_B_computed_fill_v24');
      var childMerchEl2 = Y.qs('#yro_B_child_merch_v24');
      var parentMerchEl2 = Y.qs('#yro_B_parent_merch_v24');
...
(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v24');
    ...
    var t2title = Y.qs('#yro_t2_title_v24');
  ...
  function confirmTwoButtons(htmlMsg, onYes, onCancel) {
    ...
    if (window.UI && UI.ConfirmationBox) {
      UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v24', true);
    } else {
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v24');
      surplusGid = getSelValue('#yro_A_surplus_v24');
      var capA = getNumValue('#yro_A_cap_v24', st.modeA.capPct || 80);
      var scapA = getNumValue('#yro_A_scap_v24', st.modeA.surplusCapPct || 95);
      var ironDeltaA = getNumValue('#yro_A_iron_v24', st.modeA.ironDeltaPct || 0);

      targetIds = await resolveSelection(targetGid, 'A_target', true);
      surplusIds = await resolveSelection(surplusGid, 'A_surplus', true);

      var rA = Y.compute.planBalance(targetIds, surplusIds, Y.runtime.snapshotsById, st.cache.incomingMap, capA, scapA, ironDeltaA);
      Y.runtime.plan = {
        mode: mode,
        shipments: rA.shipments,
        meta: {
          cap: capA,
          scap: scapA,
          reserve: 0,
          ironDelta: ironDeltaA,
          targetGid: targetGid,
          surplusGid: surplusGid
        },
        targetIds: rA.targetIds,
        surplusIds: rA.surplusIds,
        states: rA.states,
        targetSummary: Y.compute.summarize(rA.states, rA.targetIds),
        surplusSummary: Y.compute.summarize(rA.states, rA.surplusIds)
      };
      ...
      st.modeA.ironDeltaPct = ironDeltaA;
    } else if (mode === 'push') {
      var parentGid = getSelValue('#yro_B_parents_v24');
      var childGid = getSelValue('#yro_B_children_v24');
      surplusGid = getSelValue('#yro_B_surplus_v24');
      var parentReservePct = getNumValue('#yro_B_parent_reserve_v24', st.modeB.parentReservePct || 1);
      var childrenMaxFillPct = getNumValue('#yro_B_child_max_v24', st.modeB.childrenMaxFillPct || 80);
      var surplusCapPct = getNumValue('#yro_B_scap_v24', st.modeB.surplusCapPct || 95);
      ...
      targetGid = getSelValue('#yro_C_target_v24');
      surplusGid = getSelValue('#yro_C_surplus_v24');
      var reserveC = getNumValue('#yro_C_reserve_v24', st.modeC.reservePct || 1);
      var capC = getNumValue('#yro_C_cap_v24', st.modeC.capPct || 80);
      var scapC = getNumValue('#yro_C_scap_v24', st.modeC.surplusCapPct || 95);
      var ironDeltaC = getNumValue('#yro_C_iron_v24', st.modeC.ironDeltaPct || 0);
  ...
  function buildShipmentsTableBBCode(plan) {
    ...
    var lines = isPush ? buildPushMetaLines(plan, 'SHIPMENTS') : buildGenericMetaLines(plan, 'SHIPMENTS');

    if (isPush && plan.meta && plan.meta.diagnostics && Array.isArray(plan.meta.diagnostics.lines) && plan.meta.diagnostics.lines.length) {
  ...
  function copyPlanSurplusBBCode() {
    ...
    flashButton('#yro_copy_plan_surplus_v24', 'Copy SURPLUS');
  ...
  function copyPlanShipmentsBBCode() {
    ...
    flashButton('#yro_copy_plan_ship_v24', 'Copy SHIPMENTS');
  ...
  function copyBBCode() {
    ...
    if (String(plan.mode || '').toLowerCase() === 'push' && plan.meta && plan.meta.diagnostics && Array.isArray(plan.meta.diagnostics.lines) && plan.meta.diagnostics.lines.length) {
      lines.push('[b]Diagnostics[/b]');
      lines.push('[list]');
      plan.meta.diagnostics.lines.forEach(function (line) {
        lines.push('[*]' + bbEscape(line));
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v24'), 'click', function () { Y.destroy(); });
    Y.on(Y.qs('#yro_load_v24'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v24');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v24'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v24');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v24');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v24'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v24');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v24');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v24'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v24'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v24'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v24'), 'click', function () { openPicker('t2'); });
    Y.on(Y.qs('#yro_copy_bb_v24'), 'click', function () { copyBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_target_v24'), 'click', function () { copyPlanTargetBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_surplus_v24'), 'click', function () { copyPlanSurplusBBCode(); });
    Y.on(Y.qs('#yro_copy_plan_ship_v24'), 'click', function () { copyPlanShipmentsBBCode(); });

    var modeBox = Y.qs('#yro_modes_v24');
    if (!modeBox) return;
    ...
    Y.on(modeBox, 'click', function (e) {
      ...
      if (t.id === 'yro_A_plan_v24') return planFromMode('balance');
      if (t.id === 'yro_B_plan_v24') return planFromMode('push');
      if (t.id === 'yro_C_plan_v24') return planFromMode('funnel');

      if (t.id === 'yro_A_exec_v24' || t.id === 'yro_B_exec_v24' || t.id === 'yro_C_exec_v24') return executePlan();

      if (String(t.id || '').startsWith('pick_') && String(t.id || '').endsWith('_v24')) {
        var key = String(t.id).replace('pick_', '').replace('_v24', '');
      ...
      if (row && row.getAttribute) {
        ...
        if (mode) {
          st.orchestrator.mode = mode;
          Y.saveState();
          Y.qsa('#yro_modes_v24 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
    ...
    Y.on(modeBox, 'change', function (e) {
      ...
      if (t.id === 'yro_A_target_v24') st.groups.A_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_surplus_v24') st.groups.A_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parents_v24') st.groups.B_parents = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_children_v24') st.groups.B_children = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_surplus_v24') st.groups.B_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_target_v24') st.groups.C_target = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_C_surplus_v24') st.groups.C_surplus = Y.safeInt(t.value, 0);
      else if (t.id === 'yro_A_cap_v24') st.modeA.capPct = Y.safeInt(t.value, st.modeA.capPct);
      else if (t.id === 'yro_A_scap_v24') st.modeA.surplusCapPct = Y.safeInt(t.value, st.modeA.surplusCapPct);
      else if (t.id === 'yro_A_iron_v24') st.modeA.ironDeltaPct = String(t.value).trim() === '' ? null : Y.safeInt(t.value, 0);
      else if (t.id === 'yro_B_parent_reserve_v24') st.modeB.parentReservePct = Y.safeInt(t.value, st.modeB.parentReservePct);
      else if (t.id === 'yro_B_child_max_v24') st.modeB.childrenMaxFillPct = Y.safeInt(t.value, st.modeB.childrenMaxFillPct);
      else if (t.id === 'yro_B_scap_v24') st.modeB.surplusCapPct = Y.safeInt(t.value, st.modeB.surplusCapPct);
      else if (t.id === 'yro_C_reserve_v24') st.modeC.reservePct = Y.safeInt(t.value, st.modeC.reservePct);
      else if (t.id === 'yro_C_cap_v24') st.modeC.capPct = Y.safeInt(t.value, st.modeC.capPct);
      else if (t.id === 'yro_C_scap_v24') st.modeC.surplusCapPct = Y.safeInt(t.value, st.modeC.surplusCapPct);
      else if (t.id === 'yro_C_iron_v24') st.modeC.ironDeltaPct = String(t.value).trim() === '' ? null : Y.safeInt(t.value, 0);
