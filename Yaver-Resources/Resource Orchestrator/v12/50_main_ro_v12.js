(function () {
  ...
  async function loadTables(forceGroupFetch) {
    ...
    var t1title = Y.qs('#yro_t1_title_v12');
    ...
    var t2title = Y.qs('#yro_t2_title_v12');
  ...
  function confirmTwoButtons(htmlMsg, onYes) {
    ...
    if (window.UI && UI.ConfirmationBox) UI.ConfirmationBox(htmlMsg, actions, 'yro_exec_v12', true);
  ...
  async function planFromMode(mode) {
    ...
    if (mode === 'balance') {
      targetGid = getSelValue('#yro_A_target_v12');
      surplusGid = getSelValue('#yro_A_surplus_v12');
      cap = getNumValue('#yro_A_cap_v12', 80);
      scap = getNumValue('#yro_A_scap_v12', 95);
      reserve = 0;
      ...
      senderGid = getSelValue('#yro_B_sender_v12');
      targetGid = getSelValue('#yro_B_target_v12');
      surplusGid = getSelValue('#yro_B_surplus_v12');
      reserve = getNumValue('#yro_B_reserve_v12', 1);
      cap = getNumValue('#yro_B_cap_v12', 80);
      scap = getNumValue('#yro_B_scap_v12', 95);
      ...
      targetGid = getSelValue('#yro_C_target_v12');
      surplusGid = getSelValue('#yro_C_surplus_v12');
      reserve = getNumValue('#yro_C_reserve_v12', 1);
      cap = getNumValue('#yro_C_cap_v12', 80);
      scap = getNumValue('#yro_C_scap_v12', 95);
  ...
  function bindEvents() {
    ...
    Y.on(Y.qs('#yro_close_v12'), 'click', function () { Y.destroy(); });

    Y.on(Y.qs('#yro_load_v12'), 'click', function () { loadAll(true); });

    var search = Y.qs('#yro_search_v12');
    if (search) {
    ...
    Y.on(Y.qs('#yro_t1_toggle_v12'), 'click', function () {
      st.ui.minimized1 = !st.ui.minimized1;
      var w = Y.qs('#yro_t1_wrap_v12');
      if (w) w.style.display = st.ui.minimized1 ? 'none' : '';
      var b = Y.qs('#yro_t1_toggle_v12');
      if (b) b.textContent = st.ui.minimized1 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t2_toggle_v12'), 'click', function () {
      st.ui.minimized2 = !st.ui.minimized2;
      var w = Y.qs('#yro_t2_wrap_v12');
      if (w) w.style.display = st.ui.minimized2 ? 'none' : '';
      var b = Y.qs('#yro_t2_toggle_v12');
      if (b) b.textContent = st.ui.minimized2 ? 'Expand' : 'Minimize';
      Y.saveState();
    });

    Y.on(Y.qs('#yro_t1_group_sel_v12'), 'change', function (e) {
      st.groups.sel1 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t2_group_sel_v12'), 'change', function (e) {
      st.groups.sel2 = Y.safeInt(e.target.value, 0);
      Y.saveState();
      loadTables(true);
    });

    Y.on(Y.qs('#yro_t1_pick_v12'), 'click', function () { openPicker('t1'); });
    Y.on(Y.qs('#yro_t2_pick_v12'), 'click', function () { openPicker('t2'); });

    Y.on(Y.qs('#yro_copy_bb_v12'), 'click', function () { copyBBCode(); });

    var modeBox = Y.qs('#yro_modes_v12');
    if (modeBox) {
      // click delegation
      Y.on(modeBox, 'click', function (e) {
        ...
        if (t.id === 'yro_A_plan_v12') return planFromMode('balance');
        if (t.id === 'yro_B_plan_v12') return planFromMode('push');
        if (t.id === 'yro_C_plan_v12') return planFromMode('funnel');

        if (t.id === 'yro_A_exec_tabs_v12' || t.id === 'yro_B_exec_tabs_v12' || t.id === 'yro_C_exec_tabs_v12') return executeTabs();
        if (t.id === 'yro_A_exec_auto_v12' || t.id === 'yro_B_exec_auto_v12' || t.id === 'yro_C_exec_auto_v12') return executeAuto();

        if (String(t.id || '').startsWith('pick_') && String(t.id).endsWith('_v12')) {
          var key = String(t.id).replace('pick_', '').replace('_v12', '');
        ...
        if (row && row.getAttribute) {
          ...
          if (mode) {
            st.orchestrator.mode = mode;
            Y.saveState();
            Y.qsa('#yro_modes_v12 .yro_mode_row').forEach(function (r) { r.classList.remove('active'); });
      ...
      Y.on(modeBox, 'change', function (e) {
        ...
        if (t.id === 'yro_A_target_v12') st.groups.A_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_A_surplus_v12') st.groups.A_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_sender_v12') st.groups.B_sender = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_target_v12') st.groups.B_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_B_surplus_v12') st.groups.B_surplus = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_target_v12') st.groups.C_target = Y.safeInt(t.value, 0);
        else if (t.id === 'yro_C_surplus_v12') st.groups.C_surplus = Y.safeInt(t.value, 0);

        else if (t.id === 'yro_A_cap_v12' || t.id === 'yro_B_cap_v12' || t.id === 'yro_C_cap_v12') st.orchestrator.cap = Y.safeInt(t.value, st.orchestrator.cap);
        else if (t.id === 'yro_A_scap_v12' || t.id === 'yro_B_scap_v12' || t.id === 'yro_C_scap_v12') st.orchestrator.scap = Y.safeInt(t.value, st.orchestrator.scap);
        else if (t.id === 'yro_B_reserve_v12' || t.id === 'yro_C_reserve_v12') st.orchestrator.reserve = Y.safeInt(t.value, st.orchestrator.reserve);
