...
/* Content scroll area */
#${Y.cfg.PANEL_ID} #yro_content_v12{
  padding:8px;
  flex:1;
  overflow-y:auto;
  overflow-x:auto;
...
#${Y.cfg.PICKER_OVERLAY_ID} .yro_modal_foot{
  ...
  function buildPanel() {
    ...
    root.innerHTML = `
<div class="yro_hdr" id="yro_drag_v12" style="cursor:move;">
  <div class="yro_ttl"><span class="icon header wood"></span> Resource Orchestrator <span class="yro_small">(v12)</span></div>
  <div style="display:flex; align-items:center;">
    <span class="yro_dev">Developed by controleng</span>
    <button class="yro_btn" id="yro_close_v12">✖</button>
  </div>
</div>

<div class="yro_toolbar">
  <button class="yro_btn btn-confirm" id="yro_load_v12">Yükle / Yenile (Full Scan)</button>
  <div class="yro_progress_outer"><div class="yro_progress_inner" id="yro_prog_bar_v12"></div></div>
  <span id="yro_prog_txt_v12" class="yro_small">0/0 - Ready</span>

  <span class="yro_small"><b>Search:</b></span>
  <input type="text" id="yro_search_v12" placeholder="Köy adı…" style="width:180px;">

  <button class="yro_btn btn-confirm" id="yro_copy_bb_v12">Copy BBCode</button>
</div>

<div class="yro_msg" id="yro_msg_v12" style="color:#0a6;">Hazır ✅</div>

<div id="yro_content_v12">
  <div class="yro_sec_head">
    <div>1) Villages — Production (per hour + 24h) (<span id="yro_t1_title_v12">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t1_toggle_v12">${st.ui.minimized1 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t1_group_sel_v12"></select>
      <button class="yro_mini" id="yro_t1_pick_v12">Pick…</button>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t1_wrap_v12" style="${st.ui.minimized1 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t1_tbl_v12"></table>
  </div>

  <div class="yro_sec_head">
    <div>2) Villages — Current + Incoming + Outgoing (per village) (<span id="yro_t2_title_v12">All villages</span>)</div>
    <div class="yro_sec_controls">
      <button class="yro_mini" id="yro_t2_toggle_v12">${st.ui.minimized2 ? 'Expand' : 'Minimize'}</button>
      <span class="yro_small"><b>Group:</b></span>
      <select id="yro_t2_group_sel_v12"></select>
      <button class="yro_mini" id="yro_t2_pick_v12">Pick…</button>
      <span class="yro_small" id="yro_scan_ts_v12"></span>
    </div>
  </div>
  <div class="yro_scroll" id="yro_t2_wrap_v12" style="${st.ui.minimized2 ? 'display:none;' : ''}">
    <table class="yro_table" id="yro_t2_tbl_v12"></table>
  </div>
  ...
  <div class="yro_mode_box" id="yro_modes_v12"></div>

  <div class="yro_msg" id="yro_plan_meta_v12" style="margin-top:8px;">
    <b>Mode:</b> - | <b>Cap%:</b> - | <b>Reserve(each):</b> - | <b>Surplus Cap%:</b> - | <b>Shipments:</b> 0
  </div>

  <div class="yro_scroll">
    <table class="yro_table" id="yro_plan_target_tbl_v12"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_surplus_tbl_v12"></table>
  </div>
  <div class="yro_scroll" style="margin-top:8px;">
    <table class="yro_table" id="yro_plan_ship_tbl_v12"></table>
  </div>
</div>
    `;
  ...
  function setProgress(step, total, text) {
    var bar = Y.qs('#yro_prog_bar_v12');
    var tx = Y.qs('#yro_prog_txt_v12');
    if (!bar || !tx) return;
  ...
  function fillGroupSelects(groups) {
    ...
    fill('#yro_t1_group_sel_v12', st.groups.sel1);
    fill('#yro_t2_group_sel_v12', st.groups.sel2);
  }
...
<div class="yro_mode_row ${st.orchestrator.mode === 'balance' ? 'active' : ''}" data-mode="balance">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_A_target_v12">${optionsHTML(st.groups.A_target)}</select></div>
    <button class="yro_mini" id="pick_A_target_v12">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_A_surplus_v12">${optionsHTML(st.groups.A_surplus)}</select></div>
    <button class="yro_mini" id="pick_A_surplus_v12">Pick…</button>

    <div class="yro_kv"><label>Cap%</label><input id="yro_A_cap_v12" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_A_scap_v12" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <button class="yro_btn btn-confirm" id="yro_A_plan_v12">Plan</button>
    <button class="yro_btn btn-warn" id="yro_A_exec_tabs_v12">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_A_exec_auto_v12">Execute (Auto)</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'push' ? 'active' : ''}" data-mode="push">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Sender</label><select id="yro_B_sender_v12">${optionsHTML(st.groups.B_sender)}</select></div>
    <button class="yro_mini" id="pick_B_sender_v12">Pick…</button>

    <div class="yro_kv"><label>Target</label><select id="yro_B_target_v12">${optionsHTML(st.groups.B_target)}</select></div>
    <button class="yro_mini" id="pick_B_target_v12">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_B_surplus_v12">${optionsHTML(st.groups.B_surplus)}</select></div>
    <button class="yro_mini" id="pick_B_surplus_v12">Pick…</button>

    <div class="yro_kv"><label>Reserve%</label><input id="yro_B_reserve_v12" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_B_cap_v12" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_B_scap_v12" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <button class="yro_btn btn-confirm" id="yro_B_plan_v12">Plan</button>
    <button class="yro_btn btn-warn" id="yro_B_exec_tabs_v12">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_B_exec_auto_v12">Execute (Auto)</button>
  </div>
</div>
...
<div class="yro_mode_row ${st.orchestrator.mode === 'funnel' ? 'active' : ''}" data-mode="funnel">
  ...
  <div class="yro_mode_controls">
    <div class="yro_kv"><label>Target</label><select id="yro_C_target_v12">${optionsHTML(st.groups.C_target)}</select></div>
    <button class="yro_mini" id="pick_C_target_v12">Pick…</button>

    <div class="yro_kv"><label>Surplus</label><select id="yro_C_surplus_v12">${optionsHTML(st.groups.C_surplus)}</select></div>
    <button class="yro_mini" id="pick_C_surplus_v12">Pick…</button>

    <div class="yro_kv"><label>Reserve%</label><input id="yro_C_reserve_v12" type="number" min="0" max="100" step="1" value="${st.orchestrator.reserve}" style="width:60px;"></div>
    <div class="yro_kv"><label>Cap%</label><input id="yro_C_cap_v12" type="number" min="1" max="100" step="1" value="${st.orchestrator.cap}" style="width:60px;"></div>
    <div class="yro_kv"><label>Surplus Cap%</label><input id="yro_C_scap_v12" type="number" min="1" max="100" step="1" value="${st.orchestrator.scap}" style="width:60px;"></div>

    <button class="yro_btn btn-confirm" id="yro_C_plan_v12">Plan</button>
    <button class="yro_btn btn-warn" id="yro_C_exec_tabs_v12">Execute (Tabs)</button>
    <button class="yro_btn btn-confirm" id="yro_C_exec_auto_v12">Execute (Auto)</button>
  </div>
</div>
    `;
  }
...
<div class="yro_modal_head">
  ...
  <button class="yro_btn" id="yro_picker_close_v12">✖</button>
</div>
<div class="yro_modal_body">
  <div class="yro_modal_toolbar">
    <span class="yro_small"><b>Search:</b></span>
    <input type="text" id="yro_picker_search_v12" style="width:220px;" placeholder="name / coord">
    <button class="yro_btn" id="yro_picker_all_v12">Select all</button>
    <button class="yro_btn" id="yro_picker_none_v12">Select none</button>
    <span class="yro_small" id="yro_picker_count_v12"></span>
  </div>
  <table class="yro_modal_list" id="yro_picker_tbl_v12">
    <tr><th></th><th>Village</th><th>Coord</th><th class="yro_right">Storage</th></tr>
  </table>
</div>
<div class="yro_modal_foot">
  <button class="yro_btn" id="yro_picker_cancel_v12">Cancel</button>
  <button class="yro_btn btn-confirm" id="yro_picker_save_v12">Kaydet</button>
</div>
    `;
    ...
    function updateCount() {
      var el = box.querySelector('#yro_picker_count_v12');
      if (el) el.textContent = 'Selected: ' + selected.size;
    }

    function renderList(filterText) {
      filterText = String(filterText || '').toLowerCase();
      var tbl = box.querySelector('#yro_picker_tbl_v12');
      if (!tbl) return;
...
<td class="yro_right">${Y.formatTwNumber(storage)}</td>
    ...
    box.querySelector('#yro_picker_close_v12').addEventListener('click', close);
    box.querySelector('#yro_picker_cancel_v12').addEventListener('click', close);

    box.querySelector('#yro_picker_all_v12').addEventListener('click', function () {
      candidates.forEach(function (v) { selected.add(String(v.id)); });
      renderList(box.querySelector('#yro_picker_search_v12').value);
    });

    box.querySelector('#yro_picker_none_v12').addEventListener('click', function () {
      selected.clear();
      renderList(box.querySelector('#yro_picker_search_v12').value);
    });

    box.querySelector('#yro_picker_save_v12').addEventListener('click', function () {
      var ids = Array.from(selected).map(function (s) { return Y.safeInt(s, 0); }).filter(Boolean);
      try { onSaveCallback(ids); } catch (e) {}
      close();
    });

    box.querySelector('#yro_picker_search_v12').addEventListener('input', Y.debounce(function (e) {
...
<tr class="yro_total_row">
  ...
  function renderPlanTables(plan, snapshotsById) {
    var metaEl = Y.qs('#yro_plan_meta_v12');
    if (metaEl) {
      metaEl.innerHTML =
    ...
    var tTbl = Y.qs('#yro_plan_target_tbl_v12');
    var sTbl = Y.qs('#yro_plan_surplus_tbl_v12');
    var shTbl = Y.qs('#yro_plan_ship_tbl_v12');
