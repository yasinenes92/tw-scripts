(function () {
  ...
  var KEY = "__YAVER_RES_TROOPS_V1__";
  ...
  Y.cfg = {
    // unit classification (v1 default)
    DEF_UNITS: ["spear", "sword", "archer", "heavy"],
    OFF_UNITS: ["axe", "light", "marcher", "ram", "catapult"],
    OTHER_UNITS: ["spy", "knight", "snob", "militia"],

    // safe list (if world lacks some units, we just skip)
    ALL_UNITS: ["spear", "sword", "axe", "archer", "spy", "light", "marcher", "heavy", "ram", "catapult", "knight", "snob", "militia"],

    // per unit fallback pop (if parsing fails)
    POP_FALLBACK: {
  ...
  Y.state = {
    villages: [],              // {id,name}
    perVillage: new Map(),     // id -> { units:{u:count}, prodDay:{u:n}, popTotal:int, unitMeta:{u:{pop,build_time,requirements_met}} }
    totals: null,
    isLoading: false,
  ...
  Y.destroy = function () {
    ...
    try { $("#yaver_res_troops_panel_v1").remove(); } catch (e2) {}
    try { $("#yaver_res_troops_style_v1").remove(); } catch (e3) {}
  };
