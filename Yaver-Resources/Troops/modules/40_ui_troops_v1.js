(function () {
  ...
  var PANEL_ID = "yaver_res_troops_panel_v1";
  var STYLE_ID = "yaver_res_troops_style_v1";
  ...
  function ensureStyle() {
    ...
    st.textContent = `
      ...
      #${PANEL_ID} .scroll{ max-height: 320px; overflow:auto; overflow-x:auto; -webkit-overflow-scrolling:touch; touch-action: pan-x pan-y; border:1px solid rgba(0,0,0,.15); }
      #${PANEL_ID} .units_cell{ display:flex; flex-wrap:wrap; gap:10px; align-items:flex-start; }
      #${PANEL_ID} .unit_box{
        background:#fff5da;
          ...
          <th style="text-align:left;">
            <div class="hdr">
              <div>
                ...
                Yaver Resources: Troops <span class="grey">(v1)</span>
              </div>
              <div class="hdr_right">
              ...
              <table class="vis">
                <thead>
                  <tr>
                    <th>Village</th>
                    <th>Troops</th>
                    <th class="right"><span class="icon header population"></span> Pop (total)</th>
                  </tr>
                </thead>
      ...
      <table class="vis">
        ...
        <tr>
          <td>
            ...
            <div class="small">Not: Defense seti: spear, sword, archer, heavy. Spy/knight/snob/militia “Other” sayılır.</div>
          </td>
        </tr>
      </table>
  ...
  function unitBoxHTML(u, count, perDay) {
    ...
    // Count 0 ise hiç göstermiyoruz (senin şartın).
    if (!(cnt > 0)) return "";

    var html = '<div class="unit_box">' +
  ...
  Y.ui.renderPerVillage = function () {
    ...
    arr.forEach(function (pv) {
      ...
      // unit boxes
      var boxes = [];
      // show only units that exist in that village (count>0)
      ...
      tr.innerHTML =
        "<td><b>" + pv.name + "</b></td>" +
        '<td><div class="units_cell">' + (boxes.join("") || "<span class='small'>-</span>") + "</div></td>" +
        '<td class="right"><b>' + Y.util.n(pv.popTotal || 0) + "</b></td>";
