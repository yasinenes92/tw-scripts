(function () {
  ...
  Y.data.loadVillageList = async function () {
    ...
    if (!curVillage) throw new Error("game_data.village.id not found.");

    var url = buildUrl(curVillage, "am_troops");
    var r = await fetchText(url);
    if (!r.ok) throw new Error("Failed to fetch am_troops: " + r.status);

    var villages = parseVillageListFromAmTroops(r.text);
    if (!villages.length) throw new Error("Could not extract village list from am_troops.");
