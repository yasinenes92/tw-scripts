(function () {
  ...
  var FILES = [
    "00_bootstrap_troops_v3.js",
    "10_data_troops_v2.js",
    "20_fetch_recruit_troops_v2.js",
    "30_compute_troops_v2.js",
    "40_ui_troops_v7.js",
    "50_main_troops_v2.js"
  ];
  ...
  function loadOne(f) {
    var url = bust(BASE + f);
    console.log("[YRT Troops Loader v7] loading:", url);
    return new Promise(function (res, rej) {
      $.ajax({ url: url, dataType: "script", cache: false, timeout: 30000 })
        .done(function () { console.log("[YRT Troops Loader v7] OK:", f); res(true); })
        .fail(function (jq, st, er) {
          console.log("[YRT Troops Loader v7] FAIL", { file: f, status: st, err: er, http: jq && jq.status, url: url });
          console.log("[YRT Troops Loader v7] head:", ((jq && jq.responseText) || "").slice(0, 200));
  ...
  (async function () {
    try {
      ...
      if (window.__YAVER_RES_TROOPS_V1__ && typeof window.__YAVER_RES_TROOPS_V1__.init === "function") {
        ...
        (window.UI && UI.SuccessMessage) && UI.SuccessMessage("Yaver Resources: Troops Ready ✅ (v7)", 2000);
      } else {
        (window.UI && UI.ErrorMessage) && UI.ErrorMessage("Troops loaded but init not found (check Console).", 5000);
      }
    } catch (e) {
      console.error("[YRT Troops Loader v7] ERROR:", e);
