(function () {
  ...
  function loadOne(f) {
    var url = bust(BASE + f);
    console.log("[YRT Troops Loader v2] loading:", url);
    return new Promise(function (res, rej) {
      $.ajax({ url: url, dataType: "script", cache: false, timeout: 30000 })
        .done(function () { console.log("[YRT Troops Loader v2] OK:", f); res(true); })
        .fail(function (jq, st, er) {
          console.log("[YRT Troops Loader v2] FAIL", { file: f, status: st, err: er, http: jq && jq.status, url: url });
          console.log("[YRT Troops Loader v2] head:", ((jq && jq.responseText) || "").slice(0, 200));
  ...
  (async function () {
    try {
      ...
      if (window.__YAVER_RES_TROOPS_V1__ && typeof window.__YAVER_RES_TROOPS_V1__.init === "function") {
        ...
        (window.UI && UI.SuccessMessage) && UI.SuccessMessage("Yaver Resources: Troops Hazır ✅ (v2)", 2000);
      } else {
        (window.UI && UI.ErrorMessage) && UI.ErrorMessage("Troops yüklendi ama init bulunamadı (Console'a bak).", 5000);
      }
    } catch (e) {
      console.error("[YRT Troops Loader v2] ERROR:", e);
