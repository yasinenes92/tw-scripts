(function () {
  ...
  function sumGroup(setIds) {
    ...
    setIds.forEach(function (vid) {
      ...
      out.incWood += e.inc.wood; out.incStone += e.inc.stone; out.incIron += e.inc.iron;
  ...
  function marketAdvice(totalWood, totalStone, totalIron) {
    ...
    function mkMoves() {
      ...
      // greedy: stone -> wood/iron, iron -> wood/stone, wood -> stone/iron
      move("stone", "wood"); move("stone", "iron");
      move("iron", "wood"); move("iron", "stone");
      move("wood", "stone"); move("wood", "iron");

      return { target: target, moves: moves, leftoverNeed: need, leftoverExcess: exc };
    }
  ...
  function buildTransferPlan() {
    ...
    var donors = parents.map(function (p) {
      ...
      // donor goods limit (merchant availability)
      var supSum = sup.wood + sup.stone + sup.iron;
      if (supSum > capGoods && capGoods > 0) {
        // scale down (merchant bottleneck) - simple proportional cut
    ...
    // helper: pick donor with max supply of resource and remaining cap
    ...
    // priority: <=2000 first (as user requested)
    ...
    // sort within: lower points first
    ...
    function minResourceKey(c) {
      // çocuk köy içinde en az olan kaynağa öncelik (dengeye yaklaş)
    ...
    function allocate(list) {
      // greedy: her turda "en düşük kaynağı olan" köyün en düşük kaynağını 1000’lik bloklarla besle
      while (anySupply() && anyNeed(list)) {
        // en düşük min(resource) olanı bul
        var bestC = null;
        var bestMin = null;
        ...
        var chunk = 1000; // merchant packing için
    ...
    var childrenRecv = { wood: 0, stone: 0, iron: 0, total: 0 };
    recvs.forEach(function (c) {
      childrenRecv.wood += c.recv.wood;
      childrenRecv.stone += c.recv.stone;
      childrenRecv.iron += c.recv.iron;
    });
    childrenRecv.total = childrenRecv.wood + childrenRecv.stone + childrenRecv.iron;

    // merchant feasibility summary (per donor)
    donors.forEach(function (d) {
    ...
    return {
      params: {
        ...
        capPct: capPct,
        lowPts: Y.cfg.LOW_POINTS_THRESHOLD
      },
      donors: donors,
      recvs: recvs,
      totals: {
        parentsSent: parentsSent,
  ...
  Y.compute.run = function () {
    ...
    Y.state.computed = {
      ...
      avgs: {
        parents: parentsAvg,
        children: childrenAvg,
        all: allAvg
      },
      advice: {
        all: adviceAll,
        parents: adviceParents
      },
      plan: plan
    };
