(function () {
  ...
  var FILES = [
    ...
    "20_fetch_resources_v1.js",
    "30_compute_resources_v1.js",
    "40_ui_resources_v1.js",
    "50_main_resources_v1.js"
  ];
  ...
  function loadOne(f) {
    var url = bust(BASE + f);
    console.log("[YRA Loader v1] loading:", url);
    return new Promise(function (res, rej) {
      $.ajax({
        ...
        .done(function () {
          console.log("[YRA Loader v1] OK:", f);
          res(true);
        })
        .fail(function (jq, st, er) {
          console.log("[YRA Loader v1] FAIL:", f, { st: st, er: er, http: jq && jq.status });
  ...
  (async function () {
    try {
      ...
      if (window[KEY] && typeof window[KEY].init === "function") {
        window[KEY].init();
        if (window.UI && UI.SuccessMessage) UI.SuccessMessage("Yaver Resources Analyzer v1 Hazır ✅", 2000);
      } else {
        alert("YRA init bulunamadı. Modüller yüklenmiş ama bootstrap tamamlanmamış olabilir.");
      }
    } catch (e) {
      console.error("[YRA Loader v1] ERROR:", e);
      if (window.UI && UI.ErrorMessage) UI.ErrorMessage("Yaver Resources Analyzer yüklenemedi ❌ (Console'a bak)", 6000);
