...
window.FarmGod.Library = (function () {
  ...
  const setUnitSpeeds = function () {
    let unitSpeeds = {};
    // IMPORTANT: return a promise so callers can wait for speeds to be available
  ...
  const ensureUnitSpeeds = function () {
    let cached = getUnitSpeeds();
    return cached ? $.Deferred().resolve(cached).promise() : setUnitSpeeds();
  };
  ...
  const timestampFromString = function (timestr) {
    ...
    const t = timeMatch
      ? [timeMatch[1], timeMatch[2], timeMatch[3], timeMatch[4] || 0].map(
          (x) => +x
        )
      : null;
    ...
    let todayPattern = null,
      ...
      laterDatePattern = null;

    // Try translated patterns first (original behaviour)
    try {
    ...
    if (todayPattern !== null) {
      const tt = todayPattern[1].split(':');
      date = new Date(d[2], d[1] - 1, d[0], +tt[0], +tt[1], +tt[2], +tt[3] || 0);
    } else if (tomorrowPattern !== null) {
      const tt = tomorrowPattern[1].split(':');
      date = new Date(
        d[2],
        d[1] - 1,
        d[0] + 1,
      ...
      const tt = laterDatePattern[2].split(':');
      date = new Date(d[2], d[1] - 1, d[0], +tt[0], +tt[1], +tt[2], +tt[3] || 0);
    } else {
      // Fallback for servers/languages where the strings don't match the expected patterns
      // Supports formats like: "idag kl 01:23:45", "imorgon kl 01:23:45", "08.02. 01:23:45"
      const lower = timestr.toLowerCase();

      // Try to extract an explicit date (dd.mm[.yyyy] or dd/mm[/yyyy])
      const dm = timestr.match(
        /(\d{1,2})[\.\/-](\d{1,2})(?:[\.\/-](\d{2,4}))?/
      );
      if (dm && t) {
        let dd = +dm[1],
          ...
          yy = dm[3] ? +dm[3] : d[2];
        if (yy < 100) yy += 2000;
        date = new Date(yy, mm - 1, dd, t[0], t[1], t[2], t[3] || 0);
      } else if (t) {
        let dayOffset = 0;

        // Common "today / tomorrow / yesterday" keywords across languages
        if (/(\bidag\b|\btoday\b|\bheute\b|\bbug[uü]n\b)/i.test(lower)) {
          dayOffset = 0;
        } else if (
          /(\bimorgon\b|\btomorrow\b|\bmorgen\b|\byar[iı]n\b)/i.test(
            lower
          )
        ) {
          dayOffset = 1;
        } else if (
          /(\big[åa]r\b|\byesterday\b|\bgestern\b|\bd[uü]n\b)/i.test(
            lower
          )
        ) {
          dayOffset = -1;
        ...
        // As a last resort, don't crash the pipeline.
        date = new Date();
      }
    }
...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const getData = function (group, newbarbs, losses) {
    ...
    return Promise.resolve(lib.ensureUnitSpeeds()).then(() => Promise.all([
      lib.processAllPages(
