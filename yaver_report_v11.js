/* =======================================================================
   Yaver Report v10.0.0  (TribalWars / DS)
   SAME WORKING CORE AS v7 (only small changes requested):
   - "Clan" -> "Tribe" in output table/labels
   - Tribe comparison headers:
       Σ Res Gathered, Σ Res Plundered, Total Res, Score (no abbreviations)
   - Output notes are English (you speak TR with me, but report is EN)
   ======================================================================= */
...
(function () {
  ...
  const CFG = {
    ...
    topAlliesN: 15,
    allyRankingPagesToScan: 8, // scan a few pages to safely cover Top 15
    allyRankingOffsetStep: 25,

    ourTopPlayersN: 25,

    // keep it light (same as v7)
    sleepEvery: 10,
    sleepMs: 180,
    ajaxTimeoutMs: 25000,
  };

  function log(...a) { if (CFG.debug) console.log("[YaverV10]", ...a); }
  function warn(...a) { console.warn("[YaverV10]", ...a); }
  function err(...a) { console.error("[YaverV10]", ...a); }
  ...
  // in_a_day filtered by name (key method)
  ...
  function parseRankingAlly(html) {
    ...
    $t.find("tr").slice(1).each(function () {
      ...
      // total points: largest number on the row usually
      let totalPoints = 0;
      $td.each(function () {
  ...
  // parse in_a_day (name-filtered) score for expected player name
  ...
  // parse player points + sum of village points (villages > 2000)
  ...
  // daily values via name-filter
  const dailyCache = new Map(); // type|name -> {score,date}
  ...
  // ✅ Small change: Tribe wording + headers + English note
  ...
  function showDialog(bb) {
    const id = "yaver_v10_text";
    const html = `
      <div style="max-width:980px;">
        <div style="margin:6px 0 10px 0;">
          <a class="btn" id="yaver_v10_copy">Copy BBCode</a>
        </div>
    ...
    setTimeout(() => {
      ...
      $("#yaver_v10_copy").off("click").on("click", async () => {
        try {
  ...
  async function run() {
    try {
      if (typeof $ === "undefined") throw new Error("jQuery not available ($ is undefined).");

      const myAllyId = getAllyId();
      if (!myAllyId) throw new Error("Tribe (ally_id) not found for your player.");
      ...
      // Our members
      ok("Fetching your tribe member list...");
      const myAllyHtml = await getHtml(urlInfoAlly(myAllyId));
      const myMembers = parseAllyMembers(myAllyHtml);
      if (!myMembers.length) throw new Error("Your tribe member list returned empty (info_ally parse).");

      // Our tribe: per player daily + static
      ok("Your tribe: fetching player daily values (name-filtered)...");
      const myDaily = [];
      let req = 0;
      ...
      // Tribe comparison (Top 15): sum ALL members (same as v7)
      ok("Top 15 tribes: summing member daily values...");
      const tribeCompare = [];
      for (let a = 0; a < topAllies.length; a++) {
        const ally = topAllies[a];
        ok(`Tribe ${a + 1}/${topAllies.length}: [${ally.allyTag}] processing...`);
