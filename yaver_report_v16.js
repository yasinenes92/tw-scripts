/* =======================================================================
   Yaver Report v16.0.0  (TribalWars / DS)

   v16 FIX (critical):
   - Some worlds/pages do NOT expose game_data.player.ally_id.
   - But game_data.player.ally *does* contain the numeric ally id (e.g. "402").
   - v15 was failing early because getMyAllyId() only checked ally_id.

   v16 changes:
   - getMyAllyId(): ally_id -> fallback to ally (numeric) -> robust parsing.
   - Everything else kept identical to v15.

   v15 TAG logic kept:
   - (1) ally properties page (best)
   - (2) info_ally page: "Tag:" row
   - (3) ranking player page: current player row -> tribe cell
   - (4) last resort: game_data.player.ally (may be numeric)
   ======================================================================= */
...
(function () {
  ...
  function log(...a) { if (CFG.debug) console.log("[YaverV16]", ...a); }
  function err(...a) { console.error("[YaverV16]", ...a); }
  ...
  // v16: robust ally id detection
  function getMyAllyId() {
    const p = window.game_data && game_data.player ? game_data.player : null;
    if (!p) return null;

    // Prefer ally_id if present
    if (p.ally_id !== undefined && p.ally_id !== null && String(p.ally_id).trim() !== "") {
      const v = String(p.ally_id).trim();
      // Some worlds may use 0 when no tribe
      return v === "0" ? 0 : v;
    }

    // Fallback: ally often contains numeric ally id as string (e.g. "402")
    if (p.ally !== undefined && p.ally !== null) {
      const v = String(p.ally).trim();
  ...
  async function getMyTribeTag(myAllyId) {
    // 1) BEST: ally properties page (Tag row exists here)
    try {
  ...
  function showDialog(bb) {
    const id = "yaver_v16_text";
    const html = `
      <div style="max-width:980px;">
        <div style="margin:6px 0 10px 0;">
          <a class="btn" id="yaver_v16_copy">Copy BBCode</a>
        </div>
    ...
    setTimeout(() => {
      ...
      $("#yaver_v16_copy").off("click").on("click", async () => {
        try {
  ...
  async function run() {
    try {
      ...
      if (myAllyId === null || myAllyId === undefined || String(myAllyId).trim() === "" || String(myAllyId) === "0") {
        throw new Error("Your tribe (ally_id) could not be detected (ally_id/ally missing).");
      }

      const dateStr = todayDDMMYYYY();
      ok(`Yaver ${CFG.version} started...`);
      log("Detected myAllyId =", myAllyId, "game_data.player =", window.game_data && window.game_data.player);

      ok("Fetching your tribe info...");
      const myAllyHtml = await getHtml(urlInfoAlly(myAllyId));

      // Tribe TAG (e.g. QWERTY)
