  ...
  const processPage = function (url, page, wrapFn) {
    ...
    return twLib
      ...
      .then(
        ...
        (jq, st, er) => {
          try {
            console.error('[FarmGod] request failed', {
              url: url + pageText,
              http: jq && jq.status,
              status: st,
              err: er,
            });
            console.error('[FarmGod] response head:', ((jq && jq.responseText) || '').slice(0, 300));
          } catch (_) {}
          throw new Error('FarmGod request failed: ' + (jq && jq.status) + ' ' + st + ' ' + er);
        }
      );
          ...
          $('.optionButton')
            ...
            .on('click', () => {
              ...
              getData(
                ...
                try { console.error('[FarmGod] getData failed:', err); } catch (_) {}
                if (window.UI && UI.ErrorMessage) {
                  UI.ErrorMessage('FarmGod: data fetch failed. Disable adblock/extensions and check Console (F12).', 8000);
                } else {
                  alert('FarmGod: data fetch failed. Check Console (F12).');
                }
              });
            });
