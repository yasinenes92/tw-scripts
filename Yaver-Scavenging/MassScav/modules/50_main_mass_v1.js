(function () {
  ...
  function currentCfg() {
    ...
    // ensure unitsEnabled & keepHome exist for each sendOrder unit
  ...
  Y.main.loadData = async function () {
    ...
    // always read groups from current DOM
    Y.data.readGroupsFromDOM();
    Y.ui.renderGroups();

    // if no group selected, default to current group param or all
    ...
    // always load current page first (fast)
    ...
    // apply selected groups to selection sources (so they become selected villages)
  ...
  Y.main.buildBatches = function () {
    ...
    if (!reqs.length) {
      Y.ui.msg("Request üretilemedi (OK satır yok).", "err");
      return;
    }
  ...
  Y.main.sendBatchIndex = async function (idx) {
    ...
    try {
      await Y.sender.sendBatch(Y.state.batches[idx]);
      if (cell) cell.innerHTML = "<span class='sendok'>OK</span>";
      Y.ui.msg("✅ Batch " + (idx + 1) + " gönderildi.", "ok");
    } catch (e) {
      console.error(e);
  ...
  Y.init = async function () {
    ...
    // preload group list + current page data quickly
    Y.data.readGroupsFromDOM();
    Y.ui.renderGroups();

    // select current group by default
    var g = Y.util.getUrlParam("group");
    if (g == null) g = "0";
    Y.state.selectedGroups.add(String(g));

    // load current page embedded data (instant)
    var ok = false;
    try { ok = Y.data.loadCurrentPageData(); } catch (e2) {}
    if (ok) {
      Y.data.rebuildVillageArray();
      Y.data.setGroupSelected(String(g), true); // selects those villages
      Y.ui.renderOptions();
      Y.ui.renderUnits();
      Y.ui.renderVillages();
