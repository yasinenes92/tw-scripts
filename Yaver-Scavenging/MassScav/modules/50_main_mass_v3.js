(function () {
  ...
  Y.main.loadData = async function () {
    ...
    Y.ui.msg("Loading data... (selected groups will be scanned)", "info");
    ...
    Y.ui.msg("✅ Data is ready. Check village selection → PLAN → BUILD BATCHES.", "ok");
  };
  ...
  Y.main.plan = function () {
    ...
    if (!sel || sel.size === 0) {
      Y.ui.msg("No village selected. Select a group or tick villages in the list.", "err");
      return;
    }
    ...
    Y.ui.msg("Plan is ready. OK rows will be included in batches. Now BUILD BATCHES.", "ok");
  };

  Y.main.buildBatches = function () {
    var cfg = currentCfg();
    if (!Y.state.planRows || Y.state.planRows.length === 0) {
      Y.ui.msg("Run PLAN first.", "err");
      return;
    }

    var reqs = Y.sender.buildRequests(Y.state.planRows, cfg.usePremiumBoost);
    if (!reqs.length) {
      Y.ui.msg("Could not build requests (no OK rows / payload may be empty).", "err");
      return;
    }
    ...
    Y.ui.renderBatches();
    Y.ui.msg("✅ Batches ready: " + Y.state.batches.length + " batch / " + reqs.length + " request.", "ok");
  };
  ...
  Y.main.sendBatchIndex = async function (idx) {
    if (!Y.state.batches || !Y.state.batches.length) {
      Y.ui.msg("Run BUILD BATCHES first.", "err");
      return;
    }
    ...
    try {
      ...
      if (err === 0) Y.ui.msg("✅ Batch " + (idx + 1) + " sent. (" + summary + ")", "ok");
      else Y.ui.msg("⚠️ Batch " + (idx + 1) + " partial errors. (" + summary + ") - Check the batch table.", "info");

    } catch (e) {
      console.error(e);
      if (cell) cell.innerHTML = "<span class='senderr'>ERR</span> (console)";
      Y.ui.msg("❌ Batch could not be sent (check console).", "err");
    }
  };
  ...
  Y.main.sendNext = async function () {
    ...
    if (Y.state.sendCursor >= Y.state.batches.length) {
      Y.ui.msg("No more batches to send.", "ok");
      return;
    }
  ...
  Y.init = async function () {
    ...
    try {
      ...
      (window.UI && UI.ErrorMessage) ? UI.ErrorMessage("Mass Scav screen not found. Refresh the page and run again.", 5000) : alert("Mass Scav screen not found");
      return;
    }
    ...
    if (ok) {
      ...
      Y.ui.renderUnits();
      Y.ui.renderVillages();
      Y.ui.msg("✅ Current page data loaded. You can select other groups and run LOAD DATA.", "ok");
    } else {
      Y.ui.msg("Initial data could not be parsed. Use LOAD DATA.", "info");
    }
