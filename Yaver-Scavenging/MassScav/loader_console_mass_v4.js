(function () {
  ...
  function loadOne(f) {
    var url = bust(BASE + f);
    console.log("[YaverMassLoader v4] loading:", url);
    return new Promise(function (res, rej) {
      $.ajax({ url: url, dataType: "script", cache: false, timeout: 25000 })
        .done(function () { res(true); })
        .fail(function (jq, st, er) {
          console.log("[YaverMassLoader v4] FAIL", { file: f, status: st, err: er, http: jq && jq.status, url: url });
          console.log("[YaverMassLoader v4] head:", ((jq && jq.responseText) || "").slice(0, 200));
  ...
  (async function () {
    try {
      ...
      (window.UI && UI.SuccessMessage) && UI.SuccessMessage("Yaver Mass Scav v4 Hazır ✅", 2000);
    } catch (e) {
      (window.UI && UI.ErrorMessage) ? UI.ErrorMessage("Yaver Mass Scav v4 yüklenemedi ❌ (Console'a bak)", 6000) : alert("Yüklenemedi");
      console.error("[YaverMassLoader v4] ERROR:", e);
    }
  })();
})();
