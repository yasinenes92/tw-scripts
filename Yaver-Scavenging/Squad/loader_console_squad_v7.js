(function () {
  ...
  function loadOne(file) {
    return new Promise(function (resolve, reject) {
      ...
      s.async = false;
      s.onload = function () {
        console.log("[YaverSquadLoader v7] OK:", file);
        resolve(true);
      };
      s.onerror = function () {
        console.log("[YaverSquadLoader v7] FAIL:", file, s.src);
  ...
  (async function () {
    try {
      ...
      if (window.__YAVER_SQUAD_V1__ && typeof window.__YAVER_SQUAD_V1__.init === "function") {
        ...
        if (window.UI && UI.SuccessMessage) UI.SuccessMessage("Yaver Squad v7 ready ✅", 2000);
      } else {
        console.warn("[YaverSquadLoader v7] init not found after module load.");
        if (window.UI && UI.ErrorMessage) UI.ErrorMessage("Yaver Squad loaded but init is missing (check Console).", 5000);
      }
    } catch (e) {
      console.error("[YaverSquadLoader v7] ERROR:", e);
