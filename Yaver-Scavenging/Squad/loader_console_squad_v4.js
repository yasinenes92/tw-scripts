(function () {
  ...
  function loadOne(f) {
    var url = bust(BASE + f);
    console.log("[YaverSquadLoader v4] loading:", url);
    return new Promise(function (res, rej) {
      $.ajax({ url: url, dataType: "script", cache: false, timeout: 25000 })
        .done(function () { console.log("[YaverSquadLoader v4] OK:", f); res(true); })
        .fail(function (jq, st, er) {
          console.log("[YaverSquadLoader v4] FAIL", { file: f, status: st, err: er, http: jq && jq.status, url: url });
          console.log("[YaverSquadLoader v4] head:", ((jq && jq.responseText) || "").slice(0, 200));
  ...
  (async function () {
    try {
      ...
      if (window.__YAVER_SQUAD_V1__ && typeof window.__YAVER_SQUAD_V1__.init === "function") {
        ...
        (window.UI && UI.SuccessMessage) && UI.SuccessMessage("Yaver Squad v4 hazır ✅", 2000);
      } else {
        (window.UI && UI.ErrorMessage) && UI.ErrorMessage("Yaver Squad yüklendi ama init yok (Console'a bak).", 5000);
      }
    } catch (e) {
      console.error("[YaverSquadLoader v4] ERROR:", e);
