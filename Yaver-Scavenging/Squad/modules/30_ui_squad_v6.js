(function () {
  ...
  var PANEL_ID = "yaver_squad_panel_v6";
  var STYLE_ID = "yaver_squad_style_v6";
  ...
  function ensurePanel() {
    ...
    var html = `
      <div id="${PANEL_ID}">
        <h3 class="ys_hdr">🛡️ Yaver Squad Report: Active Scav (v6)</h3>

        <div class="ys_meta" id="yaver_squad_meta_v6">
          <div>
            Server: <b id="ys_server_ts_v6">-</b>
            &nbsp;|&nbsp; Active squads: <b id="ys_active_cnt_v6">0</b>
            &nbsp;|&nbsp;
            <span class="icon header wood"></span> <b id="ys_tot_w_v6">0</b>
            <span class="icon header stone"></span> <b id="ys_tot_s_v6">0</b>
            <span class="icon header iron"></span> <b id="ys_tot_i_v6">0</b>
            <span class="icon header res"></span> <b id="ys_tot_all_v6" style="color:#005c00;">0</b>
          </div>
          <div style="margin-top:4px;">
            <b>Next finish (Server):</b> <span style="color:#005c00;"><b id="ys_next_server_v6">-</b></span>
            &nbsp;|&nbsp; <b>Remaining:</b> <b id="ys_next_rem_v6">-</b>
          </div>
        </div>

        <div class="ys_scroll">
          <table class="vis" id="yaver_squad_table_v6">
            <thead>
              <tr>
                <th>Village</th>
            ...
            <tbody id="yaver_squad_tbody_v6"></tbody>
          </table>
        </div>

        <div class="ys_actions">
          <button id="yaver_squad_refresh_v6" class="btn" style="cursor:pointer;">🔄 Refresh</button>
          <button id="yaver_squad_copy_v6" class="btn" style="cursor:pointer;">📋 Copy for Notes</button>
        </div>
      </div>
    `;
  ...
  function setMeta(totals, nearestRtSec) {
    ...
    $("#ys_server_ts_v6").text(serverTime);
    $("#ys_active_cnt_v6").text(String(t.activeSquads || 0));
    $("#ys_tot_w_v6").text(Y.util.n(t.wood || 0));
    $("#ys_tot_s_v6").text(Y.util.n(t.stone || 0));
    $("#ys_tot_i_v6").text(Y.util.n(t.iron || 0));
    $("#ys_tot_all_v6").text(Y.util.n(t.total || 0));

    Y.state.__nearestRtSec = nearestRtSec || 0;
    $("#ys_next_server_v6").text(fmtServerDateTime(nearestRtSec || 0));

    var now = Y.util.getServerNowSec();
    var rem = (nearestRtSec > 0) ? Math.max(0, nearestRtSec - now) : 0;
    $("#ys_next_rem_v6").text(nearestRtSec > 0 ? Y.util.fmtHMS(rem) : "-");
  }

  function renderRows(villages, statsById, totals) {
    var tb = $("#yaver_squad_tbody_v6");
    tb.empty();
    ...
    arr.forEach(function (v) {
      ...
      var mm = computeMinMaxReturnForVillage(v);
      var remainSpan = "<span class='yaver_rem_rng_v6' data-vid='" + vid + "' data-minrt='" + (mm.minRt || 0) + "' data-maxrt='" + (mm.maxRt || 0) + "'>-</span>";
  ...
  Y.ui.renderPanel = function (villages, statsById, totals) {
    ...
    $("#yaver_squad_refresh_v6").off("click").on("click", function () {
      try {
        window.__YAVER_SQUAD_V1__ && window.__YAVER_SQUAD_V1__.init && window.__YAVER_SQUAD_V1__.init(true);
      } catch (e) {}
    });

    $("#yaver_squad_copy_v6").off("click").on("click", function () {
  ...
  Y.ui.updateCountdowns = function () {
    ...
    $(".yaver_rem_rng_v6").each(function () {
      var $el = $(this);
    ...
    if (nrt > 0) {
      $("#ys_next_rem_v6").text(Y.util.fmtHMS(Math.max(0, nrt - now)));
    }
  };
})();