(function () {
  ...
  Y.init = function (forceRefresh) {
    if (!Y.util.isMassPage()) {
      UI && UI.ErrorMessage && UI.ErrorMessage("This script works only on the Mass Scavenging page.", 3000);
      return;
    }

    // panel zaten varsa ve force yoksa tekrar render etmesin
    if (!forceRefresh && $("#yaver_squad_panel_v5").length) {
      UI && UI.ErrorMessage && UI.ErrorMessage("Report is already on the table, Commander.", 2000);
      return;
    }
    ...
    try {
      ...
      UI && UI.SuccessMessage && UI.SuccessMessage("Yaver Squad report ready ✅", 2000);
    } catch (e) {
      console.error("Yaver Squad error:", e);
      UI && UI.ErrorMessage && UI.ErrorMessage("Yaver Squad failed to calculate: " + (e.message || e), 5000);
    }
  };
