(function () {
  ...
  var PANEL_ID = "yaver_squad_panel_v4";
  var STYLE_ID = "yaver_squad_style_v4";
  ...
  function ensurePanel() {
    ...
    var html = `
      <div id="${PANEL_ID}">
        <h3 class="ys_hdr">🛡️ Yaver Squad Raporu: Aktif Scav (v4)</h3>

        <div class="ys_meta" id="yaver_squad_meta_v4">
          <div>
            Server: <b id="ys_server_ts_v4">-</b>
            &nbsp;|&nbsp; Active squads: <b id="ys_active_cnt_v4">0</b>
            &nbsp;|&nbsp;
            <span class="icon header wood"></span> <b id="ys_tot_w_v4">0</b>
            <span class="icon header stone"></span> <b id="ys_tot_s_v4">0</b>
            <span class="icon header iron"></span> <b id="ys_tot_i_v4">0</b>
            <span class="icon header res"></span> <b id="ys_tot_all_v4" style="color:#005c00;">0</b>
          </div>
          <div style="margin-top:4px;">
            <b>En yakın bitiş (TR):</b> <span style="color:#005c00;"><b id="ys_next_tr_v4">-</b></span>
            &nbsp;|&nbsp; <b>Kalan:</b> <b id="ys_next_rem_v4">-</b>
          </div>
        </div>

        <div class="ys_scroll">
          <table class="vis" id="yaver_squad_table_v4">
            <thead>
              <tr>
                <th>Village</th>
            ...
            <tbody id="yaver_squad_tbody_v4"></tbody>
          </table>
        </div>

        <div class="ys_actions">
          <button id="yaver_squad_refresh_v4" class="btn" style="cursor:pointer;">🔄 Yenile</button>
          <button id="yaver_squad_copy_v4" class="btn" style="cursor:pointer;">📋 Defter İçin Kopyala</button>
        </div>
      </div>
    `;
  ...
  function fmtLocalDateTime(epochSec) {
    ...
    var d = new Date(epochSec * 1000); // cihaz TR ise TR döner
  ...
  function setMeta(totals, nearestRtSec) {
    ...
    $("#ys_server_ts_v4").text(serverTime);
    $("#ys_active_cnt_v4").text(String(t.activeSquads || 0));
    $("#ys_tot_w_v4").text(Y.util.n(t.wood || 0));
    $("#ys_tot_s_v4").text(Y.util.n(t.stone || 0));
    $("#ys_tot_i_v4").text(Y.util.n(t.iron || 0));
    $("#ys_tot_all_v4").text(Y.util.n(t.total || 0));

    Y.state.__nearestRtSec = nearestRtSec || 0;
    $("#ys_next_tr_v4").text(fmtLocalDateTime(nearestRtSec || 0));

    var now = Y.util.getServerNowSec();
    var rem = (nearestRtSec > 0) ? Math.max(0, nearestRtSec - now) : 0;
    $("#ys_next_rem_v4").text(nearestRtSec > 0 ? Y.util.fmtHMS(rem) : "-");
  }

  function renderRows(villages, statsById, totals) {
    var tb = $("#yaver_squad_tbody_v4");
    tb.empty();
    ...
    arr.forEach(function(v){
      ...
      var remainSpan = "<span class='yaver_rem_rng_v4' data-vid='"+vid+"' data-minrt='"+(mm.minRt||0)+"' data-maxrt='"+(mm.maxRt||0)+"'>-</span>";
  ...
  Y.ui.updateCountdowns = function () {
    ...
    $(".yaver_rem_rng_v4").each(function () {
      var $el = $(this);
    ...
    if (nrt > 0) {
      $("#ys_next_rem_v4").text(Y.util.fmtHMS(Math.max(0, nrt - now)));
    }
  };
})();
