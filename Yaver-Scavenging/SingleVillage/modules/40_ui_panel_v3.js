(function () {
  ...
  var PANEL_ID = "yss_panel_native_v3";
  var STYLE_ID = "yss_style_native_v3";
  ...
  Y.ui.ensureUI = function () {
      ...
      <table class="vis" style="width:100%;">
        <tr>
          <th style="text-align:left;">
            <div class="yss_hdr" id="yss_drag_v3">
              <div>
                <span class="icon header place"></span>
                Yaver Scavenging (Single Village) <span class="grey">(v3)</span>
              </div>
              <div class="yss_hdr_right">
                <span class="yss_dev">Developed by controleng</span>
                <input type="button" class="btn" id="yss_close_v3" value="X">
              </div>
            </div>
          </th>
        </tr>
          ...
          <td>
            <div class="yss_toolbar">
              ...
              <span class="yss_timebox">
                <label>Saat <input id="yss_h_v3" type="number" step="1" min="0"></label>
                <label>Dk <input id="yss_m_v3" type="number" step="1" min="0" max="59"></label>
                <label>Sn <input id="yss_s_v3" type="number" step="1" min="0" max="59"></label>
              </span>

              <span style="margin-left:10px;">
                <label style="white-space:nowrap;">
                  <input id="yss_mode_bal_v3" type="radio" name="yss_mode_v3" value="balanced">
              ...
              <span style="margin-left:10px;">
                <label style="white-space:nowrap;">
                  <input id="yss_mode_prio_v3" type="radio" name="yss_mode_v3" value="priority">
                  <b>Priority on filling higher categories</b>
                </label>
              </span>

              <input type="button" class="btn" id="yss_plan_v3" value="PLAN">
              <input type="button" class="btn" id="yss_fill_next_v3" value="FILL NEXT">
              <input type="button" class="btn" id="yss_clear_v3" value="CLEAR">
            </div>

            <div id="yss_msg_v3" class="info_box" style="margin-top:8px;">
          ...
          <td>
            <div class="yss_tablewrap">
              <table class="vis" style="width:100%;">
                ...
                <tbody id="yss_plan_tbody_v3"></tbody>
              </table>
            </div>
    ...
    // restore old cfg values from v2 storage
    var saved = Y.loadCfg();
    var t = saved.time || { h: 1, m: 0, s: 0 };
    document.getElementById("yss_h_v3").value = (t.h != null ? t.h : 1);
    document.getElementById("yss_m_v3").value = (t.m != null ? t.m : 0);
    document.getElementById("yss_s_v3").value = (t.s != null ? t.s : 0);

    var mode = saved.mode || "priority";
    document.getElementById("yss_mode_bal_v3").checked = (mode === "balanced");
    document.getElementById("yss_mode_prio_v3").checked = (mode === "priority");

    document.getElementById("yss_close_v3").onclick = function () {
      try {
    ...
    enableDrag(el, document.getElementById("yss_drag_v3"));
  };
  ...
  Y.ui.renderPickers = function () {
    // units
    var uWrap = document.getElementById("yss_units_v3");
    var saved = Y.loadCfg();
    ...
    Y.sendOrderDefault.forEach(function (u) {
      ...
      html += (
        '<label class="yss_unitlbl" style="white-space:nowrap;">' +
          '<input type="checkbox" class="yss_unit_v3" data-unit="' + u + '" ' + (checked ? "checked" : "") + ">" +
    ...
    // options
    var oWrap = document.getElementById("yss_opts_v3");
    ...
    opts.forEach(function (opt) {
      ...
      oh += (
        '<label style="white-space:nowrap; ' + (disabled ? "opacity:.55" : "") + '">' +
          '<input type="checkbox" class="yss_opt_v3" data-opt="' + opt.id + '" ' +
            (disabled ? "disabled" : "") + " " +
  ...
  Y.ui.renderPlan = function (plan, targetSec) {
    var tb = document.getElementById("yss_plan_tbody_v3");
    if (!tb) return;
    ...
    plan.forEach(function (p, i) {
      ...
      tr.innerHTML =
        ...
        '<td><input type="button" class="btn yss_fill_btn_v3" data-i="' + i + '" value="Fill"></td>';

      tb.appendChild(tr);
    });

    tb.querySelectorAll(".yss_fill_btn_v3").forEach(function (btn) {
  ...
  // override Y.util.msg to v3 message box (but keep same API)
  var _oldMsg = Y.util.msg;
  Y.util.msg = function (html, kind) {
    var el = document.getElementById("yss_msg_v3");
    if (!el) return _oldMsg && _oldMsg(html, kind);
    kind = kind || "info";
    el.className = (kind === "ok" ? "success_box" : (kind === "err" ? "error_box" : "info_box"));
    el.innerHTML = '<div class="content">' + html + "</div>";
  };

  // override dom reader for v3 ids (time + mode + cb classes)
  var _oldGetUIState = Y.dom.getUIState;
  Y.dom.getUIState = function () {
    var hRaw = document.getElementById("yss_h_v3") ? document.getElementById("yss_h_v3").value : "";
    var mRaw = document.getElementById("yss_m_v3") ? document.getElementById("yss_m_v3").value : "";
    var sRaw = document.getElementById("yss_s_v3") ? document.getElementById("yss_s_v3").value : "";
    ...
    var mode = (document.getElementById("yss_mode_bal_v3") && document.getElementById("yss_mode_bal_v3").checked) ? "balanced" : "priority";

    var unitsEnabled = {};
    document.querySelectorAll(".yss_unit_v3").forEach(function (cb) {
      unitsEnabled[cb.getAttribute("data-unit")] = cb.checked;
    });

    var optsEnabled = [];
    document.querySelectorAll(".yss_opt_v3").forEach(function (cb) {
