(function () {
  ...
  Y.main.plan = function () {
    ...
    Y.util.msg(extra + "✅ PLAN hazır. START ile otomatik başlat.", r.infoKind || "ok");
  };
  ...
  Y.main.startAll = function () {
    ...
    if (!sc || !sc.village || !sc.village.village_id) {
      Y.util.msg("ScavengeScreen hazır değil. Sayfayı yenileyip tekrar dene.", "err");
      return;
    }
    ...
    if (!reqs.length) {
      Y.util.msg("START için geçerli satır yok.", "err");
      return;
    }
    ...
    Y.util.msg("⏳ Gönderiliyor... (" + reqs.length + " squad)", "info");
    ...
    try {
      ...
      console.error(e);
      if (btn) { btn.disabled = false; btn.value = old; }
      Y.util.msg("❌ START hata verdi (Console'a bak).", "err");
    }
  };
  ...
  Y.init = async function () {
    ...
    try {
      ...
      if (window.UI && UI.ErrorMessage) UI.ErrorMessage("ScavengeScreen yüklenmedi. F5 deneyip tekrar.", 5000);
      return;
    }
    ...
    Y.util.msg("Hazır. PLAN → START.", "info");
  };
})();
