(function () {
  ...
  Y.math.buildPlan = function () {
    ...
    if (!optsAll.length) {
      return { ok: false, err: "No selected & eligible (OK) categories. (LOCKED/ACTIVE excluded)" };
    }
    ...
    if (totalCarryAvail <= 0) {
      return { ok: false, err: "Selected units have no carry. Check unit selection." };
    }
    ...
    if (st.mode === "priority") {
      ...
      if (!haulTargetP) {
        return { ok: false, err: "No result for this duration. Time may be too low (game minimum duration)." };
      }
      ...
      if (totalCarryAvail >= totalNeed) {
        ...
        infoKind = "ok";
        infoHtml = "✅ Priority: target time <b>" + Y.util.fmtHMS(targetSec) + "</b>.";
      } else {
        ...
        infoKind = "info";
        infoHtml = "⚠️ Not enough troops. Priority is on → higher categories are filled first; some may remain empty.";
      }
    }
    ...
    if (st.mode === "balanced") {
      ...
      infoKind = "ok";
      infoHtml = "✅ Balanced: target time <b>" + Y.util.fmtHMS(targetSec) + "</b>.";
      ...
      if (totalCarryAvail < needTotalAtTarget) {
        ...
        infoKind = "info";
        infoHtml = "⚠️ Not enough troops → Balanced time reduced to ~ <b>" + Y.util.fmtHMS(estCommonTime) + "</b>.";
      }

      // Kritik düzeltme:
      // Balanced modda hedef süre bir "üst limit" gibi davranmalı. Troop fazlaysa,
      // tüm carry'ı kullanıp süreyi uzatmayız. Bu yüzden carryBudget = min(avail, needAtTarget).
      var carryBudget = Math.min(totalCarryAvail, needTotalAtTarget);
      carryBudget = Math.max(0, Math.floor(carryBudget));
      ...
      // carryBudget üstüne çıkma, sadece bütçeye kadar tamamla
      var rem2 = carryBudget - sum;
