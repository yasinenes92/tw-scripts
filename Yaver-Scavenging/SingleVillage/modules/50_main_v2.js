(function () {
  ...
  Y.main.fillIndex = function (i, advanceIndex) {
    ...
    Y.util.msg("Oyun arayüzüne <b>" + (i + 1) + ". satır</b> yazıldı: <b>" + row.name + "</b>. Karttaki <b>Start</b> butonuna sen basacaksın.", "ok");
  };
  ...
  Y.main.plan = function () {
    ...
    if (r.plan.length) {
      // PLAN basinca 1. satir otomatik oyuna yazilsin
      Y.main.fillIndex(0, true);
      Y.util.msg("PLAN oluşturuldu. Oyun arayüzüne <b>1. satır</b> yazıldı: <b>" + r.plan[0].name + "</b>. Diğerleri için tablodan Fill veya FILL NEXT.", "ok");
    }
  };
  ...
  Y.init = async function () {
    ...
    document.getElementById("yss_plan_v2").onclick = function () { Y.main.plan(); };
    document.getElementById("yss_fill_next_v2").onclick = function () { Y.main.fillNext(); };
    document.getElementById("yss_clear_v2").onclick = function () { Y.main.clear(); };
