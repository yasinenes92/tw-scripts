(function () {
  ...
  var PANEL_ID = "yss_panel_native_v4";
  var STYLE_ID = "yss_style_native_v4";
  ...
  function enableDrag(panel, handle) {
    if (isMobile()) return; // mobilde drag kapalı -> swipe/scroll rahat
  ...
  Y.ui.ensureUI = function () {
    ...
    try {
      ...
      document.getElementById("yss_panel_native_v3") && document.getElementById("yss_panel_native_v3").remove();
      document.getElementById("yss_style_native_v3") && document.getElementById("yss_style_native_v3").remove();
    } catch (e) {}
    ...
    style.textContent = `
      ...
      /* Mobil/dar ekran: panel viewport'a sığsın, içerik yatay kayabilsin */
      @media (max-width: 900px){
        ...
        #${PANEL_ID} table.vis{
          min-width: 980px; /* tablolar yatay kaydırılsın */
        }
      }
    `;
      ...
      <table class="vis" style="width:100%;">
        <tr>
          <th style="text-align:left;">
            <div class="yss_hdr" id="yss_drag_v4">
              <div>
                <span class="icon header place"></span>
                Yaver Scavenging (Single Village) <span class="grey">(v4)</span>
              </div>
              <div class="yss_hdr_right">
                <span class="yss_dev">Developed by controleng</span>
                <input type="button" class="btn" id="yss_close_v4" value="X">
              </div>
            </div>
          </th>
        </tr>
          ...
          <td>
            <div class="yss_toolbar">
              ...
              <span class="yss_timebox">
                <label>Saat <input id="yss_h_v4" type="number" step="1" min="0"></label>
                <label>Dk <input id="yss_m_v4" type="number" step="1" min="0" max="59"></label>
                <label>Sn <input id="yss_s_v4" type="number" step="1" min="0" max="59"></label>
              </span>

              <span style="margin-left:10px;">
                <label style="white-space:nowrap;">
                  <input id="yss_mode_bal_v4" type="radio" name="yss_mode_v4" value="balanced">
              ...
              <span style="margin-left:10px;">
                <label style="white-space:nowrap;">
                  <input id="yss_mode_prio_v4" type="radio" name="yss_mode_v4" value="priority">
                  <b>Priority on filling higher categories</b>
                </label>
              </span>

              <input type="button" class="btn" id="yss_plan_v4" value="PLAN">
              <input type="button" class="btn" id="yss_fill_next_v4" value="FILL NEXT">
              <input type="button" class="btn" id="yss_clear_v4" value="CLEAR">
              <input type="button" class="btn btn-confirm-yes" id="yss_start_all_v4" value="START">
            </div>

            <div id="yss_msg_v4" class="info_box" style="margin-top:8px;">
          ...
          <td>
            <div class="yss_tablewrap">
              <table class="vis" style="width:100%;">
                ...
                <tbody id="yss_plan_tbody_v4"></tbody>
              </table>
            </div>
            <div class="yss_small" style="margin-top:6px;">
              Not: START butonu plan satırlarını sırayla doldurup ilgili seçenekten <b>otomatik başlatır</b>.
              (LOCKED/ACTIVE zaten plan dışıdır.)
            </div>
          </td>
        </tr>
      </table>
    ...
    document.getElementById("yss_h_v4").value = (t.h != null ? t.h : 1);
    document.getElementById("yss_m_v4").value = (t.m != null ? t.m : 0);
    document.getElementById("yss_s_v4").value = (t.s != null ? t.s : 0);

    var mode = saved.mode || "priority";
    document.getElementById("yss_mode_bal_v4").checked = (mode === "balanced");
    document.getElementById("yss_mode_prio_v4").checked = (mode === "priority");

    document.getElementById("yss_close_v4").onclick = function () {
      try {
    ...
    enableDrag(el, document.getElementById("yss_drag_v4"));
  };
  ...
  Y.ui.renderPickers = function () {
    // units
    var uWrap = document.getElementById("yss_units_v4");
    var saved = Y.loadCfg();
    ...
    Y.sendOrderDefault.forEach(function (u) {
      ...
      html += (
        '<label class="yss_unitlbl" style="white-space:nowrap;">' +
          '<input type="checkbox" class="yss_unit_v4" data-unit="' + u + '" ' + (checked ? "checked" : "") + ">" +
    ...
    // options
    var oWrap = document.getElementById("yss_opts_v4");
    var opts = Y.dom.listOptions();
    var savedOpts = saved.optsEnabled || null;
    ...
    opts.forEach(function (opt) {
      ...
      var checkedOpt = savedOpts ? (savedOpts.indexOf(opt.id) >= 0) : true;

      oh += (
        '<label style="white-space:nowrap; ' + (disabled ? "opacity:.55" : "") + '">' +
          '<input type="checkbox" class="yss_opt_v4" data-opt="' + opt.id + '" ' +
            (disabled ? "disabled" : "") + " " +
  ...
  Y.ui.renderPlan = function (plan, targetSec) {
    var tb = document.getElementById("yss_plan_tbody_v4");
    if (!tb) return;
    ...
    plan.forEach(function (p, i) {
      ...
      tr.innerHTML =
        ...
        '<td><input type="button" class="btn yss_fill_btn_v4" data-i="' + i + '" value="Fill"></td>';

      tb.appendChild(tr);
    });

    tb.querySelectorAll(".yss_fill_btn_v4").forEach(function (btn) {
      btn.onclick = function () {
        var i = parseInt(btn.getAttribute("data-i"), 10);
        if (Y.main && typeof Y.main.fillIndex === "function") Y.main.fillIndex(i, false);
      };
    });
  };

  // override msg to v4 box
  Y.util.msg = function (html, kind) {
    var el = document.getElementById("yss_msg_v4");
    if (!el) return;
    kind = kind || "info";
  ...
  // override getUIState to v4 ids/classes
  Y.dom.getUIState = function () {
    var hRaw = document.getElementById("yss_h_v4") ? document.getElementById("yss_h_v4").value : "";
    var mRaw = document.getElementById("yss_m_v4") ? document.getElementById("yss_m_v4").value : "";
    var sRaw = document.getElementById("yss_s_v4") ? document.getElementById("yss_s_v4").value : "";
    ...
    var mode = (document.getElementById("yss_mode_bal_v4") && document.getElementById("yss_mode_bal_v4").checked) ? "balanced" : "priority";

    var unitsEnabled = {};
    document.querySelectorAll(".yss_unit_v4").forEach(function (cb) {
      unitsEnabled[cb.getAttribute("data-unit")] = cb.checked;
    });

    var optsEnabled = [];
    document.querySelectorAll(".yss_opt_v4").forEach(function (cb) {
