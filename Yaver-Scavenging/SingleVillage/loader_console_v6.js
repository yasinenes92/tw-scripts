(function () {
  ...
  if (!isScavengePage()) {
    ...
    if (window.UI && UI.ErrorMessage) {
      UI.ErrorMessage("Going to the Scavenging page... When it opens, click Quickbar again.", 5000);
    }
    return;
  }
  ...
  var FILES = [
    "00_bootstrap_v3.js",
    "10_dom_gamebridge_v3.js",
    "20_math_planner_v4.js",
    "30_apply_inputs_v3.js",
    "40_ui_panel_v7.js",
    "50_main_v9.js"
  ];
  ...
  function loadOne(file) {
    return new Promise(function (resolve, reject) {
      ...
      s.onload = function () {
        console.log("[YSS Loader v6] OK:", file);
        resolve(true);
      };
      s.onerror = function () {
        console.log("[YSS Loader v6] FAIL:", file, s.src);
  ...
  (async function () {
    try {
      ...
      if (window.__YSS_SINGLE_V2__ && typeof window.__YSS_SINGLE_V2__.init === "function") {
        ...
        if (window.UI && UI.SuccessMessage) UI.SuccessMessage("Yaver Scav ready ✅", 2000);
      } else {
        console.warn("[YSS Loader v6] __YSS_SINGLE_V2__.init not found.");
        if (window.UI && UI.ErrorMessage) UI.ErrorMessage("Yaver Scav loaded but init not found (check Console).", 5000);
      }
    } catch (e) {
      console.error("[YSS Loader v6] ERROR:", e);
      if (window.UI && UI.ErrorMessage) UI.ErrorMessage("Yaver Scav failed to load ❌ (check Console)", 6000);
      else alert("Yaver Scav failed to load. Check Console.");
    }
  })();
})();
