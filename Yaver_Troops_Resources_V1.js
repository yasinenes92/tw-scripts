(function () {
  ...
  function rowsToBBCode(rows, units) {
    // compact BBCode table
    const headerCells = ['Village','Wood','Clay','Iron','Storage','Pop', ...units]
      .map(h => `[b]${h}[/b]`).join('[||]');
    const body = rows.map(r => {
      const pop = `${fmt(r.res.popNow)}/${fmt(r.res.popMax)}`;
      const unitVals = units.map(u => fmt(r.units[u] || 0));
      return [r.name, fmt(r.res.wood), fmt(r.res.stone), fmt(r.res.iron), fmt(r.res.storage), pop, ...unitVals].join('[||]');
    }).join('\n');
    return `[table]\n[**]${headerCells}[/**]\n${body.split('\n').map(line => `[*]${line}`).join('\n')}\n[/table]`;
  }
