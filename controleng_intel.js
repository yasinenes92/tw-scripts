/* CONTROLENG'S TOP 5 CONTINENT TRIBE ANALYSIS v2.0 (The Perfection)
   Author: controleng
   Methodology: Global Percentile + GeoMean Loot + Roster Karma + Bootstrap CI
   Hosted on GitHub
*/
...
(function() {
    ...
    <div id="controleng_gui" style="position:fixed; top:50px; left:50%; margin-left:-380px; width:760px; background:#f4e4bc; border:3px solid #7d510f; z-index:99999; padding:10px; box-shadow:0 0 15px rgba(0,0,0,0.6); font-family: Verdana, Arial; font-size:12px;">
        <h3 style="margin:0; background:#7d510f; color:#fff; padding:8px; text-align:center;">📊 Controleng's Scientific Analysis v2.0</h3>
        <div id="intel_content" style="padding:15px; text-align:center;">
            <p><strong>Methodology:</strong> Global Percentile & Roster Karma</p>
            <ul style="text-align:left; margin-left:120px; font-size:11px;">
                <li>✅ <strong>Full Data:</strong> Records, Dates ("yesterday" fixed), and Percentiles.</li>
                <li>✅ <strong>Loot Logic:</strong> Geometric Mean of (Res Amount & Village Count).</li>
                <li>✅ <strong>Coverage:</strong> Shows how many members have active data.</li>
                <li>✅ <strong>Certainty:</strong> Bootstrap CI (95%) included.</li>
            </ul>
    ...
    function fixDateStr(dateStr) {
        ...
        // Eğer zaten tarih formatındaysa (DD.MM.YYYY) temizle ve döndür
        // "on 02.01.2026" gibi gelirse temizle
        var match = ds.match(/(\d{2}\.\d{2}\.\d{4})/);
    ...
    function processPlayerMetrics() {
        ...
        $.when(p1, p2, p3).done(function(r1, r2, r3) {
            ...
            player.lootResDate = fixDateStr(lResData.date);
            
            player.lootVil = lVilData.score; // Count
            player.lootVilDate = fixDateStr(lVilData.date);
            
            player.scavenge = scavData.score;
            player.scavengeDate = fixDateStr(scavData.date);
    ...
    // --- MATHEMATICAL ENGINE (The Core) ---
    function performScientificAnalysis() {
        $('#intel_status_text').text("Calculating Statistics (Percentiles, Karma)...");
        
        // 1. Create Sorted Arrays for Percentiles
        ...
        allPlayersGlobal.forEach(p => {
            ...
            // This balances "Big Raids" with "Frequent Raids"
            var p_L = Math.sqrt(p_LA * p_LC);

            // Weighting (Rank Influence)
            var w = 0.5 + (0.5 * p_R);

            // Player Karma (Raw 0-100)
            // Geometric Mean of Scavenge and Loot Performance
        ...
        targetTribes.forEach(t => {
            ...
            // Roster Karma (Weighted Mean of Player Karmas)
            var sumW = 0, sumKarma = 0, sumS = 0, sumL = 0;
            members.forEach(m => {
                sumW += m.metrics.w;
                sumKarma += (m.metrics.w * m.metrics.karma);
                // Also track category scores for display
    ...
    function runBootstrap(members, iterations) {
        ...
        for(var i=0; i<iterations; i++) {
            ...
            // Calculate Roster Karma for sample
            var sW = 0, sK = 0;
            sample.forEach(m => {
                sW += m.metrics.w;
    ...
    function generateFinalReport(stats) {
        ...
        stats.forEach(s => {
            ...
            s.members.forEach(m => {
                // Show if active in at least one
                if (m.points > 0 && (m.metrics.p_S > 0.01 || m.metrics.p_L > 0.01)) {
                    ...
                    // Loot (Using LootRes for record display, but logic uses combined)
                    var lootDisplay = "-";
                    if (m.lootRes > 0) {
