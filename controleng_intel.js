/* CONTROLENG'S TOP 5 CONTINENT TRIBE ANALYSIS v1.1
   Author: controleng
   Methodology: Global Percentile + Bootstrap CI + Raw Records + Scores
   Hosted on GitHub
*/
...
(function() {
    ...
    <div id="controleng_gui" style="position:fixed; top:50px; left:50%; margin-left:-350px; width:700px; background:#f4e4bc; border:3px solid #7d510f; z-index:99999; padding:10px; box-shadow:0 0 15px rgba(0,0,0,0.6); font-family: Verdana, Arial; font-size:12px;">
        ...
        <div id="intel_content" style="padding:15px; text-align:center;">
            ...
            <ul style="text-align:left; margin-left:80px; font-size:11px;">
                <li>✅ <strong>Global Comparison:</strong> Compares all players in the continent.</li>
                <li>✅ <strong>Full Detail:</strong> Shows Records, Dates AND Performance Percentages.</li>
                <li>✅ <strong>Statistical Certainty:</strong> Uses Bootstrap CI to determine superiority.</li>
            </ul>
    ...
    function generateFinalReport(stats) {
        ...
        output += "(*) [b]Karma Score (0-100):[/b] The ultimate power rating. Combines Looting and Scavenging performance.\n";
        output += "(*) [b]Scav/Loot %:[/b] Global Percentile. '90%' means this player is better than 90% of the entire continent.\n";
        output += "(*) [b]CI (95%):[/b] Confidence Interval. If Tribe A's lower number is higher than Tribe B's upper number, Tribe A is scientifically superior.\n\n";
        ...
        stats.forEach(s => {
            ...
            // EKLENEN SÜTUNLAR: Scav % ve Loot %
            output += `[table]\n[**]Player[||]Points (Rank)[||]Scav Record (Date)[||]Scav %[||]Loot Record (Date)[||]Loot %[||]Karma %[/**]\n`;
            ...
            s.members.forEach(m => {
                if (m.points > 0 && (m.metrics.p_S > 0.01 || m.metrics.p_L > 0.01)) {
                    ...
                    var scavP = (m.metrics.p_S * 100).toFixed(1); // Scavenge % Score
                    var lootP = (m.metrics.p_L * 100).toFixed(1); // Loot % Score
                    
                    var scavDisplay = m.scavenge > 0 ? `${numFmt(m.scavenge)} (${m.scavengeDate})` : "-";
                    var lootDisplay = m.lootRes > 0 ? `${numFmt(m.lootRes)} (${m.lootResDate})` : "-";

                    output += `[*] [player]${m.name}[/player] [|] ${numFmt(m.points)} (${rankP}%) [|] ${scavDisplay} [|] [b]${scavP}[/b] [|] ${lootDisplay} [|] [b]${lootP}[/b] [|] [b]${karmaP}[/b]\n`;
                }
            });
            output += `[/table]\n[/spoiler]\n\n`;
        });
