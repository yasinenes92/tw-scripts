(function () {
  ...
  if (window.YTR_V5 && window.YTR_V5.version === SCRIPT.version) {
    window.YTR_V5.show();
    return;
  }
  ...
  function getVillageListFromCombined(combinedHtml) {
    ...
    for (const tr of rows) {
      ...
      const name =
        (label?.getAttribute('data-text') || label?.textContent || '').trim() ||
  ...
  // ---------- Merchants (Production overview) ----------
  function parseMerchantsText(txt) {
    const raw = String(txt || '').trim();
    const m = raw.match(/^\s*([\d.\s]+)\s*\/\s*([\d.\s]+)\s*$/);
    if (!m) return { text: raw || '—', avail: 0, total: 0, ok: false };
    const avail = toIntLoose(m[1]);
    const total = toIntLoose(m[2]);
    return { text: `${avail}/${total}`, avail, total, ok: true };
  }
  ...
  function getMerchantsMapFromProd(prodHtml) {
    ...
    for (const tr of rows) {
      ...
      // merchants cell has link to market and text like "110/110"
      const a = tr.querySelector('a[href*="screen=market"]');
      const t = parseMerchantsText(a ? a.textContent : '');
      map[id] = t.ok ? t : { text: '—', avail: 0, total: 0, ok: false };
    }
  ...
  function rowsToTSV(rows, units) {
    const header = ['Village', 'Wood', 'Clay', 'Iron', 'Storage', 'Merchants', 'Pop', ...units].join('\t');

    const lines = (rows || []).map((r) => {
      const pop = `${r.res.popNow}/${r.res.popMax}`;
      const merch = r.merchants?.text || '—';
      const unitVals = units.map((u) => String(r.units[u] || 0));
      return [r.name, r.res.wood, r.res.stone, r.res.iron, r.res.storage, merch, pop, ...unitVals].join('\t');
    });

    // Totals row (no storage/merchants/pop totals)
  ...
  function rowsToBBCodeIcons(rows, units) {
    const headerCells = [
      ...
      bbImg('/graphic/buildings/market.png'), // Merchants icon
      ...
      <table id="ytr_table" class="vis overview_table" style="white-space:nowrap; width:auto; table-layout:auto;">
        <thead>
          <tr>
            ...
            <th style="text-align:center">${iconHTML('/graphic/holz.png', 'Wood')}</th>
            <th style="text-align:center">${iconHTML('/graphic/lehm.png', 'Clay')}</th>
            <th style="text-align:center">${iconHTML('/graphic/eisen.png', 'Iron')}</th>
            <th style="text-align:center">${iconHTML('/graphic/buildings/storage.png', 'Storage')}</th>
            <th style="text-align:center">${iconHTML('/graphic/buildings/market.png', 'Merchants')}</th>
            <th style="text-align:center">${iconHTML('/graphic/buildings/farm.png', 'Population')}</th>
            ${unitHeaders}
          </tr>
        </thead>
      ...
      <div id="ytr_root" style="display:inline-block;">
        <div class="vis" style="padding:8px; display:inline-block; overflow-x:auto; max-width:calc(100vw - 40px);">
          <h2 style="margin:4px 0 10px 0;">
            ...
            <span class="grey" style="float:right; font-size:12px; margin-top:6px;">Developed by Controleng</span>
          </h2>
          ...
          <div style="margin-top:8px; display:inline-block; clear:both;">
  ...
  function updateTotalsRow(units, totals) {
    ...
    // td indices: 0 wood, 1 clay, 2 iron, 3 storage(—), 4 merchants(—), 5 pop(—), then units...
    ...
    // cells[4] merchants no total; keep —
    // cells[5] pop no total; keep —
  ...
  function renderRow(tbody, idx, row, units) {
    ...
    const merchText = row.merchants?.text || '—';
  ...
  async function scanAll(state) {
    ...
    // Merchants map from Production overview (single fetch)
    let merchantsMap = {};
    try {
      const prodUrl = `/game.php?village=${baseVillageId}&screen=overview_villages&mode=prod&group=0&page_size=100`;
      const prodHtml = await httpGet(prodUrl);
      merchantsMap = getMerchantsMapFromProd(prodHtml);
    } catch (e) {
      merchantsMap = {};
      console.warn('[YTR] merchants map load failed:', e);
    }
    ...
    for (let i = 0; i < villages.length; i++) {
      ...
      const merch = merchantsMap[v.id] || { text: '—', avail: 0, total: 0, ok: false };
      ...
      try {
        ...
        const row = { id: v.id, name: v.name, res, units: unitTotals, merchants: merch };
        rows.push(row);
