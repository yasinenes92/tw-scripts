/* W152 | Gündüz (3 Saat) - Mass Scav Infantry | QuickBar External Script */
(function () {
  "use strict";

  const SETTINGS = { targetHours: 3, usePremium: true, dryRun: false };
  ...
  function planAndSend() {
    ...
    const unitOrder = ["spear", "sword", "axe", "archer"]
      .filter(u => cfg.unitCarry[u] > 0)
      .sort((a,b) => cfg.unitCarry[b] - cfg.unitCarry[a]);

    const squadRequests = [];
    console.log(`=== W152 MASS SCAV | ${SETTINGS.targetHours}h | MODE=INF ===`);
    ...
    TribalWars.post(
      ...
      function () {
        UI?.SuccessMessage?.(`☀️ 3 Saat (INF) seferleri başlatıldı! (${squadRequests.length})`);
