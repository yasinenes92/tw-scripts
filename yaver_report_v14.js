/* =======================================================================
   Yaver Report v14.0.0  (TribalWars / DS)

   Fix in v14:
   - Tribe tag detection is now robust:
     Primary source: game_data.player.ally  (usually the tribe tag)
     Fallbacks: parse from info_ally header, else "YOUR_TRIBE"

   Everything else stays the same as v13 (no major refactor).
   ======================================================================= */
...
(function () {
  ...
  function log(...a) { if (CFG.debug) console.log("[YaverV14]", ...a); }
  function warn(...a) { console.warn("[YaverV14]", ...a); }
  function err(...a) { console.error("[YaverV14]", ...a); }
  ...
  function getMyAllyId() {
    ...
    if (!p) return null;
    return p.ally_id || p.ally_id === 0 ? p.ally_id : (p.ally || null);
  }
  ...
  // ---------- tribe tag detection (FIXED) ----------
  function getMyTribeTagFromGameData() {
    const p = window.game_data && game_data.player ? game_data.player : null;
    if (!p) return "";
    // In many TW worlds, game_data.player.ally is the tribe tag (string).
    // If it is numeric, ignore it.
    const a = p.ally;
  ...
  function parseTribeTagFromInfoAlly(html) {
    ...
    // scan some text as fallback
    const txt = ($doc.text() || "").slice(0, 1500);
  ...
  function showDialog(bb) {
    const id = "yaver_v14_text";
    const html = `
      <div style="max-width:980px;">
        <div style="margin:6px 0 10px 0;">
          <a class="btn" id="yaver_v14_copy">Copy BBCode</a>
        </div>
    ...
    setTimeout(() => {
      ...
      $("#yaver_v14_copy").off("click").on("click", async () => {
        try {
  ...
  async function run() {
    try {
      ...
      // ✅ v14: robust tribe tag
      const myTribeTag =
        getMyTribeTagFromGameData() ||
        parseTribeTagFromInfoAlly(myAllyHtml) ||
        "YOUR_TRIBE";
