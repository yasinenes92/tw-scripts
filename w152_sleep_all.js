/* W152 | Uyku (8 Saat) - Mass Scav All | QuickBar External Script */
(function () {
  "use strict";
  ...
  function buildUnitOrder(cfg) {
    let all = ["knight", "light", "marcher", "heavy", "spear", "sword", "axe", "archer"];
    if (!SETTINGS.includeKnight) all = all.filter(u => u !== "knight");
    return all.filter(u => cfg.unitCarry[u] > 0);
  }
  ...
  function planAndSend() {
    ...
    TribalWars.post(
      ...
      function () {
        UI?.SuccessMessage?.(`🌙 8 Saat (ALL) seferleri başlatıldı! (${squadRequests.length})`);
