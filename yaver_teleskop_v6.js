(function () {
  ...
  // compute deltas but ATTRIBUTE THEM TO THE PREVIOUS DAY (fix +1 day shift)
  const out = [];
  for (let i = 1; i < rows.length; i++) {
    ...
    out.push({
      date: prev.date, // <-- 핵 fix: use previous day's date
  ...
  if (tw && tw.history && tw.history.length) {
    ...
    // 4) The 3 separate "Daily * Gain" tables (like UI)
    s += `[b]Daily Points Gain[/b]\n`;
    s += `[table]\n[**]Date[||]Δ[/**]\n`;
    for (const r of tw.history) {
      s += `[*]${r.date}[|]${fmtSigned(r.pointsDelta)}\n`;
    }
    s += `[/table]\n\n`;

    s += `[b]Daily Villages Change[/b]\n`;
    s += `[table]\n[**]Date[||]Δ[/**]\n`;
    for (const r of tw.history) {
      s += `[*]${r.date}[|]${fmtSigned(r.villagesDelta)}\n`;
    }
    s += `[/table]\n\n`;
