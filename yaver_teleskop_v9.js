/* =======================================================================
   ...
   - Player autocomplete -> select -> fetch (kept)
   - Works on info_player page: auto-detect player (kept)
   - TWStats fetch: via CORS proxies (kept)
   - Manual TWStats Import (CSV) (kept)
   - Copy BBCode: ONLY Daily Records + TWStats History table (Snapshot removed)
   ======================================================================= */
(function () {
  "use strict";
  ...
  const CFG = {
    ...
    // TWStats: use proxies only (no direct twstats XHR)
  ...
  const STATE = {
    ...
    last: {
      playerName: "",
      playerId: null,
      dailyRows: null,   // [{label,score,date}]
      twstats: null,     // { history:[{date, pointsDelta, villagesDelta, odDelta, ...}] }
    },
  };
  ...
  const I18N = {
    en: {
      ...
      twstatsProxyUsed: (p) => `TWStats fetched via proxy.`,
    ...
    tr: {
      ...
      twstatsProxyUsed: (p) => `TWStats proxy ile çekildi.`,
  ...
  async function getHtmlCorsSafe(url) {
    if (isTwStatsUrl(url)) return await getHtmlViaProxy(url); // proxy only
    return { html: await getHtml(url), proxy: null };         // same-origin
  }
  ...
  // --------------------- CSV parsing (with date-shift fix) ---------------------
  function parseCsvHistory(text) {
    // lines like: 2026-01-21,Cocothecoc,coco,8,589326,237,64968,11025,53943
    ...
    for (const line of lines) {
      ...
      rows.push({
        date: parts[0],
        points: parseNumberAny(parts[4]),
        villages: parseNumberAny(parts[5]),
        od: parts[6] != null ? parseNumberAny(parts[6]) : 0,
      });
    }

    rows.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 : 0));

    // FIX: delta belongs to PREVIOUS day's date (prevents +1 day shift)
    const out = [];
    for (let i = 1; i < rows.length; i++) {
      ...
      out.push({
        ...
        odValue: cur.od,
        pointsDelta: cur.points - prev.points,
        villagesDelta: cur.villages - prev.villages,
        odDelta: cur.od - prev.od,
      });
    }
    return out;
  }

  function takeLastNDays(arrAsc, n) {
    const slice = arrAsc.slice(Math.max(0, arrAsc.length - n));
    return slice.reverse(); // newest-first for display
  }
  ...
  // --------------------- BBCode copy (NO Snapshot, NO extra tables) ---------------------
  ...
  function renderTwStatsFromHistory(historyNewestFirst) {
    const $out = $("#" + IDS.outTw);
    if (!$out.length) return;
  ...
  async function fetchAndRenderTwStats() {
    ...
    try {
      const r = await getHtmlCorsSafe(urls.history);
      ...
      STATE.last.twstats = { history: hist };
      renderTwStatsFromHistory(hist);

      showStatus(t("twstatsProxyUsed", r.proxy || "proxy"), false);
      ok(t("done"));
    } catch (e) {
  ...
  function bindUI() {
    ...
    $twOpen.off("click").on("click", function () {
      ...
      const url = (pid && baseUrl)
        ? `${baseUrl}index.php?page=player&id=${encodeURIComponent(pid)}&tab=history`
        : ctxTw;
