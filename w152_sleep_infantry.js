/* W152 | Uyku (8 Saat) - Mass Scav Infantry | QuickBar External Script */
(function () {
  "use strict";

  const SETTINGS = {
    targetHours: 8,
    mode: "infantry",      // "infantry" | "all"
    usePremium: true,
    dryRun: false          // true => sadece konsola plan basar, göndermez
  };

  // W152 teyitli / fallback sabitler
  const FALLBACK = {
  ...
  function getWorldConfig() {
    // W152’de genelde bunlar mevcut olur; yoksa fallback kullanırız
  ...
  // HARD-CAP kapasite hesabı (diğer yapay zekanın formülü ile aynı; +100 yok!)
  function capacityForTarget(seconds, lootFactor, cfg) {
    ...
    const cap = Math.sqrt(t2 / 100) / lootFactor;
    return Math.floor(cap);
  }

  function buildUnitOrder(cfg) {
    // infantry: taşıma büyükten küçüğe (daha iyi paketleme)
    const infantry = ["spear", "sword", "axe", "archer"];
    return infantry
      .filter(u => cfg.unitCarry[u] > 0)
      .sort((a,b) => cfg.unitCarry[b] - cfg.unitCarry[a]);
  }
  ...
  function planAndSend() {
    ...
    const unitOrder = buildUnitOrder(cfg);

    const squadRequests = [];

    console.log(`=== W152 MASS SCAV | ${SETTINGS.targetHours}h | MODE=${SETTINGS.mode} ===`);
    ...
    for (const vid in villages) {
      ...
      for (let optionId = 4; optionId >= 1; optionId--) {
        ...
        if (optState.scavenging_squad) continue; // aktif
        ...
        for (const u of unitOrder) {
          ...
          const maxTake = Math.floor((remaining + 1e-9) / per); // float güvenliği
    ...
    if (!squadRequests.length) {
      UI?.ErrorMessage?.("Gönderilecek uygun piyade / boş seçenek yok.");
      return;
    }
    ...
    TribalWars.post(
      ...
      function () {
        UI?.SuccessMessage?.(`🌙 8 Saat (INF) seferleri başlatıldı! (${squadRequests.length})`);
