/* CONTROLENG'S INTEL v4.0 (The Complete Edition)
   Author: controleng
   Methodology: Global Percentile + Roster Karma + Fixed Dates + Point Columns
   Hosted on GitHub
*/
...
(function() {
    ...
    <div id="controleng_gui" style="position:fixed; top:50px; left:50%; margin-left:-380px; width:760px; background:#f4e4bc; border:3px solid #7d510f; z-index:99999; padding:10px; box-shadow:0 0 15px rgba(0,0,0,0.6); font-family: Verdana, Arial; font-size:12px;">
        <h3 style="margin:0; background:#7d510f; color:#fff; padding:8px; text-align:center;">📊 Controleng's Intel v4.0</h3>
        <div id="intel_content" style="padding:15px; text-align:center;">
            ...
            <ul style="text-align:left; margin-left:120px; font-size:11px;">
                <li>✅ <strong>Full Data:</strong> Records + Dates + Percentages % included.</li>
                <li>✅ <strong>New Columns:</strong> Tribe Points & Player Points added.</li>
    ...
    function performScientificAnalysis() {
        ...
        targetTribes.forEach(t => {
            ...
            // NEW: Tribe Total Points Calculation
            var totalTribePoints = members.reduce((sum, m) => sum + m.points, 0);
            ...
            tribeStats.push({
                name: t.name,
                points: totalTribePoints, // Tribe Points
                members: members,
                n: members.length,
                cov: coverageStr,
    ...
    function generateFinalReport(stats) {
        ...
        var output = "[b][size=14]📊 Controleng's Intel v4.0 (" + dateStr + ")[/size][/b]\n";
        ...
        // ADDED: Points column to Tribe Table
        output += "[table]\n[**]Rank[||]Tribe[||]Points[||]Members (Cov)[||]Scav Score[||]Loot Score[||]ROSTER KARMA (CI)[/**]\n";
        stats.forEach((s, i) => {
            output += `[*] ${i+1} [|] ${s.name} [|] ${numFmt(s.points)} [|] ${s.n} (${s.cov}) [|] ${s.scores.S.toFixed(1)} [|] ${s.scores.L.toFixed(1)} [|] [b]${s.scores.K.toFixed(1)}[/b] (CI: ${s.ci.lower}-${s.ci.upper})\n`;
        });
        output += "[/table]\n\n";
        output += "--------------------------------------------------\n\n";
        ...
        stats.forEach(s => {
            ...
            // CONFIRMED: Points (Rank%) is present and clear
