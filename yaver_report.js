...
/* Özellikler: Verimlilik Tablosu eklendi (Kaynak / 2k+ Köy Puanı) */
...
function generateFinalBBCode() {
    ...
    // Her oyuncu için rasyoyu hesapla
    playerArray.forEach(function(p) {
    ...
    // Sıralama (En yüksek rasyo en üstte)
    playerArray.sort(function(a, b) {
    ...
    playerArray.forEach(function(p) {
        ...
        // Sadece skoru 0'dan büyük olanları göster (veya aktif puanı olanları)
        if(ratio > 0) {
             var displayRatio = ratio.toFixed(2); // Virgülden sonra 2 hane
             var bar = generateProgressBar(ratio, maxRatio);
             
             // Binlik ayırıcı ekle (Görsel güzellik için)
