(async () => {
  ...
  function ensureUI() {
    ...
    el.querySelector("#yss_plan").onclick = () => buildPlan(true); // PLAN -> also preview on game UI
  ...
  // ========= pickers =========
  const sendOrderDefault = ["spear","sword","axe","archer","light","marcher","heavy"]; // native list
  ...
  function readTimeSeconds() {
    ...
    // If user left all blank, fallback to saved/default (1:00:00)
  ...
  function fillGameInputsForRow(row) {
    const inputs = document.querySelectorAll(".candidate-squad-widget input.unitsInput");
    inputs.forEach(inp => {
  ...
  function buildPlan(previewOnPlan) {
    ...
    // PLAN -> oyunda da göster: 1. satırı doldur + highlight
    if (previewOnPlan) {
      fillGameInputsForRow(plan[0]);
      // Fill Next bir sonraki satırdan devam etsin
      Y.state.idx = 1;
