...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const buildActionIcon = function (row) {
    if (row.action_mode === 'c_preview') {
      return `<span class="farm_icon farm_icon_c decoration" style="margin:auto;display:inline-block;opacity:0.7;"></span>`;
    }

    return `<a href="#" data-origin="${row.origin.id}" data-target="${row.target.id}" data-template="${row.template.id}" class="farmGod_icon farm_icon farm_icon_${row.template.name}" style="margin:auto;"></a>`;
  };

  const buildActionLabel = function (row) {
    if (row.action_mode === 'c_preview') {
      return 'C preview';
    }
    return 'A path';
  ...
  const buildRowKey = function (planToken, row) {
    return `${planToken}__${row.origin.id}__${row.target.coord}`;
  };
  ...
  const buildTable = function (plan, planToken) {
    let summary = `<div style="width:98%;margin:0px auto 5px auto;text-align:left;font-size:11px;">
                    Sendable A-path rows: ${plan.counter} |
                    C preview rows: ${plan.cPreviewCounter} |
                    ...
                    <span class="fgResolveSummary">Resolving C preview rows...</span>
                  </div>`;
    ...
    let html = `<div class="vis farmGodContent"><h4>FarmGod</h4>${summary}<div id="FarmGodProgessbar" class="progress-bar live-progress-bar progress-bar-alive" style="width:98%;margin:5px auto;"><div style="background: rgb(146, 194, 0);"></div><span class="label" style="margin-top:0px;"></span></div><table class="vis" width="100%">
                <tr><th style="text-align:center;">${t.table.origin}</th><th style="text-align:center;">${t.table.target}</th><th style="text-align:center;">${t.table.wall}</th><th style="text-align:center;">${t.table.attacked}</th><th style="text-align:center;">${t.table.action}</th><th style="text-align:center;">${t.table.cLight}</th><th style="text-align:center;">${t.table.resolved}</th><th style="text-align:center;">${t.table.fields}</th><th style="text-align:center;">${t.table.farm}</th></tr>`;

    if (!$.isEmptyObject(plan.farms)) {
      for (let prop in plan.farms) {
        if (game_data.market == 'nl') {
          html += `<tr><td colspan="9" style="background: #e7d098;"><input type="button" class="btn switchVillage" data-id="${plan.farms[prop][0].origin.id}" value="${t.table.goTo} ${plan.farms[prop][0].origin.name} (${plan.farms[prop][0].origin.coord})" style="float:right;"></td></tr>`;
        }

        plan.farms[prop].forEach((val, i) => {
          let rowKey = buildRowKey(planToken, val);
          let cLightCell =
            val.action_mode === 'c_preview'
              ? `<td class="fgCLight" data-row-key="${rowKey}" style="text-align:center;">...</td>`
          ...
          html += `<tr class="farmRow row_${i % 2 == 0 ? 'a' : 'b'}">
                    ...
                    <td style="text-align:center;">${buildActionLabel(val)}</td>
                    ${cLightCell}
                    ${resolvedCell}
                    <td style="text-align:center;">${val.fields.toFixed(2)}</td>
                    <td style="text-align:center;">${buildActionIcon(val)}</td>
                  </tr>`;
        });
      }
    } else {
      html += `<tr><td colspan="9" style="text-align: center;">${t.table.noFarmsPlanned}</td></tr>`;
    }
  ...
  const parseForecastAttr = function (raw) {
    if (!raw) return null;
  ...
  const getData = function (group, newbarbs, losses) {
    ...
    const readWallLevel = function ($row) {
      ...
      return /^\d+$/.test(wallText)
        ? parseInt(wallText, 10)
        : null;
    };
    ...
    for (let prop in data.villages) {
      ...
      orderedFarms.forEach((el) => {
        ...
        let arrival = Math.round(
          serverTime +
            distance * reserveTemplate.speed * 60 +
  ...
  const fetchOriginCForecasts = function (originId) {
    ...
    cResolveCache[originId] = lib
      .processAllPages(
        `/game.php?village=${originId}&screen=am_farm`,
        ($html) => {
          $html
            .find('#plunder_list')
            .find('tr[id^="village_"]')
            .each((i, el) => {
              ...
              let rawForecast = $cButton.attr('data-units-forecast');
              let parsedForecast = parseForecastAttr(rawForecast);
              let cLight = null;

              if (
                parsedForecast &&
                typeof parsedForecast === 'object' &&
                parsedForecast.hasOwnProperty('light')
              ) {
                let numeric = Number(parsedForecast.light);
      ...
      .then(() => forecastMap)
      .catch((error) => {
        console.error('C resolve fetch failed for village', originId, error);
        return { __error: true };
      });
  ...
  const updateResolveSummary = function (counts, planToken) {
    ...
    $summary.text(
      `Resolved C rows: ready ${counts.cReady}, A fallback ${counts.aFallback}, skip ${counts.skip}, unreadable ${counts.unreadable}, errors ${counts.error}`
    );
  };

  const resolveCPreviewRows = function (plan, planToken) {
    let previewRows = [];
    ...
    Object.keys(grouped).forEach((originId) => {
      fetchOriginCForecasts(originId).then((forecastMap) => {
        if (planToken !== cResolvePlanToken) return;

        grouped[originId].forEach((row) => {
          ...
          if (!$cLightCell.length || !$resolvedCell.length) {
            return;
          }

          if (forecastMap.__error) {
            $cLightCell.text('ERR');
            $resolvedCell.text('Resolve error');
            counts.error++;
            return;
          }

          let resolved = forecastMap[row.target.coord];
          if (!resolved || !resolved.readable) {
            $cLightCell.text('--');
            $resolvedCell.text('Unreadable');
            counts.unreadable++;
            return;
          }

          $cLightCell.text(resolved.cLight);

          if (resolved.cLight >= C_LIGHT_MIN) {
            $resolvedCell.text('C ready');
            counts.cReady++;
          } else if (row.wall < 2) {
            $resolvedCell.text('A fallback');
            counts.aFallback++;
          } else {
            $resolvedCell.text('Skip');
            counts.skip++;
          }
        });
