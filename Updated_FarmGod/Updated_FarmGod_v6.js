...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const buildActionLabel = function (row) {
    if (row.action_mode === 'c_preview') return 'C preview';
    return 'A path';
  };
        ...
        plan.farms[prop].forEach((val, i) => {
          ...
          let reasonCell =
            val.action_mode === 'c_preview'
              ...
              : `<td style="text-align:center;">A rule</td>`;
  ...
  const getFirstCoordText = function ($row) {
    return (
      $row
  ...
  const fetchOriginCForecasts = function (originId) {
    ...
    cResolveCache[originId] = lib
      .processAllPages(
        `/game.php?village=${originId}&screen=am_farm&order=distance&dir=asc`,
        ($html) => {
          $html
            .find('#plunder_list')
            .find('tr[id^="village_"]')
  ...
  const updateResolveSummary = function (counts, planToken) {
    ...
    $summary.text(
      `Resolved C rows: ready ${counts.cReady}, A fallback ${counts.aFallback}, skip ${counts.skip}, row not found ${counts.rowNotFound}, c button missing ${counts.cButtonMissing}, forecast missing ${counts.forecastMissing}, forecast parse error ${counts.forecastParseError}, light missing ${counts.lightMissing}, fetch errors ${counts.fetchError}`
    );
  };

  const classifyResolveFailure = function (reason, counts) {
    if (reason === 'c button missing') {
      counts.cButtonMissing++;
      return;
    }
    if (reason === 'forecast missing') {
      counts.forecastMissing++;
      return;
    }
    if (reason === 'forecast parse error') {
      counts.forecastParseError++;
      return;
    }
    if (reason === 'light missing') {
      counts.lightMissing++;
      return;
    }
  ...
  const updateResolvedRow = function (rowKey, payload) {
  ...
  const resolveCPreviewRows = function (plan, planToken) {
    ...
    Object.keys(grouped).forEach((originId) => {
      fetchOriginCForecasts(originId).then((originResult) => {
        if (planToken !== cResolvePlanToken) return;

        grouped[originId].forEach((row) => {
          let rowKey = buildRowKey(planToken, row);

          if (originResult.fetchError) {
            updateResolvedRow(rowKey, {
              cLightText: 'ERR',
              resolvedText: 'Resolve error',
              reasonText: 'page fetch failed',
            });
            counts.fetchError++;
            return;
          }
          ...
          if (!resolved.readable) {
            updateResolvedRow(rowKey, {
              cLightText: '--',
              resolvedText: 'Unreadable',
              reasonText: resolved.reason || 'row not found',
            });
            classifyResolveFailure(resolved.reason || 'row not found', counts);
            return;
          }

          if (resolved.cLight >= C_LIGHT_MIN) {
            updateResolvedRow(rowKey, {
              cLightText: String(resolved.cLight),
