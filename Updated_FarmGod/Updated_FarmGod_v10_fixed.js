...
window.FarmGod.Library = (function () {
  ...
  const getFarmTotalPages = function ($html) {
    if ($html.find('#am_widget_Farm').length === 0) {
      return 1;
    }
    ...
    let maxHumanPage = 1;
    $navItems.each((i, el) => {
      let txt = $(el).text().replace(/\D/g, '');
      let val = parseInt(txt, 10);
      if (Number.isFinite(val)) {
        maxHumanPage = Math.max(maxHumanPage, val);
      }
    });

    return Math.max(1, maxHumanPage);
  };
...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const RESOLVE_PAGE_DELAY_MS = 1100;
  const RESOLVE_ORIGIN_DELAY_MS = 2000;
  const RESOLVE_429_BACKOFF_MS = [4000, 8000, 15000, 25000];
  ...
  const buildASendHtml = function (rowKey, row, templateId) {
    return `<a href="#" class="farmGod_send_icon farm_icon farm_icon_a" data-send-mode="a" data-origin="${row.origin.id}" data-target="${row.target.id}" data-template="${templateId}" data-row-key="${rowKey}" style="margin:auto;"></a>`;
  };

  const buildCSendHtml = function (rowKey, row) {
    return `<a href="#" class="farmGod_send_icon farm_icon farm_icon_c" data-send-mode="c" data-origin="${row.origin.id}" data-report="${row.target.report_id}" data-row-key="${rowKey}" style="margin:auto;"></a>`;
  };

  const buildDisabledAHtml = function () {
    return `<span class="farm_icon farm_icon_a decoration" style="margin:auto;display:inline-block;opacity:0.45;"></span>`;
  };

  const buildDisabledCHtml = function () {
  ...
  const parseOriginFarmRowsFromHtml = function ($html, result) {
    $html
      .find('#plunder_list')
      .find('tr[id^="village_"]')
      .each((i, el) => {
  ...
  const fetchOriginCForecastsSerial = async function (originId) {
    ...
    try {
      let firstPageHtml = await ajaxGetWithBackoff(
        `/game.php?village=${originId}&screen=am_farm&order=distance&dir=asc&Farm_page=0`
      );
      let $firstPage = $(firstPageHtml);
      parseOriginFarmRowsFromHtml($firstPage, result);
      result.totalPages = lib.getFarmTotalPages($firstPage);
    } catch (errorObj) {
      result.incomplete = true;
      result.failedPages.push(0);
    ...
    for (let page = 1; page < result.totalPages; page++) {
      ...
      try {
        let html = await ajaxGetWithBackoff(
          `/game.php?village=${originId}&screen=am_farm&order=distance&dir=asc&Farm_page=${page}`
        );
        let $html = $(html);
        parseOriginFarmRowsFromHtml($html, result);
      } catch (errorObj) {
        result.incomplete = true;
    ...
    if (!resolved) {
      if (originResult.incomplete) {
        return {
          ...
          cLight: null,
          forecastUnitsObj: emptyUnitObj(),
          reason: originResult.failedPages && originResult.failedPages.length > 0
            ? `page fetch incomplete (${originResult.failedPages.length} failed pages)`
            : 'page fetch incomplete',
        };
      }
      ...
      plan.farms[originCoord].forEach((row) => {
        ...
        if (row.action_mode === 'a_path') {
          ...
          if (nextUnits) {
            ...
            updateRowCells(rowKey, {
              actionText: 'A send',
              cLightText: '-',
              resolvedText: 'A ready',
              reasonText: 'base A rule',
              farmHtml: buildASendHtml(rowKey, row, plan.templateA.id),
            });
          } else {
            updateRowCells(rowKey, {
              actionText: 'No send',
        ...
        if (!resolved) {
          updateRowCells(rowKey, {
            ...
            reasonText: 'missing resolve state',
            farmHtml: buildDisabledCHtml(),
          });
          return;
        }

        let cLightText = resolved.cLight === null ? '--' : String(resolved.cLight);

        if (resolved.mode === 'c') {
          let nextCUnits = subtractUnitObjs(available, resolved.forecastUnitsObj);

          if (nextCUnits) {
            available = nextCUnits;
            sendableCount++;
            ...
            updateRowCells(rowKey, {
              ...
              reasonText: resolved.reason,
              farmHtml: buildCSendHtml(rowKey, row),
            });
            return;
          }

          if (row.wall < 2) {
            let nextAUnitsAfterCFail = subtractUnitObjs(available, plan.templateA.unitsObj);

            if (nextAUnitsAfterCFail) {
              available = nextAUnitsAfterCFail;
              sendableCount++;

              updateRowCells(rowKey, {
                actionText: 'A send',
                cLightText: cLightText,
                resolvedText: 'A fallback',
                reasonText: 'C units insufficient, wall < 2 -> A fallback',
                farmHtml: buildASendHtml(rowKey, row, plan.templateA.id),
              });
            } else {
              updateRowCells(rowKey, {
                ...
                cLightText: cLightText,
                resolvedText: 'Skip',
                reasonText: 'insufficient C units, then insufficient A units',
                farmHtml: buildDisabledAHtml(),
              });
            }
          } else {
        ...
        if (resolved.mode === 'a') {
          let nextAUnits = subtractUnitObjs(available, plan.templateA.unitsObj);

          if (nextAUnits) {
            available = nextAUnits;
            sendableCount++;
        ...
        if (resolved.mode === 'unresolved') {
          if (row.wall < 2) {
            let nextAUnitsAfterUnreadable = subtractUnitObjs(available, plan.templateA.unitsObj);

            if (nextAUnitsAfterUnreadable) {
              available = nextAUnitsAfterUnreadable;
              sendableCount++;
              ...
              updateRowCells(rowKey, {
                ...
                cLightText: cLightText,
                resolvedText: 'A fallback',
                reasonText: `${resolved.reason}, wall < 2 -> A fallback`,
                farmHtml: buildASendHtml(rowKey, row, plan.templateA.id),
              });
            } else {
              updateRowCells(rowKey, {
                ...
                cLightText: cLightText,
                resolvedText: 'Skip',
                reasonText: `${resolved.reason}, then insufficient A units`,
                farmHtml: buildDisabledAHtml(),
              });
            }
          } else {
        ...
        updateRowCells(rowKey, {
          ...
          cLightText: cLightText,
          resolvedText: 'Unreadable',
          reasonText: resolved.reason || 'unresolved',
          farmHtml: buildDisabledCHtml(),
        });
      });
    });
  ...
  const sendResolvedFarm = function ($this) {
    ...
    const onDone = function (okMessage) {
