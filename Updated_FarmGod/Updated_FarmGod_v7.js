...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const buildActionIcon = function (row) {
    ...
    return `<a href="#" data-origin="${row.origin.id}" data-target="${row.target.id}" data-template="${row.template.id}" class="farmGod_icon farm_icon farm_icon_${row.template.name}" style="margin:auto;opacity:0.45;"></a>`;
  };
        ...
        plan.farms[prop].forEach((val, i) => {
          ...
          let resolvedCell =
            val.action_mode === 'c_preview'
              ...
              : `<td style="text-align:center;">A ready</td>`;

          let reasonCell =
            val.action_mode === 'c_preview'
              ? `<td class="fgReason" data-row-key="${rowKey}" style="text-align:center;">Pending</td>`
              : `<td style="text-align:center;">base A rule</td>`;
  ...
  const updateResolveSummary = function (counts, planToken) {
    ...
    if (!resolveState.ready) {
      ...
      $summary.text(
        `Resolution complete | ready ${counts.cReady}, A fallback ${counts.aFallback}, skip ${counts.skip}, page fetch incomplete ${counts.pageFetchIncomplete}, row not found ${counts.rowNotFound}, c button missing ${counts.cButtonMissing}, forecast missing ${counts.forecastMissing}, forecast parse error ${counts.forecastParseError}, light missing ${counts.lightMissing}`
      );
    }
  };
  ...
  const finalizeTableReadyState = function () {
    resolveState.ready = true;
    resolveState.running = false;
    $('.farmGod_icon').css('opacity', '1');
  };
  ...
  const fetchOriginCForecastsSerial = async function (originId) {
    ...
    while (more) {
      ...
      try {
        ...
        let nextPage = lib.processPage
          ? determineNextPage(page, $html)
          : false;
  ...
  const resolveSinglePreviewRow = function (row, originResult) {
    let resolved =
    ...
    if (!resolved) {
      if (originResult.incomplete) {
        return {
          ...
          reasonText: 'page fetch incomplete',
    ...
    if (!resolved.readable) {
      let reason = resolved.reason || 'row not found';
  ...
  const resolveCPreviewRows = async function (plan, planToken) {
    ...
    for (let i = 0; i < originIds.length; i++) {
      ...
      for (let j = 0; j < originRows.length; j++) {
        ...
        let resolvedPayload = resolveSinglePreviewRow(row, originResult);

        updateResolvedRow(rowKey, resolvedPayload);

        if (counts.hasOwnProperty(resolvedPayload.bucket)) {
          counts[resolvedPayload.bucket] += 1;
        }
