...
window.FarmGod.Main = (function (Library, Translation) {
  ...
  const buildTable = function (plan) {
    ...
    let html = `<div class="vis farmGodContent"><h4>FarmGod</h4>${summary}<div id="FarmGodProgessbar" class="progress-bar live-progress-bar progress-bar-alive" style="width:98%;margin:5px auto;"><div style="background: rgb(146, 194, 0);"></div><span class="label" style="margin-top:0px;"></span></div><table class="vis" width="100%">
                <tr><th style="text-align:center;">${t.table.origin}</th><th style="text-align:center;">${t.table.target}</th><th style="text-align:center;">${t.table.wall}</th><th style="text-align:center;">${t.table.attacked}</th><th style="text-align:center;">${t.table.action}</th><th style="text-align:center;">${t.table.fields}</th><th style="text-align:center;">${t.table.farm}</th></tr>`;

    if (!$.isEmptyObject(plan.farms)) {
      for (let prop in plan.farms) {
        if (game_data.market == 'nl') {
          html += `<tr><td colspan="7" style="background: #e7d098;"><input type="button" class="btn switchVillage" data-id="${plan.farms[prop][0].origin.id}" value="${t.table.goTo} ${plan.farms[prop][0].origin.name} (${plan.farms[prop][0].origin.coord})" style="float:right;"></td></tr>`;
        }
      ...
      html += `<tr><td colspan="7" style="text-align: center;">${t.table.noFarmsPlanned}</td></tr>`;
    }
