...
var version = "7.0.0";
var scriptName = "LA Enhancer Smart (7.0.0)";
...
function run() {
    ...
    if (userset[s.enable_auto_run] != false && !smartConsumeSkipAutoRun()) {
        applySettings();
    }
...
function smartRequestStop(reasonText) {
    ...
    if (!smartState.stopRequested) {
        ...
        smartLog("Stop requested: " + reasonText);
        smartShowInfo("Smart mode stopping: " + reasonText);
    }
}
function smartResolvePendingVillageSwitch() {
    ...
    if (String(window.top.game_data.village.id) === String(smartState.pendingVillageSwitch.villageId)) {
        smartLog("Switched active village to " + window.top.game_data.village.display_name + ".");
...
function smartFindFreshnessCell($row, $targetCell) {
    var $cells = $row.children("td");
    var startIndex = $targetCell.length ? $cells.index($targetCell) + 1 : 0;
    var found = window.top.$();
    $cells.slice(startIndex).each(function () {
        var $cell = window.top.$(this);
        var text = window.top.$.trim($cell.text());
        if (text.indexOf(":") === -1) {
            return;
        }
...
function smartParseResourcesCell($cell) {
    var matches = window.top.$.trim($cell.text()).match(/\d[\d.]*/g);
...
function smartParseWallAndDistance($row, $resourcesCell) {
    ...
    $cells.slice(resourceIndex + 1).each(function () {
        if (window.top.$(this).find("a,input,button,select").length) {
            return;
        }
...
async function smartStartRunner() {
    ...
    smartLog("Starting planner from " + window.top.game_data.village.display_name + ".");
    try {
        ...
        if (smartState.stopRequested || !smartState.isHoldingEnter) {
            smartLog("Runner stopped before execution started.");
            return;
        }
        ...
        smartLog("Runner finished.");
        smartState.isHoldingEnter = false;
        smartState.isRunning = false;
        smartState.stopRequested = false;
        smartState.lastPlan = null;
        smartResetPlanningState();
    }
}
async function smartBuildExecutionPlan() {
    currentGameTime = getCurrentGameTime();
    var targets = smartParseVisibleTargets();
    if (!targets.length) {
        return {jobs: [], groups: []};
    }
    var offensiveGroupId = await smartFetchOffensiveGroupId();
    var sources = await smartFetchOffensiveSources(offensiveGroupId);
    if (!sources.length) {
        throw new Error("No Offensive source villages could be parsed.");
    }
    var jobs = [];
    for (var i = 0; i < targets.length; i++) {
        ...
        var job = smartPlanJobForTarget(targets[i], sources);
        if (job) {
            jobs.push(job);
        }
    }
...
function smartParseVisibleTargets() {
    var $rows = window.top.$("#plunder_list tr:visible").not(":first");
    if (!$rows.length) {
        smartLog("No visible plunder rows found.");
        return [];
    }
    var targets = [];
    $rows.each(function (index) {
        var parsed = smartParseTargetRow(window.top.$(this), index + 1);
        if (parsed) {
            targets.push(parsed);
        }
    });
...
function smartParseTargetRow($row, rowOrder) {
    var targetVillageId = $row.attr("name");
    if (!targetVillageId) {
        var rowIdMatch = ($row.attr("id") || "").match(/village_(\d+)/);
        if (rowIdMatch) {
            targetVillageId = rowIdMatch[1];
        }
    }
    var $placeLink = $row.find("a[href*='screen=place'][href*='target=']").first();
    if (!targetVillageId && $placeLink.length) {
        targetVillageId = smartExtractQueryParam($placeLink.attr("href"), "target");
    }
    if (!targetVillageId) {
        smartLog("Skipping row " + rowOrder + ": target village id missing.");
        return null;
    }
    var $targetLink = $row.find("a[href*='screen=report'][href*='view=']").first();
    var coords = smartParseCoords($targetLink.text() || $row.text());
    if (!coords) {
        smartLog("Skipping row " + rowOrder + " target " + targetVillageId + ": coordinates missing.");
        return null;
    }
    var $targetCell = $targetLink.length ? $targetLink.closest("td") : window.top.$();
    var $freshnessCell = smartFindFreshnessCell($row, $targetCell);
    if (!$freshnessCell.length) {
        smartLog("Skipping row " + rowOrder + " target " + targetVillageId + ": freshness cell missing.");
        return null;
    }
    var attackDate = getVillageAttackedTime($freshnessCell);
    if (!(attackDate instanceof Date) || isNaN(attackDate.getTime())) {
        smartLog("Skipping row " + rowOrder + " target " + targetVillageId + ": freshness time invalid.");
        return null;
    }
    ...
    if (!$resourcesCell.length) {
        smartLog("Skipping row " + rowOrder + " target " + targetVillageId + ": resource cell missing.");
        return null;
    }
    var resources = smartParseResourcesCell($resourcesCell);
    if (!resources || resources.total <= 0) {
        smartLog("Skipping row " + rowOrder + " target " + targetVillageId + ": expected loot unavailable.");
        return null;
    }
    var wallAndDistance = smartParseWallAndDistance($row, $resourcesCell);
    if (!wallAndDistance) {
        smartLog("Skipping row " + rowOrder + " target " + targetVillageId + ": wall/distance cells missing.");
        return null;
    }
    return {
        rowOrder: rowOrder,
        ...
        coords: coords,
        freshnessText: window.top.$.trim($freshnessCell.text()),
...
async function smartFetchOffensiveGroupId() {
    ...
    if (!resolvedIds.length) {
        throw new Error("Could not resolve the static Offensive group id from exact strong, option, or link matches.");
    }
    if (resolvedIds.length > 1) {
        throw new Error("Conflicting Offensive group ids were found across exact strong/option/link matches: " + resolvedIds.join(", ") + ".");
    }
    var groupId = resolvedIds[0];
    smartLog("Resolved Offensive group id " + groupId + ".");
...
async function smartFetchOffensiveSources(groupId) {
    ...
    var doc = smartParseDocument(html);
    var $doc = window.top.$(doc);
    var sources = smartParseOffensiveSources($doc);
    if (!sources.length) {
        throw new Error("The Offensive units overview did not contain any parsable villages.");
    }
    smartLog("Parsed " + sources.length + " Offensive villages.");
    return sources;
}
function smartLocateOffensiveUnitsTable($doc) {
    var bestCandidate = null;
    var scannedTables = 0;
    var tablesWithSpyAndLight = 0;
    var tablesWithVillageLinks = 0;
    $doc.find("table").each(function () {
        scannedTables++;
        var $candidate = window.top.$(this);
        var hasSpyColumn = $candidate.find("img[src*='unit_spy']").length > 0;
        var hasLightColumn = $candidate.find("img[src*='unit_light']").length > 0;
        if (!(hasSpyColumn && hasLightColumn)) {
            return;
        }
        tablesWithSpyAndLight++;
        var overviewLinks = $candidate.find("a[href*='village='][href*='screen=overview']").filter(function () {
            return String(window.top.$(this).attr("href") || "").indexOf("screen=overview_villages") === -1;
        }).length;
        var villageLinks = $candidate.find("a[href*='village=']").length;
        if (villageLinks > 0) {
            tablesWithVillageLinks++;
        }
        var tableText = smartNormalizeText($candidate.text()).toLowerCase();
        var rowTypeSignals = 0;
        if (tableText.indexOf("your own") > -1) {
            rowTypeSignals++;
        }
        if (tableText.indexOf("in village") > -1) {
            rowTypeSignals++;
        }
        if (tableText.indexOf("outwards") > -1) {
            rowTypeSignals++;
        }
        var score = 0;
        score += 100;
        score += overviewLinks * 10;
        score += villageLinks * 3;
        score += rowTypeSignals * 15;
        if (!bestCandidate || score > bestCandidate.score) {
            bestCandidate = {
                $table: $candidate,
                score: score,
                overviewLinks: overviewLinks,
                villageLinks: villageLinks,
                rowTypeSignals: rowTypeSignals
            };
        }
    });
    if (!bestCandidate) {
        smartLog("Units table scan failed: scanned " + scannedTables + " tables, found " + tablesWithSpyAndLight + " with spy/light columns and " + tablesWithVillageLinks + " with village= links.");
        return null;
    }
    smartLog("Selected Offensive units table with score " + bestCandidate.score + " (" + bestCandidate.overviewLinks + " overview links, " + bestCandidate.villageLinks + " village links, " + bestCandidate.rowTypeSignals + " row-type signals).");
    return bestCandidate.$table;
}
function smartFindUnitsVillageLink($row) {
    ...
    return $row.find("a[href*='village=']").first();
}
function smartExtractUnitsRowMeta($row) {
    var $cells = $row.children("th,td");
    var $villageLink = smartFindUnitsVillageLink($row);
    ...
    var coords = smartParseCoords(villageCellText || rowText);
    if (!coords && villageCellText && villageCellText !== rowText) {
        coords = smartParseCoords(rowText);
    }
    if (!coords) {
        return {
            reason: "missing_coords",
            villageId: String(villageId),
            villageName: smartNormalizeText($villageLink.text()) || villageCellText || rowText
        };
    }
    return {
        villageId: String(villageId),
        villageName: smartNormalizeText($villageLink.text()) || villageCellText || ("Village " + villageId),
        coords: coords,
        $cells: $cells
    };
}
function smartParseOffensiveSources($doc) {
    var $table = smartLocateOffensiveUnitsTable($doc);
    if (!$table || !$table.length) {
        throw new Error("Could not locate the Offensive units table: no safe spy/light village overview table was found.");
    }
    var spyIndex = -1;
    var lightIndex = -1;
    $table.find("tr").each(function () {
        ...
        $cells.each(function (index) {
            var src = window.top.$(this).find("img").attr("src") || "";
            if (src.indexOf("unit_spy") > -1) {
                spyIndex = index;
            }
            if (src.indexOf("unit_light") > -1) {
                lightIndex = index;
            }
        });
        if (spyIndex > -1 && lightIndex > -1) {
            return false;
        }
    });
    if (spyIndex < 0 || lightIndex < 0) {
        throw new Error("Could not resolve spy/light columns in the Offensive units table.");
    }
    var sourceMap = {};
    var stats = {
    ...
    $table.find("tr").each(function () {
        ...
        var rowText = smartNormalizeText($row.text()).toLowerCase();
        var rowType = "other";
        if (rowText.indexOf("your own") > -1) {
            rowType = "own";
        } else if (rowText.indexOf("in village") > -1) {
            rowType = "in_village";
        } else if (rowText.indexOf("outwards") > -1) {
            rowType = "outwards";
        }
        if (rowType === "other") {
        ...
        stats.recognizedRows++;
        var meta = smartExtractUnitsRowMeta($row);
        if (meta.reason === "missing_village_link") {
            return;
        }
        stats.rowsWithVillageLinks++;
        if (meta.reason === "missing_village_id") {
            stats.missingVillageId++;
            smartLog("Skipping units row because the village id could not be extracted from a village= link.");
            return;
        }
        if (meta.reason === "missing_coords") {
            stats.missingCoords++;
            smartLog("Skipping Offensive source " + (meta.villageName || meta.villageId || "unknown") + " because coordinates could not be extracted from the row text.");
            return;
        }
        var villageId = meta.villageId;
        var coords = meta.coords;
        var $cells = meta.$cells || $row.children("th,td");
        if ($cells.length <= Math.max(spyIndex, lightIndex)) {
            return;
        }
        var spyCount = smartNormalizeCount($cells.eq(spyIndex).text());
        var lightCount = smartNormalizeCount($cells.eq(lightIndex).text());
        if (spyCount === null || lightCount === null) {
            stats.missingUnitCounts++;
            smartLog("Skipping Offensive source row for " + meta.villageName + " because spy/light counts could not be read safely.");
            return;
        }
        if (!sourceMap[villageId]) {
            sourceMap[villageId] = {
                villageId: String(villageId),
        ...
        if (rowType === "own") {
            if (!entry.own) {
                entry.own = {spy: parseInt(spyCount, 10), light: parseInt(lightCount, 10)};
            } else {
                entry.own.spy = Math.min(entry.own.spy, parseInt(spyCount, 10));
                entry.own.light = Math.min(entry.own.light, parseInt(lightCount, 10));
            }
        }
        if (rowType === "in_village") {
            if (!entry.inVillage) {
                entry.inVillage = {spy: parseInt(spyCount, 10), light: parseInt(lightCount, 10)};
            } else {
                entry.inVillage.spy = Math.min(entry.inVillage.spy, parseInt(spyCount, 10));
                entry.inVillage.light = Math.min(entry.inVillage.light, parseInt(lightCount, 10));
            }
        }
    });
    if (!stats.rowsWithVillageLinks) {
        throw new Error("Found the Offensive units table, but no village rows with village= overview links were parsed.");
    }
    var sources = window.top.$.map(sourceMap, function (value) {
        var availableSpy = null;
        var availableLight = null;
        if (value.own) {
            availableSpy = value.own.spy;
            availableLight = value.own.light;
        }
        if (value.inVillage) {
            availableSpy = availableSpy === null ? value.inVillage.spy : Math.min(availableSpy, value.inVillage.spy);
            availableLight = availableLight === null ? value.inVillage.light : Math.min(availableLight, value.inVillage.light);
        }
        if (availableSpy === null || availableLight === null) {
            smartLog("Skipping Offensive source " + value.villageName + " because sendable spy/light availability could not be resolved safely.");
            return null;
        }
        return {
            ...
            coords: value.coords,
            availableSpy: Math.max(0, parseInt(availableSpy, 10)),
            availableLight: Math.max(0, parseInt(availableLight, 10))
        };
    });
    if (!sources.length) {
        throw new Error("Found the Offensive units table, but no safe villages could be parsed. Rows with village links: " + stats.rowsWithVillageLinks + ", missing village ids: " + stats.missingVillageId + ", missing coords: " + stats.missingCoords + ", unreadable unit rows: " + stats.missingUnitCounts + ", accepted own/in-village rows: " + stats.acceptedRows + ".");
    }
    return sources;
}
function smartPlanJobForTarget(target, sources) {
    var rowLabel = "row " + target.rowOrder + " target " + target.coords.text;
    if (target.outgoingCount > 0) {
        smartLog("Skipping " + rowLabel + ": existing outgoing attacks (" + target.outgoingCount + ").");
        return null;
    }
    if (target.wallLevel > 3) {
        smartLog("Skipping " + rowLabel + ": wall " + target.wallLevel + " exceeds smart limit.");
        return null;
    }
    var sendType = "mixed";
    var freshnessFactor = 1;
    var lightCount = 0;
    var spyCount = 0;
    var minLight = 0;
    if (target.freshnessMinutes <= 120) {
        freshnessFactor = 1;
    } else if (target.freshnessMinutes <= 180) {
        freshnessFactor = 0.75;
    } else if (target.freshnessMinutes <= 240) {
        freshnessFactor = 0.5;
    } else {
        sendType = "spy_only";
        spyCount = 1;
    ...
    if (sendType === "mixed") {
        minLight = smartGetWallMinimumLight(target.wallLevel);
        if (minLight === null) {
            smartLog("Skipping " + rowLabel + ": unsupported wall value " + target.wallLevel + ".");
            return null;
        }
        var adjustedLoot = Math.floor(target.expectedLoot * freshnessFactor);
        var maxFullyLoadedLight = Math.floor(adjustedLoot / smartConfig.light_capacity);
        if (maxFullyLoadedLight < minLight) {
            smartLog("Skipping " + rowLabel + ": " + adjustedLoot + " expected loot can fully load only " + maxFullyLoadedLight + " LC, below wall minimum " + minLight + ".");
            return null;
        }
        lightCount = maxFullyLoadedLight;
    }
    var bestSource = null;
    ...
    for (var i = 0; i < sources.length; i++) {
        ...
        if (sendType === "mixed" && source.availableLight < lightCount) {
            continue;
        }
        var pairKey = source.villageId + ":" + target.targetVillageId;
        if (smartState.planningReservations[pairKey] || smartState.targetReservations[target.targetVillageId]) {
            continue;
        }
    ...
    if (!bestSource) {
        smartLog("Skipping " + rowLabel + ": no eligible Offensive source village.");
        return null;
    }
    ...
    smartLog("Queued " + rowLabel + " from " + bestSource.villageName + " as " + sendType + " (spy " + spyCount + ", lc " + lightCount + ").");
    return {
        key: key,
...
async function smartSwitchToVillage(villageId, infoMessage) {
    ...
    if (gameDataParts.length < 2) {
        throw new Error("Village switch failed: TribalWars.updateGameData missing for village " + villageId + ".");
    }
...
async function smartExecuteJob(job) {
    ...
    if (smartState.sentLedger[ledgerKey]) {
        smartLog("Skipping duplicate queued job " + ledgerKey + ".");
        return;
    }
    if (smartState.inFlightJobs[ledgerKey]) {
        smartLog("Skipping in-flight duplicate job " + ledgerKey + ".");
        return;
    }
    if (String(window.top.game_data.village.id) !== String(job.sourceVillageId)) {
        throw new Error("Active village mismatch before send for " + job.targetCoords.text + ".");
    }
    smartState.inFlightJobs[ledgerKey] = true;
    smartLog("Sending " + job.sendType + " job to " + job.targetCoords.text + " from " + job.sourceVillageName + ".");
    try {
        ...
        smartLog("Confirmed success for " + ledgerKey + ".");
    } finally {
...
async function smartFetchPlacePage(job) {
    ...
    if (!pageGameData || !pageGameData.village || String(pageGameData.village.id) !== String(job.sourceVillageId)) {
        throw new Error("Place page loaded with the wrong active village for " + job.targetCoords.text + ".");
    }
    if (pageGameData.screen !== "place") {
        throw new Error("Place page response is not a place screen for " + job.targetCoords.text + ".");
    }
    return {
        url: url,
        html: html,
...
function smartBuildInitialPlaceRequest(initialPage, job) {
    ...
    if (!$form.length) {
        throw new Error("Place page form for " + job.targetCoords.text + " is missing required unit inputs.");
    }
    ...
    if (!$spyInput.length || !$lightInput.length) {
        throw new Error("Place page form for " + job.targetCoords.text + " does not expose spy/light inputs.");
    }
    var submitDescriptor = smartFindActionSubmit($form, "attack");
    if (!submitDescriptor) {
        throw new Error("Place page form for " + job.targetCoords.text + " is missing the attack submit button.");
    }
    return {
...
function smartBuildFinalConfirmRequest(doc, confirmHtml, job) {
    ...
    if (!$form.length) {
        throw new Error("Confirm page for " + job.targetCoords.text + " is missing the final confirm form.");
    }
    var submitDescriptor = smartFindActionSubmit($form, "submit_confirm");
    if (!submitDescriptor) {
        throw new Error("Confirm page for " + job.targetCoords.text + " is missing submit_confirm.");
    }
    var formAction = $form.attr("action");
    if (!formAction) {
        throw new Error("Confirm page for " + job.targetCoords.text + " is missing the form action URL.");
    }
    var serialized = $form.serializeArray();
...
function smartVerifyConfirmPayload(serialized, confirmHtml, job) {
    ...
    if (sourceVillage !== null && String(sourceVillage) !== String(job.sourceVillageId)) {
        throw new Error("Confirm page source village mismatch for " + job.targetCoords.text + ".");
    }
    ...
    if (spyValue !== null && parseInt(spyValue, 10) !== job.spyCount) {
        throw new Error("Confirm page spy count mismatch for " + job.targetCoords.text + ".");
    }
    if (lightValue !== null && parseInt(lightValue, 10) !== job.lightCount) {
        throw new Error("Confirm page light count mismatch for " + job.targetCoords.text + ".");
    }
}
...
function smartVerifyReturnedSuccess(initialPage, successHtml, successDoc, job) {
    ...
    if (!pageGameData || !pageGameData.village || String(pageGameData.village.id) !== String(job.sourceVillageId)) {
        throw new Error("Success page village mismatch for " + job.targetCoords.text + ".");
    }
    if (pageGameData.screen !== "place") {
        throw new Error("Success page is not a place screen for " + job.targetCoords.text + ".");
    }
    ...
    var i;
    for (i = 0; i < initialPage.outgoingSnapshot.matches.length; i++) {
        beforeFingerprints[initialPage.outgoingSnapshot.matches[i].fingerprint] = true;
        if (initialPage.outgoingSnapshot.matches[i].id) {
            beforeIds[initialPage.outgoingSnapshot.matches[i].id] = true;
        }
    }
    for (i = 0; i < afterSnapshot.matches.length; i++) {
        if (!beforeFingerprints[afterSnapshot.matches[i].fingerprint]) {
            return;
        }
        if (afterSnapshot.matches[i].id) {
            if (!beforeIds[afterSnapshot.matches[i].id]) {
                return;
            }
        }
    }
    throw new Error("Success could not be verified for " + job.targetCoords.text + ": no new outgoing attack row was detected.");
}
...
function migrateKeypressSettings(settings) {
    ...
    if (settings.left_char === "Ã¢â€ Â") {
        settings.left_char = "←";
    }
    if (settings.right_char === "Ã¢â€ â€™") {
        settings.right_char = "→";
    }
    return settings;
}
...
function addLanguages() {
    ...
    window.top.$('#language').append("<option value='ar'>العربية</option>");
}
