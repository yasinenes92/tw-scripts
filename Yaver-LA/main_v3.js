...
var version = "3.0.0";
var scriptName = "LA Enhancer Smart (3.0.0)";
...
async function smartFetchOffensiveGroupId() {
    ...
    $doc.find("a").each(function () {
        ...
        if (text === "Offensive") {
            ...
            groupId = smartExtractQueryParam(href, "group") || smartExtractQueryParam(href, "group_id");
            if (groupId) {
                return false;
            }
        }
    });
    if (!groupId && html.indexOf("Offensive") > -1) {
        var fallbackMatch = html.match(/"group_id":"(\d+)"/);
        if (fallbackMatch) {
            groupId = fallbackMatch[1];
        }
    }
    if (!groupId) {
        throw new Error("Could not resolve the static group id for Offensive.");
    }
...
function smartParseOffensiveSources($doc) {
    ...
    $doc.find("table").each(function () {
        ...
        if ($candidate.find("img[src*='unit_spy']").length && $candidate.find("img[src*='unit_light']").length && $candidate.find("a[href*='screen=info_village']").length) {
            $table = $candidate;
            return false;
        }
    });
    if (!$table || !$table.length) {
        throw new Error("Could not locate the Offensive units table.");
    }
    var spyIndex = -1;
    var lightIndex = -1;
    ...
    $table.find("tr").each(function () {
        ...
        var $infoLink = $row.find("a[href*='screen=info_village']").first();
        if (!$infoLink.length) {
            return;
        }
        var coords = smartParseCoords($infoLink.text());
        if (!coords) {
            return;
        }
        var villageId = smartExtractQueryParam($infoLink.attr("href"), "id");
        if (!villageId) {
            return;
        }
        ...
        if (!sourceMap[villageId]) {
            sourceMap[villageId] = {
                ...
                villageName: window.top.$.trim($infoLink.text()),
                coords: coords,
                own: null,
                inVillage: null,
                fallbackMinSpy: null,
                fallbackMinLight: null
            };
        }
        var entry = sourceMap[villageId];
        if (entry.fallbackMinSpy === null || spyCount < entry.fallbackMinSpy) {
            entry.fallbackMinSpy = parseInt(spyCount, 10);
        }
        if (entry.fallbackMinLight === null || lightCount < entry.fallbackMinLight) {
            entry.fallbackMinLight = parseInt(lightCount, 10);
        }
        if (rowType === "own") {
            if (!entry.own) {
    ...
    return window.top.$.map(sourceMap, function (value) {
        ...
        if (availableSpy === null) {
            availableSpy = value.fallbackMinSpy;
            smartLog("Using fallback spy count for " + value.villageName + ".");
        }
        if (availableLight === null) {
            availableLight = value.fallbackMinLight;
            smartLog("Using fallback light count for " + value.villageName + ".");
        }
...
function smartPlanJobForTarget(target, sources) {
    ...
    if (sendType === "mixed") {
        ...
        var economicallyJustifiedLight = Math.round(adjustedLoot / smartConfig.light_capacity);
        if (economicallyJustifiedLight < minLight) {
            smartLog("Skipping " + rowLabel + ": " + adjustedLoot + " expected loot only justifies " + economicallyJustifiedLight + " LC, below wall minimum " + minLight + ".");
            return null;
        }
        lightCount = economicallyJustifiedLight;
    }
    var bestSource = null;
...
function smartVerifyReturnedSuccess(initialPage, successHtml, successDoc, job) {
    ...
    for (i = 0; i < afterSnapshot.matches.length; i++) {
        if (afterSnapshot.matches[i].id) {
            afterIds[afterSnapshot.matches[i].id] = true;
