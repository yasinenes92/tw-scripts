    ...
    window.onkeydown = function (e) {
        if (editingKey) {
            ...
            if (smartState.isRunning) {
                smartRequestStop("Manual hotkey " + e.which + " pressed.", smartState.dispatchStarted ? "after_current_dispatch" : "abort_before_first_dispatch");
                e.preventDefault();
                return;
            }
...
function smartHandleEnterKeydown() {
    ...
    if (smartState.enterCycleConsumed) {
        smartState.isHoldingEnter = true;
        if (!smartState.enterRepeatLogged) {
            smartState.enterRepeatLogged = true;
            smartLogStage("runner.key", "Run ignored because key cycle already consumed.");
        }
        return;
    }
    smartState.enterCycleConsumed = true;
    smartState.enterRepeatLogged = false;
    smartState.isHoldingEnter = true;
    smartLogStage("runner.key", "Fresh key cycle accepted.");
    if (smartState.isRunning || smartState.startTimerId !== null) {
        return;
    }
    smartStartRunner();
}
function smartHandleEnterKeyup() {
    smartState.isHoldingEnter = false;
    smartState.enterCycleConsumed = false;
    smartState.enterRepeatLogged = false;
    if (smartState.startTimerId !== null) {
        clearTimeout(smartState.startTimerId);
        smartState.startTimerId = null;
    ...
    if (smartState.isRunning) {
        if (!smartState.dispatchStarted && smartState.planningInProgress) {
            smartRequestStop("Enter released.", "after_first_dispatch");
        } else {
            smartRequestStop("Enter released.", "after_current_dispatch");
        }
    }
}
function smartRequestStop(reasonText, stopMode) {
    if (!smartState.isRunning) {
        return;
    }
    var mode = stopMode || "after_current_dispatch";
    if (!smartState.stopRequested) {
        smartState.stopRequested = true;
        smartState.stopReason = reasonText || "stop requested";
    } else if (reasonText && (!smartState.stopReason || mode === "abort_before_first_dispatch")) {
        smartState.stopReason = reasonText;
    }
    if (mode === "abort_before_first_dispatch") {
        smartState.abortBeforeFirstDispatch = true;
        smartState.lastStopPhase = smartState.planningInProgress ? "planning" : "between_phases";
        smartLogStage("runner.stop", "Hard stop requested before first dispatch: " + smartState.stopReason + ".");
        smartShowInfo("Smart mode stopping before dispatch: " + smartState.stopReason);
        return;
    }
    if (mode === "after_first_dispatch") {
        if (!smartState.stopAfterFirstDispatch) {
            smartState.stopAfterFirstDispatch = true;
            smartState.lastStopPhase = smartState.planningInProgress ? "planning" : "between_phases";
            smartLogStage("runner.stop", "Stop requested during planning: " + smartState.stopReason + ". The first dispatch is still guaranteed, then execution will stop.");
            smartShowInfo("Smart mode will stop after the first dispatch: " + smartState.stopReason);
        }
        return;
    }
    if (!smartState.stopAfterCurrentDispatch) {
        smartState.stopAfterCurrentDispatch = true;
        smartState.lastStopPhase = smartState.executionInProgress || smartState.dispatchStarted ? "execution" : "between_phases";
        ...
        smartShowInfo("Smart mode will stop after the current dispatch: " + smartState.stopReason);
    }
}
...
function smartShouldStopDispatch() {
    if (smartState.abortBeforeFirstDispatch) {
        return true;
    }
    if (!smartState.dispatchStarted) {
        return false;
    }
    if (smartState.stopAfterCurrentDispatch) {
        return true;
    }
    if (smartState.stopAfterFirstDispatch && smartState.dispatchCount >= 1) {
        return true;
    }
    return false;
}
...
async function smartStartRunner() {
    if (smartState.isRunning) {
        smartLogStage("runner.key", "Run ignored because the smart runner is already active.");
        return;
    }
    if (!filtersApplied) {
        smartState.isHoldingEnter = false;
    ...
    smartState.dispatchBusy = false;
    smartState.dispatchStarted = false;
    smartState.dispatchCount = 0;
    smartState.dispatcherLeaseGranted = false;
    smartState.stopAfterCurrentDispatch = false;
    smartState.stopAfterFirstDispatch = false;
    smartState.abortBeforeFirstDispatch = false;
    smartState.lastDispatchTimestamp = 0;
    smartState.lastDispatchElapsed = 0;
    smartState.lastDispatchSummary = null;
    smartState.dispatchQueue = [];
    smartState.dispatchIndex = 0;
    ...
    try {
        smartLogStage("plan", "Planning started for smart run #" + smartState.runSerial + ".");
        var plan = await smartBuildExecutionPlan();
        smartState.lastPlan = plan;
        smartState.dispatchQueue = smartBuildDispatchQueue(plan);
        smartState.dispatchIndex = 0;
        smartLogStage("plan", "Planning completed for smart run #" + smartState.runSerial + " with " + plan.jobs.length + " jobs and " + plan.groups.length + " villages.");
        if (!plan.jobs.length) {
        ...
        smartLogStage("dispatcher", "Queue size is " + smartState.dispatchQueue.length + " jobs.");
        ...
        smartState.dispatcherLeaseGranted = true;
        smartLogStage("dispatcher", "Execution lease granted for " + smartState.dispatchQueue.length + " queued jobs.");
...
function smartBuildDispatchQueue(plan) {
    var queue = [];
    ...
    for (var i = 0; i < plan.groups.length; i++) {
        ...
        for (var j = 0; j < group.jobs.length; j++) {
            queue.push(group.jobs[j]);
        }
    }
    return queue;
}
function smartGetElapsedTimeSinceLoadSafe() {
    if (typeof Timing !== "undefined" && typeof Timing.getElapsedTimeSinceLoad === "function") {
        return Timing.getElapsedTimeSinceLoad();
    }
    if (window.top.Timing && typeof window.top.Timing.getElapsedTimeSinceLoad === "function") {
        return window.top.Timing.getElapsedTimeSinceLoad();
    }
    return null;
}
function smartGetRecentFarmClickWaitMs() {
    if (!window.top.Accountmanager || !window.top.Accountmanager.farm) {
        return 0;
    }
    var elapsed = smartGetElapsedTimeSinceLoadSafe();
    var lastClick = window.top.Accountmanager.farm.last_click;
    if (elapsed === null || !lastClick) {
        return 0;
    }
    var diff = elapsed - lastClick;
    if (diff < 200) {
        return 200 - diff;
    }
    return 0;
}
async function smartWaitForDispatchWindow() {
    while (true) {
        var localWait = smartState.lastDispatchTimestamp ? smartConfig.dispatch_interval_ms - (Date.now() - smartState.lastDispatchTimestamp) : 0;
        var recentWait = smartGetRecentFarmClickWaitMs();
        var waitFor = Math.max(0, localWait, recentWait);
        if (!waitFor) {
            return;
        }
...
function smartAcquireDispatchLease(job) {
    if (smartState.dispatchBusy) {
        smartLogStage("dispatcher.busy", "Dispatch skipped because dispatcher busy for " + job.targetCoords.text + ".");
        return false;
    }
    smartState.dispatchBusy = true;
    smartState.dispatchStarted = true;
    var elapsed = smartGetElapsedTimeSinceLoadSafe();
    smartState.lastDispatchElapsed = elapsed || 0;
    if (window.top.Accountmanager && window.top.Accountmanager.farm && elapsed !== null) {
        window.top.Accountmanager.farm.last_click = elapsed;
    }
    smartState.lastDispatchSummary = {
        status: "started",
        dispatchCount: smartState.dispatchCount,
        queueIndex: smartState.dispatchIndex,
        sourceVillageId: job.sourceVillageId,
        sourceVillageName: job.sourceVillageName,
        targetVillageId: job.targetVillageId,
        targetCoords: job.targetCoords ? job.targetCoords.text : "",
        sendType: job.sendType,
        lightCount: job.lightCount,
        spyCount: job.spyCount,
        timestamp: Date.now()
    };
    return true;
}
function smartFinalizeDispatchLease(job, status, details) {
    smartState.dispatchBusy = false;
    smartState.lastDispatchTimestamp = Date.now();
    smartState.lastDispatchSummary = {
        status: status,
        details: details || "",
        dispatchCount: smartState.dispatchCount,
        ...
        lightCount: job.lightCount,
        spyCount: job.spyCount,
        timestamp: smartState.lastDispatchTimestamp
    };
}
function smartGetQueueSnapshot() {
    var queue = smartState.dispatchQueue || [];
    var nextJob = queue[smartState.dispatchIndex] || null;
    return {
        queueSize: queue.length,
        dispatchIndex: smartState.dispatchIndex,
        remaining: Math.max(0, queue.length - smartState.dispatchIndex),
        dispatchStarted: !!smartState.dispatchStarted,
        dispatchBusy: !!smartState.dispatchBusy,
        dispatchCount: smartState.dispatchCount,
        stopAfterFirstDispatch: !!smartState.stopAfterFirstDispatch,
        stopAfterCurrentDispatch: !!smartState.stopAfterCurrentDispatch,
        abortBeforeFirstDispatch: !!smartState.abortBeforeFirstDispatch,
        nextJob: nextJob ? {
            sourceVillageId: nextJob.sourceVillageId,
            sourceVillageName: nextJob.sourceVillageName,
            targetVillageId: nextJob.targetVillageId,
            targetCoords: nextJob.targetCoords ? nextJob.targetCoords.text : "",
            sendType: nextJob.sendType,
            lightCount: nextJob.lightCount,
            spyCount: nextJob.spyCount
        } : null
    };
}
    ...
    try {
        for (var i = 0; i < plan.groups.length; i++) {
            ...
            for (var j = 0; j < group.jobs.length; j++) {
                ...
                await smartWaitForDispatchWindow();
                if (smartShouldStopDispatch()) {
                    smartState.lastExecutionStopStage = "execution_stopped_before_next_dispatch";
                ...
                var job = group.jobs[j];
                if (!smartAcquireDispatchLease(job)) {
                    await smartDelay(25);
                    j -= 1;
                    continue;
                }
                if (smartState.dispatchCount === 0) {
                    smartLogStage("dispatcher", "First dispatch started for " + job.targetCoords.text + " from " + job.sourceVillageName + ".");
                }
                try {
                    await smartExecuteJob(job);
                    smartState.dispatchCount++;
                    smartState.dispatchIndex++;
                    smartFinalizeDispatchLease(job, "success", "");
                    if (smartState.dispatchCount === 1) {
                        smartLogStage("dispatcher", "First dispatch completed for " + job.targetCoords.text + ".");
                    }
                } catch (dispatchError) {
                    smartFinalizeDispatchLease(job, "error", dispatchError.message || String(dispatchError));
                    throw dispatchError;
                }
                if (smartState.stopAfterFirstDispatch && smartState.dispatchCount >= 1) {
                    smartState.lastExecutionStopStage = "execution_stopped_after_first_dispatch";
                    smartLogStage("runner.stop", "Stopping after first dispatch due to key release during planning. Reason: " + (smartState.stopReason || "unknown") + ".");
                    return {stopped: true, stage: smartState.lastExecutionStopStage};
                }
                if (smartState.stopAfterCurrentDispatch) {
                    smartState.lastExecutionStopStage = "execution_stopped_after_current_dispatch";
                    smartLogStage("runner.stop", "Stopping after current dispatch due to key release during execution. Reason: " + (smartState.stopReason || "unknown") + ".");
                    return {stopped: true, stage: smartState.lastExecutionStopStage};
                }
            }
        }
...
function smartCreateDebugStateSnapshot() {
    return {
        ...
        dispatchBusy: !!smartState.dispatchBusy,
        dispatchStarted: !!smartState.dispatchStarted,
        dispatcherLeaseGranted: !!smartState.dispatcherLeaseGranted,
        dispatchCount: smartState.dispatchCount || 0,
        stopAfterCurrentDispatch: !!smartState.stopAfterCurrentDispatch,
        stopAfterFirstDispatch: !!smartState.stopAfterFirstDispatch,
        abortBeforeFirstDispatch: !!smartState.abortBeforeFirstDispatch,
        enterCycleConsumed: !!smartState.enterCycleConsumed,
        lastDispatchTimestamp: smartState.lastDispatchTimestamp || 0,
        ...
        queueSnapshot: smartGetQueueSnapshot(),
        lastDispatchSummary: smartState.lastDispatchSummary ? smartClonePlainObject(smartState.lastDispatchSummary) : null
    };
}
function smartCreatePlanSnapshot(plan) {
    if (!plan) {
        return null;
    }
    var snapshot = {
        groupId: plan.groupId || null,
        diagnostics: smartClonePlainObject(plan.diagnostics || {}),
        jobs: [],
        groups: []
    };
    if (plan.jobs && plan.jobs.length) {
        for (var i = 0; i < plan.jobs.length; i++) {
            snapshot.jobs.push({
                sourceVillageId: plan.jobs[i].sourceVillageId,
                sourceVillageName: plan.jobs[i].sourceVillageName,
                targetVillageId: plan.jobs[i].targetVillageId,
                targetCoords: plan.jobs[i].targetCoords ? plan.jobs[i].targetCoords.text : "",
                sendType: plan.jobs[i].sendType,
                lightCount: plan.jobs[i].lightCount,
                spyCount: plan.jobs[i].spyCount,
                rowOrder: plan.jobs[i].rowOrder
            });
        }
    }
    if (plan.groups && plan.groups.length) {
        for (var j = 0; j < plan.groups.length; j++) {
            snapshot.groups.push({
                sourceVillageId: plan.groups[j].sourceVillageId,
                sourceVillageName: plan.groups[j].sourceVillageName,
                jobCount: plan.groups[j].jobs.length
            });
        }
    }
    return snapshot;
...
async function smartDebugBuildPlan() {
    ...
    var backup = {
        ...
        abortBeforeFirstDispatch: smartState.abortBeforeFirstDispatch,
        enterCycleConsumed: smartState.enterCycleConsumed,
        enterRepeatLogged: smartState.enterRepeatLogged,
        lastDispatchTimestamp: smartState.lastDispatchTimestamp,
        lastDispatchElapsed: smartState.lastDispatchElapsed,
        lastDispatchSummary: smartState.lastDispatchSummary ? smartClonePlainObject(smartState.lastDispatchSummary) : null,
        dispatchQueue: smartState.dispatchQueue ? smartState.dispatchQueue.slice() : [],
        dispatchIndex: smartState.dispatchIndex,
        pendingVillageSwitch: smartState.pendingVillageSwitch ? {villageId: smartState.pendingVillageSwitch.villageId} : null,
    ...
    try {
        ...
        smartState.lastPlan = plan;
        smartState.dispatchQueue = smartBuildDispatchQueue(plan);
        return smartCreatePlanSnapshot(plan);
    } finally {
        ...
        smartState.dispatchBusy = backup.dispatchBusy;
        smartState.dispatchStarted = backup.dispatchStarted;
        smartState.dispatchCount = backup.dispatchCount;
        smartState.dispatcherLeaseGranted = backup.dispatcherLeaseGranted;
        smartState.stopAfterCurrentDispatch = backup.stopAfterCurrentDispatch;
        smartState.stopAfterFirstDispatch = backup.stopAfterFirstDispatch;
        smartState.abortBeforeFirstDispatch = backup.abortBeforeFirstDispatch;
        smartState.enterCycleConsumed = backup.enterCycleConsumed;
        smartState.enterRepeatLogged = backup.enterRepeatLogged;
        smartState.lastDispatchTimestamp = backup.lastDispatchTimestamp;
        smartState.lastDispatchElapsed = backup.lastDispatchElapsed;
        smartState.lastDispatchSummary = backup.lastDispatchSummary;
        smartState.dispatchQueue = backup.dispatchQueue;
        smartState.dispatchIndex = backup.dispatchIndex;
...
function smartInstallDebugSurface() {
    var debugApi = {
        ...
        getLastPlan: function () {
            return smartCreatePlanSnapshot(smartState.lastPlan);
        },
        getQueueSnapshot: function () {
            return smartGetQueueSnapshot();
        },
        getLastDispatchSummary: function () {
            return smartState.lastDispatchSummary ? smartClonePlainObject(smartState.lastDispatchSummary) : null;
        },
        getState: function () {
