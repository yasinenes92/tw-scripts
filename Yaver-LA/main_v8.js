...
var version = "8.0.0";
var scriptName = "LA Enhancer Smart (8.0.0)";
var scriptURL = "https://github.com/yasinenes92/tw-scripts";
...
function smartRequestStop(reasonText) {
    ...
    if (!smartState.stopRequested) {
        ...
        smartLogStage("runner.stop", "Stop requested: " + reasonText);
...
function smartShouldStopDispatch() {
    return smartState.stopRequested || !smartState.isHoldingEnter;
}
...
function smartParseVisibleTargets() {
    ...
    var $rows = window.top.$("#plunder_list tr[id^='village_']").filter(":visible");
    result.visibleRowCount = $rows.length;
    if (!$rows.length) {
        smartLogStage("targets.none", "No visible target rows were found after current LA filtering.");
        return result;
    }
    ...
    if (!result.targets.length) {
        smartLogStage("targets.none", "Visible target rows were found, but none could be parsed safely. Reasons: " + smartFormatCounterMap(result.malformedCounts) + ".");
        return result;
    }
    smartLogStage("targets", "Parsed " + result.targets.length + " visible targets from " + result.visibleRowCount + " visible rows.");
    if (Object.keys(result.malformedCounts).length) {
        smartLogStage("targets.row", "Ignored malformed visible rows: " + smartFormatCounterMap(result.malformedCounts) + ".");
    }
    return result;
}
...
async function smartStartRunner() {
    ...
    try {
        ...
        if (smartShouldStopDispatch()) {
            smartLogStage("runner.stop", "Runner stopped before execution started.");
            return;
        }
...
async function smartBuildExecutionPlan() {
    ...
    var diagnostics = {
        noVisibleTargets: !targetParse.visibleRowCount,
        malformedTargetCounts: targetParse.malformedCounts || {},
        planningSkipCounts: {}
    };
...
function smartClassifySourceRowType(rowTextLower) {
    if (rowTextLower.indexOf("your own") > -1) {
        return "own";
    }
    if (/\bin village\b|\bhere\b/.test(rowTextLower)) {
        return "in_village";
    }
    if (rowTextLower.indexOf("outwards") > -1) {
        return "outwards";
    }
    if (rowTextLower.indexOf("support") > -1) {
        return "support";
    }
    return "other";
}
...
function smartInspectUnitsTable($table, tableIndex) {
    ...
    $rows.each(function () {
        ...
        var rowType = smartClassifySourceRowType(rowText.toLowerCase());
        if (rowType === "other") {
            return;
        }
        ...
        if (meta.reason === "missing_village_id") {
            smartLogStage("sources.row", "Skipping recognized source row because the village id could not be extracted from village= or id= query parameters.");
            return;
        }
        if (meta.reason === "missing_coords") {
            smartLogStage("sources.row", "Skipping recognized source row for " + (meta.villageName || meta.villageId || "unknown") + " because coordinates could not be extracted from the first village cell or full row text.");
            return;
        }
        if (meta.reason) {
            return;
        ...
        if ($cells.length <= maxColumnIndex) {
            smartLogStage("sources.row", "Skipping recognized source row for " + meta.villageName + " because the row does not expose enough direct cells for the spy/light columns.");
            return;
        }
        ...
        if (spyCount === null || lightCount === null) {
            smartLogStage("sources.row", "Skipping recognized source row for " + meta.villageName + " because spy/light counts could not be read safely.");
            return;
        }
        report.rowsWithReadableCounts++;
        uniqueVillageIds[meta.villageId] = true;
        if (rowType === "own" || rowType === "in_village") {
            report.sendableRows++;
        }
...
function smartParseOffensiveSources($doc) {
    ...
    for (var i = 0; i < tableReport.parsedRows.length; i++) {
        ...
        if (row.rowType === "own") {
            if (!sourceMap[row.villageId].own) {
                sourceMap[row.villageId].own = {spy: row.spyCount, light: row.lightCount};
            } else {
                sourceMap[row.villageId].own.spy = Math.min(sourceMap[row.villageId].own.spy, row.spyCount);
                sourceMap[row.villageId].own.light = Math.min(sourceMap[row.villageId].own.light, row.lightCount);
            }
        }
        if (row.rowType === "in_village") {
            if (!sourceMap[row.villageId].inVillage) {
                sourceMap[row.villageId].inVillage = {spy: row.spyCount, light: row.lightCount};
            } else {
                sourceMap[row.villageId].inVillage.spy = Math.min(sourceMap[row.villageId].inVillage.spy, row.spyCount);
                sourceMap[row.villageId].inVillage.light = Math.min(sourceMap[row.villageId].inVillage.light, row.lightCount);
            }
        }
    }
    var sources = [];
    window.top.$.each(sourceMap, function (_, entry) {
        ...
        if (entry.own) {
            availableSpy = entry.own.spy;
            availableLight = entry.own.light;
        }
        if (entry.inVillage) {
            availableSpy = availableSpy === null ? entry.inVillage.spy : Math.min(availableSpy, entry.inVillage.spy);
            availableLight = availableLight === null ? entry.inVillage.light : Math.min(availableLight, entry.inVillage.light);
        }
        if (availableSpy === null || availableLight === null) {
            smartLogStage("sources.row", "Skipping Offensive source " + entry.villageName + " because sendable spy/light availability could not be resolved safely from own/in-village rows.");
            return;
        }
        sources.push({
