...
var version = "2.0.0";
var scriptName = "LA Enhancer Smart (2.0.0)";
...
function smartHandleEnterKeydown() {
    ...
    if (smartState.isHoldingEnter || smartState.isRunning) {
...
function smartParseOffensiveSources($doc) {
    ...
    $table.find("tr").each(function () {
        ...
        if (!sourceMap[villageId] || sourceMap[villageId].availableSpy < spyCount || sourceMap[villageId].availableLight < lightCount) {
            sourceMap[villageId] = {
                villageId: String(villageId),
                villageName: window.top.$.trim($infoLink.text()),
                coords: coords,
                availableSpy: parseInt(spyCount, 10),
                availableLight: parseInt(lightCount, 10)
            };
        }
    });
...
function smartPlanJobForTarget(target, sources) {
    ...
    if (sendType === "mixed") {
        if (target.wallLevel === 0) {
            minLight = 10;
        } else if (target.wallLevel === 1) {
            minLight = 14;
        } else if (target.wallLevel === 2) {
            minLight = 20;
        } else if (target.wallLevel === 3) {
            minLight = 33;
        }
        var adjustedLoot = Math.floor(target.expectedLoot * freshnessFactor);
        lightCount = Math.max(minLight, Math.ceil(adjustedLoot / smartConfig.light_capacity));
        if (adjustedLoot < lightCount * smartConfig.light_capacity) {
            smartLog("Skipping " + rowLabel + ": " + adjustedLoot + " expected loot would not fully load " + lightCount + " LC.");
            return null;
        }
    }
    var bestSource = null;
    ...
    for (var i = 0; i < sources.length; i++) {
        ...
        if (smartState.planningReservations[pairKey]) {
            continue;
        }
...
async function smartExecuteJob(job) {
    ...
    smartVerifyReturnedSuccess(initialPage, successDoc, job);
...
async function smartFetchPlacePage(job) {
    ...
    return {
        ...
        html: html,
        doc: doc,
        matchingOutgoingCount: smartCountMatchingOutgoing($doc, job)
    };
}
...
function smartVerifyReturnedSuccess(initialPage, successDoc, job) {
    var $doc = window.top.$(successDoc);
    var afterCount = smartCountMatchingOutgoing($doc, job);
    if (afterCount <= initialPage.matchingOutgoingCount) {
        throw new Error("Success could not be verified for " + job.targetCoords.text + ": outgoing attack row not detected.");
    }
}
function smartCountMatchingOutgoing($doc, job) {
    var count = 0;
    var targetText = "Attack on Barbarian village (" + job.targetCoords.text + ")";
    $doc.find("tr").each(function () {
        var $row = window.top.$(this);
        var text = $row.text();
        var html = $row.html() || "";
        var type = $row.attr("data-command-type") || "";
        if (text.indexOf(targetText) === -1) {
            return;
        }
        if (type !== "attack" && html.indexOf("data-command-type=\"attack\"") === -1) {
            return;
        }
        var hasSpySignal = html.indexOf("command/spy") > -1 || html.indexOf("spy.webp") > -1 || html.indexOf("unit_spy") > -1;
        var hasFarmSignal = html.indexOf("command/farm") > -1 || text.indexOf("Looting attack") > -1;
        if (job.sendType === "spy_only") {
            if (!hasSpySignal) {
                return;
            }
        } else if (!hasSpySignal && !hasFarmSignal) {
            return;
        }
        count++;
    });
